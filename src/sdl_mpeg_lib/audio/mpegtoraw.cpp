...
{
	static bool initialized = MY_FALSE;

	last_speed			= 0;
	stereo				= true;
	forcetomonoflag 	= MY_FALSE;
	forcetostereoflag	= MY_FALSE;
	swapendianflag		= MY_FALSE;
	downfrequency		= 0;
...
bool MPEGaudio::loadheader()
{
	...
	if (frequency == 3)			{return (MY_FALSE);}
	c >>= 2;
	bitrateindex 	= (int) c;
	if (bitrateindex == 15)		{return (MY_FALSE);}
...
//
	...
	if (layer==1)
	{
		framesize = (12000 * bitrate[version][0][bitrateindex]) / frequencies[version][frequency];
		if (frequency==frequency44100 && padding)	{framesize++;}
		framesize <<= 2;
	}
	else
	{
		framesize = (144000*bitrate[version][layer-1][bitrateindex]) / (frequencies[version][frequency]<<version);
		if (padding)	{framesize++;}
		if (layer==3)
		{
			if (version)
					{layer3slots = framesize - ((mode==single)? 9:17) - (protection?0:2) - 4;}
			else	{layer3slots = framesize - ((mode==single)?17:32) - (protection?0:2) - 4;}
		}
	}
	#ifdef DEBUG_AUDIO
...
int Play_MPEGaudio(MPEGaudio *audio, u8 *stream, int render_len)
{
	...
	/* this is empirical, I don't realy know how to find out when
	   ...
	   quite right */
		...
		{
			PSPL_MixAudio(stream, rbuf, render_len, (PSPL128_MIX_MAX_VOLUME)/*volume*/);
			mixed += render_len;
		...
		{
			PSPL_MixAudio(stream, rbuf, copylen, (PSPL128_MIX_MAX_VOLUME)/*volume*/);
			mixed += copylen;
			audio->currentframe++;
			audio->ring->ReadDone();
		//	error(0/*ERR_FATAL*/, "-");
	...
	if (audio->rawdatawriteoffset >= audio->rawdatareadoffset)
	{
		...
		if (copylen >= render_len)
		{
			PSPL_MixAudio(stream, (u8 *)&audio->spillover[audio->rawdatareadoffset], (render_len+render_len), (PSPL128_MIX_MAX_VOLUME)/*volume*/);
			mixed += (render_len+render_len);
			audio->rawdatareadoffset += render_len;
			goto finished_mixing;
		}
		PSPL_MixAudio(stream, (u8 *)&audio->spillover[audio->rawdatareadoffset], (copylen+copylen), (PSPL128_MIX_MAX_VOLUME)/*volume*/);
	...
	if (audio->run(1))
	{
		...
		#endif
		PSPL_MixAudio(stream, (u8 *) audio->spillover, (render_len+render_len), (PSPL128_MIX_MAX_VOLUME)/*volume*/);
		mixed += (render_len+render_len);
		audio->rawdatareadoffset = render_len;
	}
	else
