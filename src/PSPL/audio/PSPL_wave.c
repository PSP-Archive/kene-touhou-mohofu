...
static int ReadChunk(SDL_RWops *src, Wave_Chunk *chunk);
...
static int InitMS_ADPCM(Wave_FMT *format)
{
	u8 *rogue_feel;
	...
	u8 nybble, s16 *coeff)
{
	...
	if (new_sample < min_audioval)	{new_sample = min_audioval;}
	else
	if (new_sample > max_audioval)	{new_sample = max_audioval;}
...
static int InitIMA_ADPCM(Wave_FMT *format)
{
	u8 *rogue_feel;
	...
	*audio_buf = (u8 *)malloc(*audio_len);
	...
	while (encoded_len >= IMA_ADPCM_state.wavefmt.blockalign)
	{
		...
		while (0 < samplesleft)
		{
			for (ccc=0; ccc<channels; ccc++)
			{
				Fill_IMA_ADPCM_block(decoded, encoded, ccc, channels, &state[ccc]);
				encoded 	+= 4;
				samplesleft -= 8;
			}
...
//
	...
	*audio_buf = NULL;
	...
	samplesize	= ((spec->format & 0xff)>>3) * spec->channels_is_load_wave_only;
	*audio_len	&= ~(samplesize-1);

done:
	if (format != NULL)		{free(format);}
	if (freesrc && src)		{SDL_RWclose(src);}
	if (was_error)			{spec = NULL;}
	return (spec);
}
