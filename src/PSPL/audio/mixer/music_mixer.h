...
typedef struct
{
//[重複(channel_volumeで設定すれば不要)]	s32 u8_volume;/*(再生音量)*/		/* Per-sample volume, 0-128 */
	u8 *www_abuf;/*(音の生データ)*/
	u32 www_alen;/*(音の生データの長さ)*/
	int allocated;/*(有効/無効)*/	//[重複]
//	u8 u8_volume;		/* Per-sample volume, 0-128 */
//	u8 dummy1;
//	u8 dummy2;
//	u8 dummy3;
} PSPL_MIX_VOICE_CHUNK;
...
extern PSPL_MIX_VOICE_CHUNK * /*SD LCALL*/ Mix_LoadWAV_RW(SDL_RWops *src, int freesrc);
...
/*模倣風で使用していない*/ //extern PSPL_MIX_VOICE_CHUNK * /*SD LCALL*/ Mix_QuickLoad_WAV(u8 *mem);

/* Load raw audio data of the mixer format from a memory buffer */
/*模倣風で使用していない*/ //extern PSPL_MIX_VOICE_CHUNK * /*SD LCALL*/ Mix_QuickLoad_RAW(u8 *mem, u32 len);

#endif /*00*/
/* Free an audio chunk previously loaded */
extern void /*SD LCALL*/ Mix_FreeChunk(PSPL_MIX_VOICE_CHUNK *chunk);
...
extern int /*SD LCALL*/ Mix_PlayChannelTimed(int channel, PSPL_MIX_VOICE_CHUNK *chunk, int loops, int ticks);
...
/*模倣風で使用していない*/ //extern int /*SD LCALL*/ Mix_FadeInChannelTimed(int channel, PSPL_MIX_VOICE_CHUNK *chunk, int loops, int ms, int ticks);
...
/*模倣風で使用していない*/ //extern int /*SD LCALL*/ Mix_VolumeChunk(PSPL_MIX_VOICE_CHUNK *chunk, int volume);
...
/* Get the PSPL_MIX_VOICE_CHUNK currently associated with a mixer channel
    Returns NULL if it's an invalid channel, or there's no chunk associated.
*/
extern PSPL_MIX_VOICE_CHUNK * /*SD LCALL*/ Mix_GetChunk(int channel);
