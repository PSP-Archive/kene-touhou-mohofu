...
static unsigned int music_volume = PSPL_MIXER_MAX_VOLUME;
...
static void music_internal_volume(int volume)
{
	...
	#if defined(MP3_MUSIC)
	case MUS_MP3:
	//	SMPEG_setvolume(music_playing->data.mp3, (int)(((float)volume/(float)PSPL_MIXER_MAX_VOLUME)*100.0));
...
Mix_Music *Mix_LoadMUS(const char *file)
{
	...
	music = (Mix_Music *)memalign(16, sizeof(Mix_Music));
	if (music == NULL)
	{
...
int Mix_SetMusicCMD(const char *command)
{
	...
	if (command)
	{
		music_cmd = (char *)memalign(16, strlen(command)+1);
		if (music_cmd == NULL)
		{
			return (-1);
		}
