...
static unsigned int wavestream_volume = PSPL_MIXER_MAX_VOLUME;
...
static int WAV_ReadChunk(SDL_RWops *src, Chunk *chunk, int read_data)
{
	...
	if (read_data)
	{
		chunk->data = (u8 *)memalign(16, chunk->length);
		if (chunk->data == NULL)
		{
...
WAVStream *WAVStream_LoadSong(const char *file, const char *magic)
{
	...
	wave = (WAVStream *)memalign(16, sizeof *wave);
	if (wave)
	{
	...
	if (music && ((pos=ftell(music->wavefp)) < music->stop))
	{
		if (music->cvt.needed)
		{
			...
			if (music->cvt.len != original_len)
			{
				...
				music->cvt.buf = (u8 *)memalign(16, worksize);
				if (music->cvt.buf == NULL)
				{
					return;
				}
