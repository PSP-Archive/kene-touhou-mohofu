...
PSPL_AUDIO_DEVICE *current_audio = NULL;
...
{
	PSPL_AUDIO_DEVICE 	*audio = (PSPL_AUDIO_DEVICE *)audiop;
	u8	*stream;
	int 	stream_len;
	void	*udata;
...
//	if (audio->ThreadInit)
	...
	//	void (*ThreadInit)(PSPL_AUDIO_DEVICE *this);		/* Called by audio thread at start */
	//	static  void PSPSDLAUDIO_ThreadInit(PSPL_AUDIO_DEVICE *this)
	{
...
//		silence = audio->spec.silence;
	...
	//	//	static void PSPAUD_WaitAudio(PSPL_AUDIO_DEVICE *this)
	//	//	{
	...
	//	}
		...
		//	u8 *(*GetAudioBuf)(PSPL_AUDIO_DEVICE *this);
		//	static u8 *s_PSPSDLAUDIO_GetAudioBuf(PSPL_AUDIO_DEVICE *this)
		...
		//	void (*PlayAudio)(PSPL_AUDIO_DEVICE *this);
		//	PSPSDLAUDIO_PlayAudio(audio);
		//	static void PSPSDLAUDIO_PlayAudio(PSPL_AUDIO_DEVICE *this)
...
//	int  (*OpenAudio)(PSPL_AUDIO_DEVICE *this, PSPL_AUDIO_SPEC *spec);
static int PSPSDLAUDIO_OpenAudio(PSPL_AUDIO_DEVICE *this, PSPL_AUDIO_SPEC *spec)
{
	...
	//	}
		...
		spec->size_1024 *= spec->samples;
		#else
		spec->size_1024 = (spec->samples << 2);// = /*((spec->format&0xff)/8)*/(2) * (2)/*spec->channels*/ * spec->samples;
		#endif
	//	spec->size_1024 = (1024);/*psp*/
	}
...
{
	PSPL_AUDIO_DEVICE *audio = current_audio;
	if (audio)
	{
		audio->enabled = 0;
		if (audio->thread != NULL)
...
/*only boot once*/void PSPL_AudioInit(void/*const char *driver_name_dummy*/)
{
	...
	current_audio = NULL;
	current_audio = (PSPL_AUDIO_DEVICE *)memalign(16, sizeof(PSPL_AUDIO_DEVICE));
...
//	PSPL_AUDIO_DEVICE *audio;
	...
	//	static PSPL_AUDIO_DEVICE *PSPAUD_CreateDevice(int devindex)
...
/* This function waits until it is possible to write a full sound buffer */
//廃止	void (*WaitAudio)(PSPL_AUDIO_DEVICE *this);
//static void PSPSDLAUDIO_WaitAudio(PSPL_AUDIO_DEVICE *this)
//{
//	/* Because we block when sending audio, there's no need for this function to do anything. */
//}
...
void PSPL_UnlockAudio(void)
{
	...
	//	static void SDL_Unl ockAud io_Default(PSPL_AUDIO_DEVICE *audio)
	//	static void PSPSDLAUDIO_UnlockAudio(PSPL_AUDIO_DEVICE *this)
		if (audio->thread
...
void PSPL_LockAudio(void)
{
	...
	//	static void SDL_Loc kAud io_Default(PSPL_AUDIO_DEVICE *audio)
	//	static void PSPSDLAUDIO_LockAudio(PSPL_AUDIO_DEVICE *this)
		if (audio->thread
			&& (SDL_ThreadID() == audio->threadid))
		{
			return;
