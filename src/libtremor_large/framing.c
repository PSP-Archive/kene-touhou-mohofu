...
{
	ogg_buffer_state *bs = u_ogg_calloc(1, sizeof(*bs));
	return (bs);
}
...
static void u_ogg_buffer_destroy(ogg_buffer_state *bs)
{
	ogg_buffer *bt;
	ogg_reference *rt;
	if (bs->shutdown)
...
static ogg_buffer *_fetch_buffer(ogg_buffer_state *bs, long bytes)
{
	...
	if (bs->unused_buffers)
	{
		...
		if (ob->size < bytes)
		{
			ob->data	= u_ogg_realloc(ob->data, bytes);
			ob->size	= bytes;
		}
	}
	else
	{
		...
		ob			= u_ogg_malloc(sizeof(*ob));
		ob->data	= u_ogg_malloc(bytes<16?16:bytes);
		ob->size	= bytes;
	}
	ob->refcount	= 1;
	ob->ptr.owner	= bs;
...
static void u_ogg_buffer_mark_one(ogg_reference *or)
{
	or->buffer->refcount++;
}
...
static void ogg_buffer_release_one(ogg_reference *or)
{
	...
	u_ogg_buffer_destroy(bs); /* lazy cleanup (if needed) */
}
...
{
	ogg_sync_state *oy=u_ogg_calloc(1, sizeof(*oy));
...
{
	ogg_stream_state *os=u_ogg_calloc(1, sizeof(*os));
