	...
	long m)
{
	...
	if (searched>=end || ret<0)
	{
		ogg_page_release(&og);
		vf->links=m+1;
		vf->offsets=u_ogg_malloc((vf->links+1)*sizeof(*vf->offsets));
		vf->serialnos=u_ogg_malloc(vf->links*sizeof(*vf->serialnos));
	...
	ogg_page *og_ptr)
{
	...
	//void vorbis_info_init(vorbis_info *vi)
	{
		...
		vi->codec_setup=(codec_setup_info *)u_ogg_calloc(1,sizeof(codec_setup_info));
	}
...
static void _prefetch_all_headers(OggVorbis_File *vf, ogg_int64_t dataoffset)
{
	...
	vf->vi				= u_ogg_realloc(vf->vi,vf->links*sizeof(*vf->vi));
	vf->vc				= u_ogg_realloc(vf->vc,vf->links*sizeof(*vf->vc));
	vf->dataoffsets 	= u_ogg_malloc(vf->links*sizeof(*vf->dataoffsets));
	vf->pcmlengths		= u_ogg_malloc(vf->links*2*sizeof(*vf->pcmlengths));
	...
	long ibytes, ov_callbacks callbacks)
{
	...
	vf->links=1;
	vf->vi=u_ogg_calloc(vf->links,sizeof(*vf->vi));
	vf->vc=u_ogg_calloc(vf->links,sizeof(*vf->vc));
...
int ov_clear(OggVorbis_File *vf)
{
	if (vf)
	{
		...
		if (vf->dataoffsets)u_ogg_free(vf->dataoffsets);
		if (vf->pcmlengths)u_ogg_free(vf->pcmlengths);
		if (vf->serialnos)u_ogg_free(vf->serialnos);
		if (vf->offsets)u_ogg_free(vf->offsets);
