...
static void floor0_free_look(vorbis_look_floor *i)
{
  ...
  if (look)
  {
    if (look->linearmap)u_ogg_free(look->linearmap);
    if (look->lsp_look)u_ogg_free(look->lsp_look);
...
static vorbis_info_floor *floor0_unpack (vorbis_info *vi,oggpack_buffer *opb)
{
  ...
  vorbis_info_floor0 *info=(vorbis_info_floor0 *)u_ogg_malloc(sizeof(*info));
                              ...
                              vorbis_info_floor *i)
{
  ...
  codec_setup_info   *ci=(codec_setup_info *)vi->codec_setup;
  vorbis_info_floor0 *info=(vorbis_info_floor0 *)i;
  vorbis_look_floor0 *look=(vorbis_look_floor0 *)u_ogg_calloc(1,sizeof(*look));
  look->m=info->order;
  ...
  /* the mapping from a linear scale to a smaller bark scale is
     ...
     accurate */
  look->linearmap=(int *)u_ogg_malloc((look->n+1)*sizeof(*look->linearmap));
  for (j=0;j<look->n;j++)
  {
    int val=(look->ln*
  ...
  look->lsp_look=(ogg_int32_t *)u_ogg_malloc(look->ln*sizeof(*look->lsp_look));
  for (j=0;j<look->ln;j++)
