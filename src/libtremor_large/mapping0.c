...
static void mapping0_free_look(vorbis_look_mapping *look)
{
  ...
  if (l)
  {
    ...
    u_ogg_free(l->floor_func);
    u_ogg_free(l->residue_func);
    u_ogg_free(l->floor_look);
    u_ogg_free(l->residue_look);
    memset(l,0,sizeof(*l));
    u_ogg_free(l);
  }
}
                          ...
                          vorbis_info_mapping *m)
{
  ...
  vorbis_look_mapping0 *look=(vorbis_look_mapping0 *)u_ogg_calloc(1,sizeof(*look));
  vorbis_info_mapping0 *info=look->map=(vorbis_info_mapping0 *)m;
  look->mode=vm;

  look->floor_look=(vorbis_look_floor **)u_ogg_calloc(info->submaps,sizeof(*look->floor_look));

  look->residue_look=(vorbis_look_residue **)u_ogg_calloc(info->submaps,sizeof(*look->residue_look));

  look->floor_func=(vorbis_func_floor **)u_ogg_calloc(info->submaps,sizeof(*look->floor_func));
  look->residue_func=(vorbis_func_residue **)u_ogg_calloc(info->submaps,sizeof(*look->residue_func));
...
{
  int i;
  vorbis_info_mapping0 *info=(vorbis_info_mapping0 *)u_ogg_calloc(1,sizeof(*info));
  codec_setup_info     *ci=(codec_setup_info *)vi->codec_setup;
  memset(info,0,sizeof(*info));
