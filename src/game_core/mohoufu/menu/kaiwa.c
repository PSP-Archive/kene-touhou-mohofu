...
extern void kaiwa_system_set_re_draw(void)
{
...
//
	...
	PSPL_UpperBlit(s2, NULL, cg.sdl_screen[SDL_01_BACK_SCREEN], NULL);
	SDL_FreeSurface(s2);
	s2	= NULL;
}
...
//	sitei_number = psp_min(sitei_number, (KAIWA_OBJ_99_MAX-1));
	#if (1==USE_VS3)
	/*(とりあえず)*/
	kaiwa_sprite[(sitei_number&(4-1))].draw_flag = (on_off);	/* 描画on/off。 */
	#else
...
static void kaiwa_system_vbl_sprite_move(void)
{
	...
	for (nnn=0; nnn<KAIWA_OBJ_99_MAX; nnn++)
	{
		...
		if (KAIWA_OBJ_MOVE_FLAG_00_MOVE_COMPLETE != my_std_obj->move_flag)
		{
			...
			//------------------
			#if (1)/*(到達割合を考慮して、合成する。)*/
			VR04	= ((my_std_obj->alt.x256)); 	/*(合成位置[A]t256()形式)*/
			VR05	= ((my_std_obj->alt.y256)); 	/*(合成位置[A]t256()形式)*/
			VR06	= ((my_std_obj->target.x256));	/*(合成位置[B]t256()形式)*/
			VR07	= ((my_std_obj->target.y256));	/*(合成位置[B]t256()形式)*/
			...
			// VR07: 合成値
			//-----------------------
			my_std_obj->center.x256 = (VR06);
			my_std_obj->center.y256 = (VR07);
		}
	}
}
...
//	cg_state_flag		&= (~SCENE_NUMBER_MASK);	/*(シーンを消す)*/
	...
	case_I_CODE_1c_STAGE_LOAD:		MY_ICODE_CALL(i_code_load_stage);						goto I_CODE_break;	/*(読みこみのみ) 汎用絵  立ち絵L	立ち絵R */
		...
		/*	cg_state_flag |= STATE_FLAG_0x0800_IS_BOSS; */
		/* ボス戦闘前の会話終了を設定 */
...
extern void kaiwa_system_SDL_BG_draw(void)
{
	...
	if (re_draw_BG_count_flag)
	{
		re_draw_BG_count_flag--;
		{
			...
			//	}
			//	#endif
			//	PSPL_UpperBlit(bg_st ory_window_surface, NULL, cg.sdl_screen[SDL_00_VIEW_SCREEN], NULL);
...
/*static*/extern void do_kaiwa_system_terminate(void)
{
//	bg_alpha = 0;
...
extern void kaiwa_system_execute_move_only_main(void)
{
	...
	{/* 実行停止状態の場合は、実行停止解除用件をチェック */
		...
		/*
			...
			(タスク、マスクコマンドを新設する。)
		*/
		if (0==(cg_state_flag & 0x0c00))// 道中(ステージタイトル)では、キー入力待ちをしない。
		{/*(もし道中なら、)*/
			/*(勝手に解除)*/
...
/*static*/ void kaiwa_init_obj_position(void)
{
	...
	#if (1==USE_VS3)
	// ボス(2人目)
	kaiwa_sprite[2].center.x256 = t256(352-8);		kaiwa_sprite[2].center.y256 = t256(16-8);
	kaiwa_sprite[2].origin.x256 = t256(352-8);		kaiwa_sprite[2].origin.y256 = t256(16-8);
	kaiwa_sprite[2].offset.x256 = t256( 32-8);		kaiwa_sprite[2].offset.y256 = t256(16-8);
	// ボス(3人目)
	kaiwa_sprite[3].center.x256 = t256(352+8);		kaiwa_sprite[3].center.y256 = t256(16+8);
	kaiwa_sprite[3].origin.x256 = t256(352+8);		kaiwa_sprite[3].origin.y256 = t256(16+8);
	kaiwa_sprite[3].offset.x256 = t256( 32+8);		kaiwa_sprite[3].offset.y256 = t256(16+8);
	#endif
}
...
/*---------------------------------------------------------
	...
	(1: on	会話objを描画する。)
---------------------------------------------------------*/
/*static*/extern void kaiwa_all_obj_draw_on_off(u32 on_off)
{
	kaiwa_sprite[0].draw_flag = (on_off);/* プレイヤー */
	kaiwa_sprite[1].draw_flag = (on_off);/* ボス(1人目) */
	#if (1==USE_VS3)
	kaiwa_sprite[2].draw_flag = (on_off);/* ボス(2人目) */
	kaiwa_sprite[3].draw_flag = (on_off);/* ボス(3人目) */
	#endif
}
...
/*(USE_r36_SCENE_FLAG)*/
	/* ファイル名作成 */
	{
		...
		my_file_common_name[DIRECTRY_NAME_OFFSET+4] += ((cg_state_flag>>10) & (0x03));
		NEXT_SCENE;
		//
...
/*(バグ抜き)*/if (/*'1'*/'2'==my_file_common_name[DIRECTRY_NAME_OFFSET+4])/*(bad判定が必要ならbad判定処理)*/
		{
			...
			//	if ((5) == (cg.game_now_stage))/*(5面の場合)*/
				...
				{
					if ((0)==(cg_VR20_game_difficulty))/*(easyの場合)*/
					{
...
/*(バグ抜き)*/	cg_state_flag |= 0x0400;
	...
	#if 1/*Gu描画になると要らなくなる*/
	cg.main_call_func = story_mode_local_work;/* SDL描画の場合 */
	#else
//	cg.main_call_func = kaiwa_system_execute_move_only_main;/* Gu描画の場合 */
	#endif
}
