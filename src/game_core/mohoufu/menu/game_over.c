...
//	cg_state_flag		&= (~SCENE_NUMBER_MASK);	/*(シーンを消す)*/
	cg_state_flag		&= (0xffff00ffu);	/*(シーンを消す)*/
	cg_state_flag		|= (0x00008000u); //プレイヤーループを抜ける処理(とりあえず(?))
	#else
	/* 現在の version はボスを倒さないで抜けると Gu がまずいので対策 */
	cg_state_flag &= (~(STATE_FLAG_15_DRAW_BOSS_GAUGE));	/* Guは書かないとマズイ */
	#endif
	#if (1)
...
static void render_stage_clear_result(void)
{
	...
	{
		#if 1/*原作風*/
		if (3 > cg_bombs)	/* クリアー時にボムが３つ未満なら */
		{	cg_bombs = 3;	}	/* ３つに増やす */
		#endif
		#if 1/*模倣風*/
		if (8 > cg_bombs)	/* クリアー時にボムが９(8)つ未満なら */
		{	cg_bombs++; }	/* １つ増やす */
		#endif
		#if 0/*模倣風*/
		/*(MAXにする)*/
		cg_bombs = (8); 	/* ９(8)つに増やす (9個==8) */
		#endif
...
//		(cg_weapon_power	* score(  100)) + /* パワー   x  100 pts */ 	/* 原作風 */
//		(cg_graze_point)					  /* グレイズ x   10 pts */ 	/* 原作風 */
//
		(cg.game_now_stage	* score(10000)) + /* ステージ x 10000 pts */	/* 模倣風 */
		(cg_graze_point 	* score( 1000)) + /* グレイズ x  1000 pts */	/* 模倣風 */
		(cg_weapon_power	* score(  100)) /* パワー	  x   100 pts */	/* 模倣風 */
	)));
	#else
	...
	player_dummy_add_score(adjust_score_by_difficulty(( (((u32)cg_graze_point)		* score( 1000)) 	)));	/* グレイズ x  1000 pts */	/* 模倣風 */
	player_dummy_add_score(adjust_score_by_difficulty(( (((u32)cg_weapon_power) 	* score(  100)) 	)));	/* パワー	x	100 pts */	/* 模倣風 */
	#endif
//
	{
...
//	strcpy(my_font_text, "Graze 000 x   10 pts."); /* 原作風 */
	...
	//	strcpy(my_font_text, "Graze0000 x  1000 pts.");
		...
		dec_print_format( cg_graze_point,	 4, (char*)&my_font_text[ 5]);
		//
	...
	//	strcpy(my_font_text, "Power 000 x   100 pts.");
		...
		dec_print_format( cg_weapon_power,	 3, (char*)&my_font_text[ 6]);
		//
	...
	{
		strcpy(my_font_text, (char*)level_name[((cg_VR20_game_difficulty)/*&0x03*/)]);
		//
...
{	//																			"00000000001"
//																				"01234567890"
	{	32+(64),	( 32),	JAPAN("あなたの腕前",		"Technicality Check"),	((15)|STR_CODE_NO_ENTER),	"", 			((7)|STR_CODE_NO_ENTER) }, //"PLAYER DATA"/*"GAME OVER"*/
	{	32+(48),	( 60),	JAPAN("最終得点          ", "Final Score       "),	((6)|STR_CODE_NO_ENTER),	"0000000000.",	((7)|STR_CODE_NO_ENTER) },
	{	32+(48),	( 80),	JAPAN("ミス回数          ", "Count Missed      "),	((5)|STR_CODE_NO_ENTER),	"         0.",	((7)|STR_CODE_NO_ENTER) },	/* 集計システム(player_data)総ミス回数 */
	{	32+(48),	(100),	JAPAN("ボム使用回数      ", "Bomber Used       "),	((4)|STR_CODE_NO_ENTER),	"         0.",	((7)|STR_CODE_NO_ENTER) },	/* 集計システム(player_data)総ボム使用回数 */
	{	32+(48),	(120),	JAPAN("喰らいボム成功回数", "Reversal Success  "),	((3)|STR_CODE_NO_ENTER),	"         0.",	((7)|STR_CODE_NO_ENTER) },	/* 集計システム(player_data)総喰らいボム成功回数 */
	{	32+(48),	(140),	JAPAN("コンティニュー回数", "Continue Count    "),	((2)|STR_CODE_NO_ENTER),	"         0.",	((7)|STR_CODE_NO_ENTER) },	/* 集計システム(player_data)総コンティニュー回数 */
	{	32+(48),	(160),	JAPAN("難易度            ", "Played Difficulty "),	((1)|STR_CODE_NO_ENTER),	"   Lunatic.",	((7)|STR_CODE_NO_ENTER) },
};
...
static void render_game_over_result(void)
{
	...
	cg_player_data_use_continue--;	/* 集計システム(player_data)_ (現プログラムの都合上)コンティニュー回数0で 1回カウントされるので、その分減らして辻褄あわせをする。 */
//
	u32 yyy18;
	yyy18 = 0;
	u32 i;
	for (i=0; i<7; i++)//ML_LINE_01 ... ML_LINE_07
	{
		...
		goto *aaa[(i)];
	bbb1:	dec_print_format( cg_game_score,					9, (char*)&my_font_text[0]);		goto bbb7;
	bbb2:	dec_print_format( cg_player_data_count_miss,		3, (char*)&my_font_text[7]);		goto bbb7;
	bbb3:	dec_print_format( cg_player_data_used_bomber,		3, (char*)&my_font_text[7]);		goto bbb7;
	bbb4:	dec_print_format( cg_player_data_use_kurai_bomb,	3, (char*)&my_font_text[7]);		goto bbb7;
	bbb5:	dec_print_format( cg_player_data_use_continue,		3, (char*)&my_font_text[7]);		goto bbb7;
	bbb6:	strcpy(my_font_text, (char*)level_name[((cg_VR20_game_difficulty)/*&0x03*/)]);				goto bbb7;
	bbb7:
		kanji_draw();
		yyy18 += (18);
	}
...
static MAIN_CALL_FUNC(stage_clear_result_screen_local_work)
{
	...
	//	cg.msg_time = byou60(5);	/* 約 5 秒 */
		cg.msg_time = (65536);	/* 約 18 分 */
		{
		strcpy(my_font_text, JAPAN(
			"CHALLENGE NEXT STAGE!　　少女祈祷中...",
			"WAIT FOR NEXT SCENE! Girl praying now.")
		);
		//
	...
	if (0 == result_time_out)	/* おしまい */
	{
		...
		#if (1==USE_r36_SCENE_FLAG)
		/*(?)*/
		if (cg_state_flag & 0x00008000u)//プレイヤーループを抜ける処理(とりあえず(?))
...
//			}
		...
		{
			cg.main_call_func = common_load_init;
		}
	}
	else
	{
...
extern MAIN_CALL_FUNC(stage_clear_result_screen_start)
{
	...
	cg.main_call_func = stage_clear_result_screen_local_work;
}
...
//	psp_move_screen(MODE_01_POP);/*(仮想スクリーンを復活、取って置いた画面を描く)*/
	...
	if (0 > game_over_time_out)
	{
		...
		//	( (/*3*/DEFAULT_MAX_CONTINUE-1) == cg.game_now_max_continue ) &&
			( 0 == cg_player_data_use_continue ) && 							/* ノーコンティニューの場合のみスコアランキング */
			#endif
			(cg_game_score > high_score_table[(cg_game_select_player)][4].score)
		)
		{
			/* 最終スコアがランクインしてた場合 */
			cg.main_call_func = name_entry_start;	/* 名前入力画面へ */
		}
		else	/* タイトル画面へ */
		{
			/* 最終スコアがランクインしてない場合 */
...
extern MAIN_CALL_FUNC(gameover_start)/* init */
{
	/* 初期化 */
	...
	cg.main_call_func = gameover_local_work;
}
