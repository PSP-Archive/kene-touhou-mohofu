...
extern /*static*/ void set_default_option(int *option_setting_map)
{
	u32 i;
...
extern /*static*/ void check_limit_value_option(int *option_setting_map)
{
	u32 i;
...
//	/*static*/ SDL_Rect rect_src_offset;		/* 位置 */
	...
	for (i=0; i</*12*/(OPTION_MENU_CONFIG_MAX); i++)
	{
		...
		for (i2=0; i2</*12*/(option_status_setting[i].item); i2++)
		{
			...
			PSPL_UpperBlit(option_menu_name_surface[kinou_number+i2], NULL, cg.sdl_screen[SDL_00_VIEW_SCREEN], &rect_locate_offset);
		}
		kk += (20);/* 縦の文字間隔 */
	}
	/* メニュー名称描画 */
	...
	for (i=0; i<(OPTION_MENU_MAX); i++)
	{
		...
		PSPL_UpperBlit(option_menu_name_surface[i+(KEY_TAB_17_MAX)], NULL, cg.sdl_screen[SDL_00_VIEW_SCREEN], &rect_locate_offset);
		kk += (20);/* 縦の文字間隔 */
	}
}
...
static MAIN_CALL_FUNC(option_menu_terminate)
{
	...
	#endif
	/* 指定した場所へ移動 */
	cg.main_call_func	= cg.menu_out_call_func;
}
			...
			else	/* 上下以外の通常処理(斜め入力を排除する為、上下の入力があった場合はそれ以外の処理をしない) */
			{
				...
				if (OPTION_MENU_09_QUIT == menu_cursor1)	// 項目[ QUIT ] を選んでいる場合
				{
					if (psp_pad.pad_data & PSP_KEY_BOMB_CANCEL) 	// キャンセルボタン入力
					{
						...
						cg.menu_out_call_func	= title_menu_start; 	/* タイトルメニューへ移動指示 */
						cg.main_call_func		= option_menu_terminate;
					}
					else
					if (psp_pad.pad_data & PSP_KEY_SHOT_OK) 	// ショットボタン入力 */
					{
				...
				if (OPTION_MENU_08_KEYCONFIG == menu_cursor1)	/* 項目[ KEY CONFIG ] を選んでいる場合 */
				{
					if (psp_pad.pad_data & PSP_KEY_SHOT_OK) 	/* ショットボタン入力 */
					{
						...
						cg.menu_out_call_func	= key_config_start; 	/* キーコンフィグメニューへ移動指示 */
						cg.main_call_func		= option_menu_terminate;
					}
				}
...
//			psp_move_screen(MODE_00_PUSH);/*(現在の表示画面を裏画面に保存)*/
			cg.main_call_func = option_menu_state_01_select_menu;
		}
...
//	menu_out_call_func = title_menu_start;		/* タイトルメニューへ移動指示 */
	cg.main_call_func = option_menu_state_00_slide_in;
}
