...
#if (0)/*(-r40)*/
	#define PSP_DEF_SNAP_SHOT PSP_KEY_SNAP_SHOT
#else/*(r41-)*/
	#define PSP_DEF_SNAP_SHOT (0)/*[標準では、割り当て無し]*/
#endif
/*static*/extern void set_default_key(u32 *key_setting_map, int key_setting_type)
{
	...
	//	},
		...
		{	PSP_KEY_SELECT, 		PSP_KEY_PAUSE,		PSP_DEF_SNAP_SHOT,		PSP_KEY_SLOW,
...
//	pl ay_music_num(BGM_14_th07_01);
	...
	bg_alpha_aaa		= (0);
	cg.main_call_func = key_config_state_04_fade_out;
}
		...
		else	/* 上下以外の通常処理(斜め入力を排除する為、上下の入力があった場合はそれ以外の処理をしない) */
		{
			...
			if (MENU_LINE_Y_13_QUIT == menu_cursor1)	// 項目[ QUIT ] を選んでいる場合
			{
				if (psp_pad.pad_data & PSP_KEY_BOMB_CANCEL) // キャンセルボタン入力
				{
					...
					cg.main_call_func = key_config_state_03_fade_init;
				}
				else
				if (psp_pad.pad_data & PSP_KEY_SHOT_OK) // ショットボタン入力
				{
	...
	//	/*static*/ SDL_Rect rect_src_offset;		/* 位置 */
		...
		int jj;
		{
			...
			for (ii=0; ii<KINOU_08_WARIATE_MAX; ii++)
			{
				...
				for (kk=0; kk</*12*/(MENU_LINE_Y_12_RESET/*12*/)/*KEY_CONFIG_MENU_14_MAX*/; kk++)
				{
					...
					PSPL_UpperBlit(key_name_surface[(kinou_number)], NULL, cg.sdl_screen[SDL_00_VIEW_SCREEN], &rect_locate_offset);
				//	rect_src_offset.x = 0;
				//	rect_src_offset.y = (kinou_number<<4);
				//	rect_src_offset.w = (16*9);
				//	rect_src_offset.h = (16);
				//	PSPL_UpperBlit(sdl_screen[SDL_01_BACK_SCREEN], &rect_src_offset, cg.sdl_screen[SDL_00_VIEW_SCREEN], &rect_locate_offset);
		...
		*/
		...
		/* リセットタイプの描画 */
		{
			...
			PSPL_UpperBlit(key_name_surface[DRAW_LABEL_OFFSET_Y_TYPE + key_setting_default_type], NULL, cg.sdl_screen[SDL_00_VIEW_SCREEN], &rect_locate_offset);
		}
		...
		{	int position_y;
			...
			for (ii=0; ii</*(MAX_BUTTON_12+MAX_MENU_02)*/(MENU_LINE_Y_14_MAX)/*MENU_LINE_Y_14_MAX*/; ii++)
			{
				...
				PSPL_UpperBlit(key_name_surface[DRAW_LABEL_OFFSET_Y_KINOU + (ii)], NULL, cg.sdl_screen[SDL_00_VIEW_SCREEN], &rect_locate_offset);
...
static MAIN_CALL_FUNC(key_config_state_01_fade_in)
{
	...
	if ((/*250-6*/224) < bg_alpha_aaa)
	{
		...
		cg.main_call_func = key_config_state_02_select_left_menu;
	}
	SDL_SetAlpha(cg.sdl_screen[SDL_01_BACK_SCREEN], SDL_SRCALPHA, bg_alpha_aaa);
	psp_move_screen(MODE_01_POP);/*(仮想スクリーンを復活、取って置いた画面を描く)*/
}
...
{
	play_music_num(BGM_19_th07_02);/* th07_02.IT 無何有の郷 ～ Deep Mountain キーコンフィグ */
...
//} FONT_AAA;
	...
	bg_alpha_aaa				= (0);
	cg.main_call_func = key_config_state_01_fade_in;
}
