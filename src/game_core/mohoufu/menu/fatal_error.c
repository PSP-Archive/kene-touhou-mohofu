...
//	set_kanji_hide_line(ML_LINE_99_MAX);/*(全行表示する。)*/
	...
	#if (1)/*[エラースクショ撮影]*/
	save_screen_shot();/*[エラーの場合勝手にスクショ取る]*/
	#endif
	hit_any_key();/*[簡易入力待ち]*/
	// [強制終了します。と描画]
	kanji_window_all_clear();			/* メッセージウィンドウの内容を消す。 */
	strcpy(my_font_text, JAPAN( "致命的エラーなので終了します。",
...
//	cg.main_call_func = NULL;
	sceKernelExitGame();	//exit(1);/*exit()はat_exit()のベクタにコールパック処理を登録していないとハングアップするので注意*/
	/*(
		exit()は古いタイプのpsp用SDLでは登録してない(のでハングアップ)。
		新しいタイプのpsp用SDLではフックしてあった(当然exit()でSDL側に飛ぶ)。
		標準状態のC言語(PSPSDK)ではat_exit()のコールパック処理は何も登録していない(のでハングアップ)。
		何も登録していない==NULLに設定されてる為0番地ジャンプで特権違反。
	)*/
}
