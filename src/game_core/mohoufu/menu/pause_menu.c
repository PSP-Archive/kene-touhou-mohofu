...
typedef struct
{
	...
	/*s32より小さくなる*/u32 max_items; 		/* オプションの項目数 */
...
//	{	0, 0, NULL, 	}, //----------------
	{	GAME_X_OFFSET+(96), (115) +((0)*21),	JAPAN("一時停止解除", "Continue Game"), },
	{	GAME_X_OFFSET+(96), (115) +((1)*21),	JAPAN("始めからやり直す", "Retry Game"),},
	{	GAME_X_OFFSET+(96), (115) +((2)*21),	JAPAN("タイトルへ戻る", "Discard Game"),},
	{	0, 0, NULL, 	}, //----------------
	{	GAME_X_OFFSET+(144), (138) +((0)*21),	JAPAN("はい", "Yes"),					},
	{	GAME_X_OFFSET+(136), (138) +((1)*21),	JAPAN("いいえ", "No"),					},
	{	0, 0, NULL, 	}, //---------------- ---------012345----------------------------0123456789012345678901
	{	GAME_X_OFFSET+(32), (16),				JAPAN("あと 0回コンティニューできます", "You can try more than  0game."),	},
	{	GAME_X_OFFSET+(32), (48),				JAPAN("コンティニューした場合、", "but do not score ranking , "),		},
	{	GAME_X_OFFSET+(32), (48+18),			JAPAN("スコアランキングされません", "if your continued this game."),		},
	{	GAME_X_OFFSET+(32), (120),				JAPAN("コンティニューしますか？", "Sure?"), 		},
	{	0, 0, NULL, 	}, //----------------
};
/*---------------------------------------------------------
...
static void render_continue(void)
{
		...
		cg.msg_time = (65536);	/* 約 18 分 */
	{
		...
		for (i=0; i<4; i++)
		{
			...
			if (0==i)
			{
				dec_print_format( ((cg.game_now_max_continue)), 2, (char*)&my_font_text[JAPAN(4, 22)]);
			}
...
//	set_kanji_hide_line(ML_LINE_02);/*(2行目以下を非表示にする。)*/ 	/*(ゲームモードの場合)*/
	...
	//	if (psp_pad.pad_data & PSP_KEY_RIGHT)
		...
		//	cg_game_score	= (8);/*test*/
		//	cg_graze_point	= (0);/*test*/
		//	cg_zanki		= (8);/*test*/
		//	cg_bombs		= (8);/*test*/
		//	#if (0)
		//	/* ボスチェック用 */
		...
		//	#endif
			cg_game_score	= ((cg_state_flag>>8) & 0xff);/*(常に表示)*/
			cg_graze_point	= (cg_game_score);/*test*/
		}
		#endif
		#if 0//1/*(1==DEBUG_MODE)*/
	//	else
		...
		if (/*MENU_ITEM_00_CONTINUE_GAME*/0 == aaa.selected_number)/* Continue Game */
		{
			...
			#endif
			cg.main_call_func = cg.pause_out_call_func;
		}
		else
		...
		//	cg.msg_time = (0);/* 必要(?)*/	/* 約 0 秒 */
			cg.main_call_func = stage_first_init;	/* 始めから */
		}
		else
	...
	if (MENU_TYPE_02_ASK_CONTINUE==aaa.menu_type)
	{
		if (/*MENU_ITEM_00_CONTINUE_YES*/0 == aaa.selected_number)
		{
			...
			cg_state_flag &= 0xffff7fffu;/*(とりあえず、現在仕様が良くわかんない)*/
			#endif
...
/*---------------------------------------------------------
	...
	-------------------------------------------------------
---------------------------------------------------------*/
/*static*/extern void stage_select_menu_start(void)
{
...
/*static*/extern MAIN_CALL_FUNC(pause_menu_start)
{
	{
		/*(ポーズメニューの場合の特殊処理)	*/
		if (
...
//	else
	...
	//
		...
		common_menu_initialize();
	//	fade_out_flag		= (0)/*set_fade_out_flag*/;/*構造的問題*/
	//	cg.main_call_func = pause_menu_local_workMENU_STATE_00_FADE_IN_MENU;
		cg.main_call_func = common_menu_work_MENU_STATE_00_FADE_IN_MENU;
	}
}
...
/*static*/extern MAIN_CALL_FUNC(ask_continue_menu_start)
{
		...
		psp_move_screen(MODE_00_PUSH);/*(現在の表示画面を裏画面に保存)*/
	{
		...
		common_menu_initialize();
	//	fade_out_flag		= (0)/*set_fade_out_flag*/;/*構造的問題*/
	//	cg.main_call_func = ask_continue_menu_00_FADE_IN_MENU;
		cg.main_call_func = common_menu_work_MENU_STATE_00_FADE_IN_MENU;
	}
}
