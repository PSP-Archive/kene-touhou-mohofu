...
#define ERROR_LIMIT_PER_LINE_200	(200)/* 200 文字以上はエラー */

/*---------------------------------------------------------
	shift jisコード、全角1バイト目かどうか判定する子関数
---------------------------------------------------------*/

/*static*/extern int is_kanji_1st(/*u8*/int/*int*/ high_byte)
{
	#if 1
	high_byte &= 0xff;/**/
	return (
...
/*static*/extern char *load_my_file_get_str(char *str, char *buffer, int *end_arg)
{
	int string_error_limiter;
	string_error_limiter = ERROR_LIMIT_PER_LINE_200;/* 200 文字以上はエラー */
	/*u8*/int high_byte;
	{loop:;
		high_byte = ((*str) & 0xff);
		/* 空文字列の可能性があるから、始めに判定 */
...
/*static*/extern void *my_file_fopen(void)/*const char *my_file_common_name*/ /*, const char *dummy*/
{
	SceUID fd;
...
/*static*/extern void my_file_read_to_mem(char *write_memory, int read_length)
{
...
/*static*/extern int my_file_fgets(void)/*char *buffer_name, int num, char *wfp*/
{
ggg_loop:;
	int ii;
	ii = my_fgets();
...
/*static*/extern void my_file_fclose(void)/*void *wfp*/
{
	free(malloc_buf);
}
	// (?)
/*static*/extern void my_fseek_begin(void)/*void *wfp*/
{
//	file_seek = 0;
}
#else
...
//		chooseModDir();
	...
	/* 読み込んだデーターで初期設定 */
	cg_VR20_game_difficulty 	= option_config[OPTION_CONFIG_04_CURRENT_DIFFICULTY];
	cg_game_select_player	= option_config[OPTION_CONFIG_05_CURRENT_PLAYER];
	#if 1/*(範囲外の場合は強制的に修正する。)*/
	cg_VR20_game_difficulty 	&= 0x03;/* (easy)0 1 2 3(Lunatic) */
...
//
	...
	write_line_buffer_to_file(fd);/*fp*/
	#endif	/* (USE_MEMO) */
	option_config[OPTION_CONFIG_04_CURRENT_DIFFICULTY]	= (cg_VR20_game_difficulty);
