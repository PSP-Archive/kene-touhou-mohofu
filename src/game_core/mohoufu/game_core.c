...
/*---------------------------------------------------------
	...
	-------------------------------------------------------
	ToDo:
		雑魚スペカを設定したり、割り当てたりするコマンドが必要。
---------------------------------------------------------*/

extern GAME_COMMAND_FUNC(07_game_all_clear);/* 全面クリアーの場合、このゲームコマンドを追加 */
extern GAME_COMMAND_FUNC(06_kaiwa_start_boss);/* ボス、シナリオ会話起動 */
extern GAME_COMMAND_FUNC(05_bg_control);/* 背景コントロール(スクロール速度等を指定) */
extern GAME_COMMAND_FUNC(04_);	/* 4[拡張予定] (.align)現在(r41)ダミー。 */
//
extern GAME_COMMAND_FUNC(03_);	/* 3[拡張予定] (.align)現在(r41)ダミー。 */
extern GAME_COMMAND_FUNC(02_move_chuu_boss);/* [中型敵]中ボス移動コマンド。(r41-) */
extern GAME_COMMAND_FUNC(01_regist_chuu_boss);/* [中型敵]中ボス、生成処理 */
extern GAME_COMMAND_FUNC(00_regist_zako);/* ザコ、生成処理 */
...
/*static*/extern void set_core_game_time_MAX(void)
{
	game_v_time = 65535;/* 適当に大きな値[frame](65535[frame]==約18[分]==18.xxx x 60 x 60 ) */
}

#if (1==USE_HOLD_GAME_MODE)
extern void hold_game_mode_on(void)/* ゲーム時間の一時停止(咲夜、新規格イベント(構想中)等、使う) */
{
	v_time_hold_mode = 1;
}
...
//	cg_state_flag		|= (SCENE_NUMBER_0x0400_DOUCHUU); // ステージタイトル表示後の状態にする。
	cg_state_flag		|= (SCENE_FLAG_0x0100_KAIWA_LOAD); // タイトル読みこみテスト
	#endif
	/*(雑魚停止)*/
...
//
	...
	#endif
	cg.main_call_func = shooting_game_core_work;
}
...
//extern void shooting_game_core_1st_init(void)
extern void score_panel_init(void);
extern void jiki_class_initialize(void);
extern MAIN_CALL_FUNC(stage_first_init)
{
	score_panel_init();
	...
	*/
	...
	while (NULL != aaa)
	{
		...
		{	/* 未処理のコマンド */
			...
			//	static GAME_COMMAND_FUNC(regist_game_command)
				{	/* 中間コード形式のコマンドから各関数に分岐する */
					/* game_commandの生成を番号で管理(load_stage.c の ctype_name[]に対応している) */
					void (*bbb[/*(6)*/GC_CODE_MAX/*(8)*/])(GAME_COMMAND *aaa) =
					{
						#define game_command_03_dummy game_command_07_game_all_clear
						#define game_command_04_dummy game_command_07_game_all_clear
						game_command_00_regist_zako,			/* 0"妖怪" ザコを追加する。 */
						game_command_01_regist_chuu_boss,		/* 1"魔法" 中ボスを追加する。 (特殊敵[中型敵]) */
						game_command_02_move_chuu_boss, 		/* 2"移動" 中ボス移動コマンド。(r41-) */
						game_command_03_dummy,					/* 3[拡張予定] (.align)現在(r41)ダミー。 */
						//
						game_command_04_dummy,					/* 4[拡張予定] (.align)現在(r41)ダミー。 */
						game_command_05_bg_control, 			/* 3"BG" 背景の制御コマンド。 */
						game_command_06_kaiwa_start_boss,		/* 6"会話" ボスを追加する(r35)。かつ ボスのシナリオ会話を起動する。(シナリオ会話内でボス追加) */
						game_command_07_game_all_clear, 		/* 4"QUIT" ゲーム 全ステージ クリアーを指示。 */
					};
...
/* 注意：(動作速度低下するので)static関数にしない */extern void my_special(void)
{
	/* 道中コマンド追加読み込み処理 */
	...
	if (0 < /*bomb_wait*/cg_bomber_time)		/* ボムウェイト処理 */
	{
		return;/* ボム発動中は待機 */
	}
	/* シナリオ会話をロードして開始処理 */
	...
	if (IS_SCENE_END_KAIWA_MODE)
	{
			...
			イベントシーンを次に進める。 */
		{
			...
			if (0x0c00==((cg_state_flag & SCENE_NUMBER_MASK)))/*(good end の場合)*/
			{
				cg_state_flag += 0x0400;/*(ボス終了後へ進める)*/
			}
		//	if (0x1000==((cg_state_flag & SCENE_NUMBER_MASK)))
			if (0x1000u <= ((unsigned)(cg_state_flag & SCENE_NUMBER_MASK)))
			{	/*(「ボス戦闘後の会話が終了した状態」の場合)*/
				cg.main_call_func = stage_clear_result_screen_start; /* ステージクリアー時のリザルト画面 */
			}
		}
	}
	#if (1==USE_r36_SCENE_FLAG)
...
/*extern*/static void game_core_danmaku_layer_system(void)
{
	int jj;
	for (jj=0; jj<DANMAKU_LAYER_04_MAX; jj++)/*(登録を全部調べる。)*/
	{
		/*(レイヤーが登録されていない場合は何もしない。)*/
		if (NULL != card.layer_callback[jj])
		{
			int ii;
			for (ii=0; ii<OBJ_POOL_00_TAMA_1024_MAX; ii++)/*(弾を全弾調べる。)*/
			{
				...
				{
					if (jj == ((obj->TR00_hatudan_spec_data)&(0x03)) )	/* 弾幕レイヤー[n]なら */
					{
						(card.layer_callback[jj])(obj);
					}
				}
			}
		}
...
extern MAIN_CALL_FUNC(shooting_game_core_work)
{
	{
my_game_core_loop:
		/* [A] ゲーム時間を経過させる。 */
		 ...
		 */
		if (0x0800!=(cg_state_flag & (0xff00))) 	// ボス戦闘中以外の状態ならば、すべて特殊処理が必要。
		{
		...
		*/
		...
		if (0==(psp_pad.pad_data_alter & PSP_KEY_PAUSE))/* さっきポーズが押されてなくて */
		{
			...
			{
				cg.main_call_func			= pause_menu_start;
				cg.pause_out_call_func		= shooting_game_core_work;/* ポーズ復帰後の戻り先を決める */
			}
		}
		/* ゲームコアから外に出ると、(CPUの命令キャッシュが壊れるので(?))、極端に速度低下する。
			必要ない場合は外に出ない為の処置 */
		if (shooting_game_core_work == cg.main_call_func)
		{
