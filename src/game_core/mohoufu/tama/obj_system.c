...
--------------------------------------------------------- */
#if 0/* めも */
int obj_collision_laser(OBJ/**/ *obj1, LASER/**/ *laser1)
{
//	分割処理１．判定有効なオブジェクト
...
//	int hantei_y;
	...
	/* (傾きと逆方向に回転させる) */
	rotation_angle65536 = -( (laser1->angle65536));
	/* 固定値 */
...
//	cos_angle = (int)(int256_si n1024(/*rot_cos*/(( OFFS_COS1024+			  rotation_angle1024)&(1024-1)))/*<<8*/);
	int256_sin cos65536((rotation_angle65536), &sin_angle, &cos_angle);
	#if 1
...
#if (1)
static /*(BOOL)*/int obj_collision_tama(OBJ/**/ *obj1, OBJ/**/ *obj2)
{
	#if (0)/* この機能は要らない(?)かも。 */
	/*--- 判定有効なオブジェクトでなかったら終了 */
...
static OBJ *obj_collision_check(OBJ/**/ *to_check, OBJ/**/ *s, int set_obj_type, s32 length)
{
	s32 ii;
	for (ii=0; ii<length; ii++)/* 全部調べる。 */
	{
		/* 1. 消去済みは飛ばす。 */
		if (JYUMYOU_NASI < (s->jyumyou))					{;}else {goto label_next;}
		/* 2. 判定フラグチェック */
		/*(あたり判定があるもののみチェック)*/
		/* do collision only, use collision check type. */
		if ( (ATARI_HANTEI_OFF) != (s->atari_hantei))		{;}else {goto label_next;}
		/* 3. 判定種類チェック */
		/* obj_type_set が一致する場合のみ */
		/* do collision only mach type. */
		if (s->obj_type_set & set_obj_type) 				{;}else {goto label_next;}
		/* 4. 自分自身チェック */
		/* 判定対象(自分自身)は飛ばす(全く同じもの同士のあたり判定を取っても意味がないので取らない) */
		/* Is sprites my self?(not need self collisions.) */
		if (s != to_check)									{;}else {goto label_next;}
		/* 5. 矩形/円あたり判定チェック   大まかに矩形で判別した後、近そうなら円の衝突判定 */
		/* 矩形/円あたり判定 */
		/* hit collision rectangle to circle check. */
		if (obj_collision_tama(s, to_check))				{;}else {goto label_next;}
		/* 6. あたった場合のオブジェを返す。 */
		/* hit collisioning! */
		return (s);/*(あたった)*/
	label_next:
...
/*static*/extern OBJ *obj_collision_check_00_tama(OBJ/**/ *to_check, int type)
{
	OBJ *s;
	s = &obj99[OBJ_HEAD_00_0x0000_TAMA+0];
	return obj_collision_check(to_check, s, type, OBJ_POOL_00_TAMA_1024_MAX);
}
//	/*static*/extern OBJ *obj_collision_check_A01_A02_teki(OBJ/**/ *to_check, int type)
/*static*/extern OBJ *obj_collision_check_A01_A02_teki(OBJ/**/ *to_check)/*, (OBJ_Z02_TEKI)int ty pe*/
{
	OBJ *s;
//	s = &obj99[OBJ_HEAD_02_0x0900_TEKI_FIX+0];
//	return obj_collision_check(to_check, s, /*type*/(OBJ_Z02_TEKI), (/*OBJ_POOL_01_TOUROKU_0256_MAX+*/OBJ_POOL_02_TEKI_FIX_0256_MAX));
...
extern void obj_system_move_all(void)
{
	...
	{
		my_loop:
/*r41[正しい位置]*/ 	s--;//	s++;/* レーザーの実装を簡単にする為に、逆順にする。 */
...
//r40[バグ]この位置ではダメ。		s--;//	s++;/* レーザーの実装を簡単にする為に、逆順にする。 */
		iii--;
		if (0 != iii)
		{
			goto my_loop;
...
---------------------------------------------------------*/
extern OBJ_CALL_FUNC(gamen_gai_nara_zako_osimai)
{
	if (
...
/*---------------------------------------------------------
	画面外ならおしまい(個別、主に汎用objで使用)
---------------------------------------------------------*/
//extern OBJ_CALL_FUNC(gamen_gai_limit_30_nara_zako_osimai)
//{
...
/*---------------------------------------------------------
	スプライトオブジェクトの初期化をする。
	-------------------------------------------------------
	Intialize a sprite object one.
	-------------------------------------------------------
	リスト方式でないスプライトオブジェクトも初期化する為、
	外部からも呼ばれるので注意。
--------------------------------------------------------- */
extern OBJ_CALL_FUNC(obj_initialize_one)
{
...
//
	...
	#if 1/* 自動的に消える機能 */
	src->jyumyou				= JYUMYOU_1MIN10S;		/* 1分したら勝手に自動消去。 */
...
/*---------------------------------------------------------
	弾領域に敵弾を登録する。
	-------------------------------------------------------
	Regist sprite object, Tama(bullet) area.
	-------------------------------------------------------
	模倣風では試した中ではこの方式が最も速かったので、
	2のn乗のサイズのリングバッファ方式を採用しています。
	-------------------------------------------------------
	リングバッファの場合、新規の登録位置は、
	登録出来た次の位置が登録できる可能性が最も高いので、
	強制的にそこ(から順番に調べて)に登録します。
	万一登録出来ない場合でも、リングバッファの場合、
	最大登録可能数を超えない場合は
	リングが一周してなお登録できない場合はありません。
	もちろん最大登録可能数を超えた場合は、当然リングが一周します。
--------------------------------------------------------- */

extern OBJ *obj_regist_tama(void)
{
	#if 1
...
/*(固定敵を確保)*/
extern void obj_touroku_push(void)
{
	register_kotei_first++;
	register_kotei_max--;
}
/*(固定敵を減らす)*/
extern void obj_touroku_pop(void)
{
	register_kotei_first--;
	register_kotei_max++;
}
/*---------------------------------------------------------
	...
	-------------------------------------------------------
	Regist sprite object, Teki(Zako or Boss) area.
--------------------------------------------------------- */

/*(bossを固定確保する分)*/
//#define BOSS_OBJ_KOTEI_KAKUHO_BUN (1)
extern OBJ *obj_regist_teki(void)
{
	#if 1
	/* private (この関数で)プライベートな変数 */
	static u32 register_num;/*(登録できる可能性が高そうな位置を保持)*/
...
my_retry:
		...
		register_num++; 	/*(登録できる可能性が高そうな予測位置)*/
...
/*---------------------------------------------------------
	番号を指定して直接登録する。
	-------------------------------------------------------
	Regist sprite object, used direct number.
--------------------------------------------------------- */

extern OBJ *obj_regist_direct_number(u32 direct_register_number)
{
...
/*---------------------------------------------------------
	...
	-------------------------------------------------------
	Delete all sprite object, in the sprite object list.
--------------------------------------------------------- */
...
/*---------------------------------------------------------
	スプライトオブジェクト システム初期化。
	-------------------------------------------------------
	Initilized sprite object system.
--------------------------------------------------------- */
...
/*---------------------------------------------------------
	プレイヤーにとっての敵を探す
	-------------------------------------------------------
	Serch Teki(Zako or Boss) for player.
---------------------------------------------------------*/

extern OBJ *obj_teki_search(void)
{
	int ii;
//	for (ii=0; ii<OBJ_POOL_02_TEKI_FIX_0256_MAX; ii++)/*(Search all characters.(固定敵のみ256個)全部調べる。)*/
	for (ii=0; ii<(OBJ_POOL_01_TOUROKU_0256_MAX+OBJ_POOL_02_TEKI_FIX_0256_MAX); ii++)/*(Search all characters.(固定敵と登録敵の256+256==512個)全部調べる。)*/
	{
		OBJ *h;
		h = &obj99[OBJ_HEAD_01_0x0800_TOUROKU+ii];		/* 登録敵専用スプライトの先頭 */
	//	h = &obj99[OBJ_HEAD_02_0x0900_TEKI_FIX+ii]; 	/* 固定敵専用スプライトの先頭 */
	//
		if (
			#if 1
			(JYUMYOU_NASI <= h->jyumyou) && /*(Skip deleted.削除済みは飛ばす)*/
			#endif
			(0 != (h->obj_type_set & (OBJ_Z02_TEKI)) )	/*(Zako or Boss.プレイヤーにとっての敵(ザコやボス))*/
		)
		{
			{
				#if 1/* Gu(中心座標) */
				if ((h->center.x256 > 0) && (h->center.x256 < t256(GAME_WIDTH ) ) &&
					(h->center.y256 > 0) && (h->center.y256 < t256(GAME_HEIGHT) ))	/*(Viewing only. 表示画面内の敵)*/
				#endif
				{
					return (h);/*(Im found! 見つけたよ)*/
				}
			}
		}
	}
	return (&obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_00_PLAYER]);/*(Do not found. 見つからなかった)*/
}
