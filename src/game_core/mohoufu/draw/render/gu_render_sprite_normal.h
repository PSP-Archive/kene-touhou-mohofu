...
//	for (j=0; (j+SLICE_64_SIZE)<=(/*480*/spr->w/*512*/); j+=SLICE_64_SIZE)
	...
	u32/*short*/ absolute_x_pos = ((spr->center.x256>>8)-(spr->w >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
	u32/*short*/ absolute_y_pos = ((spr->center.y256>>8)-(spr->h >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
	for (; i<count2;)
	{
	//	if ((i1+1) < count)
