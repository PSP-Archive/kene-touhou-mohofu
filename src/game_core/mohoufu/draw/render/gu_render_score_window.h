...
static void gu_draw_side_panel(void)
{
	...
	//	PGC_SEND(PGC_TEX_FILTER, PU8(GU_LINEAR, GU_LINEAR));		// sceGuTexFilter(GU_LINEAR, GU_LINEAR);/*ぼやぼや拡大画面(拡大時直線補完)*/
		...
		//	sp rintf(my_font_text, "%09d0", top_score);
			...
			dec_print_format(cg_game_score, 9/*8*/, (char*)&my_font_text[KAZU_MY_SCORE01/*10*/]);	// "now score"
			dec_print_format(cg_graze_point, 4, 	(char*)&my_font_text[KAZU_GRAZE01]);			// "graze score"
			/* FRONT 面を間借りして、スコア表示 */
		/* 数字を表示 */
			int i;
			for (i=0; i<(10+10+4); i++)
