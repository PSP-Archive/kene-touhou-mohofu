...
static void gu_init_vfpu(void)
{
	...
	//	vfpu_srand((u32)tick / 10000.0f);	これでは使い方がおかしいので間違い。(vfpu_srand は int(32bitの整数値)で値を設定する。)
...
//
	...
	// http://wiki.livedoor.jp/mikk_ni3_92/d/Cg%CA%D405%3A%3AModelMatrix%A4%C8ViewMatrix
	// sceGumMatrixMode(GU_VIEW);「View Matrix」は「ワールド座標系」を「視点座標系」に変換する行列。
	// sceGumMatrixMode(GU_MODEL);「ModelMatrix」は、オブジェクト座標系（物体の中心に原点がある座標系）をワールド座標系に変換する行列。
