...
static void gu_draw_big_maho_jin(void)
{
	/* --- 半透明合成値 */
	//#if (1==USE_VCOLOR)
	//u32 blendlevel = (0xaa2f7fff);/* あかね */
	//#endif
	//#define big_maho_color8888 (0x997f2fff)		/* --- 半透明合成値 */
	/*								透青緑赤 AABBGGRR */
		const u32 maho_color_list[8] =
		{	/*AABBGGRR*/
	...
	const u32 big_maho_color8888 = maho_color_list[cg.game_now_stage&0x07];
//
	int j;
	u16 uv_x4;
	OBJ *obj_boss;
...
//	int hankei_111 = (draw_boss_hp_value>>(2+3));/* 128==1024/8 */
...
//		cos_angle_t256 = (int)(int256_si n65536(/*rot_cos*/(( OFFS_COS65536+			  rotation_angle65536)&(65536-1)))/*<<8*/);
		...
		#else
		int256_sincos65536((rotation_angle65536), &sin_angle_t256, &cos_angle_t256);
		#endif
	int ifx;
	int ify;
...
//		rotation_angle1024	+= (16);		/* 角度 16==(1024/64分割) 1周(1024)を64分割した場合の角度 */
		rotation_angle65536	+= (16<<6);		/* 角度 16==(1024/64分割) 1周(1024)を64分割した場合の角度 */
