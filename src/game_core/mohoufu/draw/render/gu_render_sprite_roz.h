...
//	if (dxpGraphicsSetup2DTex(gptr->tex, trans) < 0)return -1;
	u32 sw_bit = 32;//dxpPsm2SliceSize_bit[gptr->tex->psm][0];
	u32 sh_bit = 32;//dxpPsm2SliceSize_bit[gptr->tex->psm][1];
	u32 sw;
	u32 sh;
	sw = (1<<sw_bit);
	sh = (1<<sh_bit);
...
//	u32 swn = (u1 + sw - 1) / sw;
//	u32 shn = (v1 + sh - 1) / sh;
	u32 swn = (u1 + sw - 1) >> sw_bit;
	u32 shn = (v1 + sh - 1) >> sh_bit;
...
//	int c18_angle;	/* 位相 +180度進み */
	{
		/* 角度は0-65535度 */
		#if (0)//
		sin_angle_t256 = (si n65536(spr->rotationCCW65536));/*fps_factor*/	/* CCWの場合 */
		cos_angle_t256 = (co s65536(spr->rotationCCW65536));/*fps_factor*/
		#else
		int256_sincos65536(spr->rotationCCW65536, &sin_angle_t256, &cos_angle_t256);
		#endif
	}
//
	/* --- 半透明合成値 */
...
//	u32/*short*/ x_pos = ((spr->center.x256>>8));
//	u32/*short*/ y_pos = ((spr->center.y256>>8));
// (r32) u32/*short*/ x_pos = ((spr->center.x256>>8)-(spr->w >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
// (r32) u32/*short*/ y_pos = ((spr->center.y256>>8)-(spr->h >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
	int/*short*/ x_pos = ( -(spr->w >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
	int/*short*/ y_pos = ( -(spr->h >> 1));/* 中心座標から画像サイズの半分を引き、左上座標を計算 */
	i = 0;
	for (; i<count4;)
	{
