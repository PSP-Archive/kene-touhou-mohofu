...
static int round_up_to_power_of_2(int x)
{
	...
	/* 万一コンパイルできないSDKの場合用 */
	{
		...
		// 大体の大きさ b を計算する。
		b = (1 << n);
		// b がちょうど 2^n の場合は特殊ケースなので、
		// 特殊ケースの場合は、値を半分にして辻褄を合わせる。
		if (b == (2 * x))	{b >>= 1;}
		//
		return (b);
	}
...
extern/*static*/ void my_file_read_to_mem(u8* write_memory, int read_length);
static void /* PRIVATE */ png_file_chache_read_data(png_structp png_ptr_dummy, png_bytep dest_data, png_size_t length)
{
	my_file_read_to_mem(dest_data, length); 	// ファイルキャッシュから指定した長さ[bytes]転送する。
}
...
extern my_image* png_load_my_image(const char *filename)
{
	...
	//
	char *malloc_buf;
	malloc_buf = my_file_fopen(/*my_file_common_name, "r"*/);	// png読み込み後ファイルキャッシュへ配置する。
...
//		fclose(fp);
	...
	//
	png_buf_data	= (u8*)/*data*/malloc_buf;
...
//nofp		png_set_read_fn(png_ptr, (void*)fp/*&png_buf*/, (png_rw_ptr)png_file_chache_read_data);
	...
	#endif
	png_set_sig_bytes(png_ptr, 0/*sig_read*/);	//u32	sig_read = 0;
