...
static int size_of_kanji(/*u8*/int high_byte)
{
	high_byte &= 0xff;
...
//	if (1==hidari_gawa) 	/* pspは0レジスタがあるので0と比較したほうが速い */
	...
	for (dy=0; dy<KANJI_FONT_16_HEIGHT_P0; dy++)
	{
		...
		for (dx=0; dx<KANJI_FONT_08_HARF_WIDTH; dx++)
		{
			...
			//	s_getpixel16
			volatile u8 *src_p = (u8 *)(/*font_bg_bitmap_surface_image*/cg.kanji_window_screen_image)	/*surface*/
					+ (dy * (512/*font_bg_bitmap_surface_pitch*/))		/*surface*/
					+ (dx+dx)
					+ (haikei_offset+haikei_offset);
			//	putpixel16
			volatile u8 *dst_p = (u8 *)(cg.kanji_window_screen_image)/*surface->pixels*/
...
/*---------------------------------------------------------
	...
	(字ごとには位置指定できません)
---------------------------------------------------------*/
static u32 default_window_locate_x;
static u32 default_window_locate_y;
static u32 default_window_font_tate_kankaku;
static u32 default_window_max_line;
extern void set_kanji_origin_xy(u32 set_x_offset_pixel, u32 set_y_offset_pixel)
{
	default_window_locate_x = (set_x_offset_pixel);/* X位置[pixel] */
	default_window_locate_y = (set_y_offset_pixel);/* Y位置[pixel] */
}
extern void set_kanji_origin_kankaku(u32 set_y_offset_pixel)
{
	default_window_font_tate_kankaku = (set_y_offset_pixel);/* Y位置[pixel] */
}
extern void set_kanji_hide_line(int set_max_line_num)
{
...
/*---------------------------------------------------------
	スクリプト等用、標準初期化
---------------------------------------------------------*/
extern void kanji_init_standard(void)
{
	#if (1)/*(漢字関連の初期化)*/
...
/*---------------------------------------------------------
	カーソルを指定位置へ移動
---------------------------------------------------------*/
/*static*/extern void set_kanji_xy(u32 set_x_pixel, u32 set_y_number)
{
...
/*---------------------------------------------------------
	カーソルをホームポジションへ移動
---------------------------------------------------------*/
/*static*/extern void kanji_cursor_move_home_position(void)
{
	#if 1
...
---------------------------------------------------------*/
#if (1)
/*static*/extern void kanji_window_clear_line(u32 line_num)
{
//	#define TEST_OFFSET (/*0*/512*10/2) /* 10 [laster? ] */
	...
	{
	//	u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/;//+y*(512*2)/*surface->pitch*/+x+x;	/* 512[pixel]x 2[bytes](short) */
		u32 dx;
		for (dx=(/*0*/TEST_OFFSET); dx<(TEST_OFFSET)+(512*(16+2)*(1/*3 2[行]*/)/**16*/); dx++)
		{
			u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/+/*y*(512*2)*/ /*surface->pitch*/+dx+dx+(BG_FONT_HAIKEI_OFFSET*2)+line_num; /* 512[pixel]x 2[bytes](short) */
			*(u16 *)p = 0/*pixel*/;
		}
	}
				#endif
	...
	//	u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/;//+y*(512*2)/*surface->pitch*/+x+x;	/* 512[pixel]x 2[bytes](short) */
		...
		p16 = cg.kanji_window_screen_image;
		p16 += (BG_FONT_HAIKEI_OFFSET); 							/* 註:16bitポインタとしての値、つまり(u16だから)2倍[byte]される。 */
		p16 += (line_num * (512*(16+2)*(1/*3 2[行]*/)/**16*/)); 	/* 註:16bitポインタとしての値、つまり(u16だから)2倍[byte]される。 */
		u32 dx;
	...
	//	u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/;//+y*(512*2)/*surface->pitch*/+x+x;	/* 512[pixel]x 2[bytes](short) */
		...
		for (dx=(/*0*/TEST_OFFSET); dx<(TEST_OFFSET)+(512*(16+2)*(1/*3 2[行]*/)/**16*/); dx++)
		{
			u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/+/*y*(512*2)*/ /*surface->pitch*/+dx+dx+dx+dx+(BG_FONT_HAIKEI_OFFSET*4)+line_num+line_num; /* 512[pixel]x 2[bytes](short) */
			*(u32 *)p = 0/*pixel*/;
		}
	}
		#endif
...
/*static*/extern void kanji_window_clear(void)
{
	u32 ii;
//	for (ii=0; ii<(3); ii++)
...
//	#define TEST_OFFSET (/*0*/512*10/2) /* 10 [laster? ] */
	...
	//	u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/;//+y*(512*2)/*surface->pitch*/+x+x;	/* 512[pixel]x 2[bytes](short) */
		...
		{
			u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/+/*y*(512*2)*/ /*surface->pitch*/+dx+dx+(BG_FONT_HAIKEI_OFFSET*2); /* 512[pixel]x 2[bytes](short) */
			*(u16 *)p = 0/*pixel*/;
		}
	}
		#else
	...
	//	u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/;//+y*(512*2)/*surface->pitch*/+x+x;	/* 512[pixel]x 2[bytes](short) */
		...
		for (dx=(/*0*/TEST_OFFSET); dx<(TEST_OFFSET)+(512*(16+2)*(3/*3 2[行]*/)/**16*/); dx++)
		{
			u8 *p = (u8 *)cg.kanji_window_screen_image/*surface->pixels*/+/*y*(512*2)*/ /*surface->pitch*/+dx+dx+dx+dx+(BG_FONT_HAIKEI_OFFSET*4);	/* 512[pixel]x 4[bytes](int) */
			*(u32 *)p = 0/*pixel*/;
		}
	}
		#endif
	...
	#if 0/*test*/
	{
		...
		{
			(*(cg.kanji_window_screen_image+kk)) = (*(font_bg_bitmap_surface_image+kk));/*BG_FONT_HAIKEI_OFFSET_U16*/
		}
	}
	#endif
...
static u32 kanji_wait_value;
extern void kanji_color(u32 set_color_type)
{
	kanji_color_type = set_color_type;
}
extern void kanji_wait(u32 set_wait)
{
	kanji_wait_value = set_wait;
}
extern /*static*/ int kanji_draw(void)
{
//	const char *str;
//	int color_type;
//	/*(要確認)*/int wait;
...
//	if (1==need_draw_this_frame)	/* pspは0レジスタがあるので0と比較したほうが速い */
	if (0!=need_draw_this_frame)
	{
			...
			if (PS PL_MUSTLOCK(font_bg_bitmap_surface)) 	{PS PL_LockSurface(font_bg_bitmap_surface);}	/* ロック */
			#endif
		loop:
		;
		/*u8*/int high_byte;
		...
		/* 旧ロック */
			...
			{
			/*u8*/int low_byte; low_byte = (*(my_font_text+my_string_offset+1) & 0xff);
			...
			else				/* ascii文字 */
			{
				...
				{
					/*u8*/int escape_sequence = (*(my_font_text+my_string_offset+1) & 0xff);
		...
		else			//終了処理
		{
			...
			#if (1==IS_LOCK_LOCAL_SCREEN)
			if (PS PL_MUSTLOCK(font_bg_bitmap_surface)) 	{PS PL_UnlockSurface(font_bg_bitmap_surface);}	/* ロック解除 */
			#endif
...
//	kanji_system_terminate();
	if (NULL != cg.kanji_window_screen_image)		{free(cg.kanji_window_screen_image);cg.kanji_window_screen_image = NULL;}
	#endif
}
...
//	default_window_font_tate_kankaku	= (18);
	default_window_max_line 			= (3);/*(値3==4行)*/
		...
		/* 背景用に10[laster]余分に確保 */
	//	cg.kanji_window_screen_image = (u16 *)memalign(16, (512*(10+64)*2));		/* (10+ 64)[laster] */
	//	cg.kanji_window_screen_image = (u16 *)memalign(16, (512*(10+128)*2));		/* (10+128)[laster] */
		cg.kanji_window_screen_image = (u16 *)memalign(16, (512*(10+(192-10))*2)); /* (10+ 64)[laster] */
	#else
		/* 色32bitモード */
		/* メッセージ表示用RAM確保 */	/* 現在表示が16bit色なので(32bit色表示なら変わる) */
		/* 背景用に10[laster]余分に確保 */
	//	cg.kanji_window_screen_image = (u16 *)memalign(16, (512*(10+64)*2));		/* (10+ 64)[laster] */
	//	cg.kanji_window_screen_image = (u32 *)memalign(16, (2*512*(10+128)*2));	/* (10+128)[laster] */
		cg.kanji_window_screen_image = (u16 *)memalign(16, (512*(10+(192-10))*2)); /* (10+ 64)[laster] */
	#endif
//
	{
	...
	//	font_bg_bitmap_surface_pitch	= font_bg_bitmap_surface->pitch;
		...
		//	pixsrc = (void *)((font_bg_bitmap_surface->pixels));
			...
			pixdst = (cg.kanji_window_screen_image);
		...
		//	font_bg_bitmap_surface_image	= font_bg_bitmap_surface->pixels;
			...
			pixdst = (cg.kanji_window_screen_image);
			{
				...
				{
				//	(*(cg.kanji_window_screen_image+kk)) = (*(/*font_bg_bitmap_surface_image*/(u32 *)(font_bg_bitmap_surface->pixels)+kk));/*BG_FONT_HAIKEI_OFFSET_U16*/
...
{
// (?)	if (PS PL_MUSTLOCK(cg.FONT_fontimg))	{PS PL_LockSurface(cg.FONT_fontimg);}	/* ロック */
	u32/*char*/ i;
	u32/*char*/ j;
	i = 0;
	...
	//	haikei_offset	= (512*8);
		...
		for (dy=0; dy<16/*KANJI_FONT_16_HEIGHT_P0*/; dy++)
		{
			...
			for (dx=0; dx<16/*KANJI_FONT_08_HARF_WIDTH*/; dx++)
			{
				...
				//	s_getpixel16
				volatile u8 *src_p = (u8 *)(/*font_bg_bitmap_surface_image*/(cg.FONT_fontimg->pixels)/*cg.kanji_window_screen_image*/)	/*surface*/
						+ (dy * (512/*font_bg_bitmap_surface_pitch*/))		/*surface*/
						+ (dx+dx)
						+ (haikei_offset+haikei_offset);
				//	putpixel16
...
// (?)	if (PS PL_MUSTLOCK(cg.FONT_fontimg))	{PS PL_UnlockSurface(cg.FONT_fontimg);} 	/* ロック解除 */
}
#endif // (0)/*(?)*/
