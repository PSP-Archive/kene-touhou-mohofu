...
/*only boot once*/extern void font_system_boot_init(void)
{
	...
	cg.FONT_fontimg = load_chache_bmp();//, 0, 1);
	#else/*(r35)*/
	{
		SDL_Surface *s1;
		s1 = IMG_Load(my_file_common_name);
		cg.FONT_fontimg = SDL_DisplayFormat(s1);/* サーフェスを表示フォーマットに変換する。 */
	}
	#endif
	SDL_SetColorKey(cg.FONT_fontimg, (SDL_SRCCOLORKEY/*|SDL_RLEACCEL*/), 0x00000000);
}
...
//	for (i=0; i<strlen(my_font_text); i++)
	{
		...
		PSPL_UpperBlit(cg.FONT_fontimg, &s, txt_image_surface, &d);
		i++;
		goto loop_str;
	}
}
...
//	SDL_SetColorKey(fonts[font_number].fontimg/*screen*/, (SDL_SRCCOLORKEY|SDL_RLEACCEL), 0x00000000);
	font16_render_surface_xy(cg.sdl_screen[SDL_00_VIEW_SCREEN] );
}
#endif
...
//	SDL_FillRect(txt_image_surface, NULL, 0/*SD L_MapRGB(txt_image_surface->format, 0, 0, 0)*/);
	PSPL_FillRect000(txt_image_surface);/*(とりあえず)*/
