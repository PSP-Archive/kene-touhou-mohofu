...
//	#define SCENE_NUMBER_MASK_IS_BOSS			(0x0d00)
	...
	#define IS_SCENE(aaa)		((cg_state_flag & SCENE_NUMBER_MASK))
	#define NEXT_SCENE			cg_state_flag += 0x0100;

//
	#define IS_SCENE_DOUCHU_TUIKA		(SCENE_NUMBER_0x0400_DOUCHUU>=(cg_state_flag & (SCENE_NUMBER_MASK)))
	#define IS_SCENE_KAIWA_TATAKAU		(SCENE_FLAG_0x0000_KAIWA_TATAKAU	==(cg_state_flag & (SCENE_FLAG_0x0300_KAIWA_MASK)))
	#define IS_SCENE_KAIWA_LOAD 		(SCENE_FLAG_0x0100_KAIWA_LOAD		==(cg_state_flag & (SCENE_FLAG_0x0300_KAIWA_MASK)))
	#define IS_SCENE_KAIWA_MODE 		(SCENE_FLAG_0x0200_KAIWA_MODE		==(cg_state_flag & (SCENE_FLAG_0x0300_KAIWA_MASK)))
	#define IS_SCENE_END_KAIWA_MODE 	(SCENE_FLAG_0x0300_KAIWA_END		==(cg_state_flag & (SCENE_FLAG_0x0300_KAIWA_MASK)))
#else
...
//-------
	#define IS_SCENE_DOUCHU_TUIKA		(cg_state_flag & STATE_FLAG_14_DOUCHU_TUIKA)
	#define IS_SCENE_KAIWA_TATAKAU		(0==(cg_state_flag & STATE_FLAG_0x0200_IS_KAIWA_MODE))
	#define IS_SCENE_KAIWA_LOAD 		(cg_state_flag & STATE_FLAG_0x0100_IS_LOAD_KAIWA_TXT)
	#define IS_SCENE_KAIWA_MODE 		(cg_state_flag & STATE_FLAG_0x0200_IS_KAIWA_MODE)
	#define IS_SCENE_END_KAIWA_MODE 	(cg_state_flag & STATE_FLAG_0x0300_END_KAIWA_MODE)
#endif
...
//	/*extern*/extern void *callback_gu_draw_haikei_modosu;	/* (カード時から通常時へ)戻す用 */
//	/*extern*/extern void *callback_gu_draw_haikei_supeka;	/* カード時用 */
//	extern void (*callback_gu_draw_haikei)(void);//u32 draw_bg_screen;				/* 背景ウィンドウ表示フラグ */
enum
{
...
#define USE_PAD_STRUCT (0)
/*(0: 共通 / GAME_CORE_GLOBAL_CLASS に入れる)*/
/*(1: 独立 / GAME_CORE_GLOBAL_CLASS に入れない)*/
...
/*(模倣風はクリアまで30分以上かかる)*/
typedef struct tag_game_core_global_class
{
//[0x00]
...
/*extern*/ SDL_Surface *sdl_screen[SDL_99_MAX_SCREEN];/*(2)*/
//[0x20]
	#if (0==USE_PAD_STRUCT)
	/*(共通)*/
	...
	/*(描画用)*/int draw_boss_hp_value; /* ボスhp描画値 */ /*(?)*/
...
//
	...
	s8 dummy_222;				/* [未使用]ダミーchuu_boss_mode */
	s8 dummy_333;				/* [未使用]ダミーstatus */
//	int dummy2222;/*無いと何故か巧く動作しない*/
//	int dummy3333;/*無いと何故か巧く動作しない*/
	u32 conv_bg_alpha;
	#if (1)/*(仮対応)*/
	SDL_Surface *FONT_fontimg;
	#endif
//[0x40]
	#if (1)/*(仮対応)*/
	/* 暫定 SDL フォント */
...
//	u16 *tex_frame; 	/*(キャッシュ画像用画面)*/
	//
} GAME_CORE_GLOBAL_CLASS;//GAME_CORE_GLOBAL_CALLBACK
...
//	s8 dr aw_flag_ka iwa_screen;		/* 会話画面の会話用せりふウィンドウ表示フラグ */
	...
	#if (1)/*(仮対応)*/
/* 意図的に入れないもの */
extern u32 cg_game_select_player;/* : (将来はともかく)現状(r33)は GAME_CORE_GLOBAL_CLASSに入れない方が良いっぽい。 */
	#endif

	#if (1==USE_PAD_STRUCT)/*(r34)*/
	/*(独立)*/
	/* アライメント関係(?) (s16)で GAME_CORE_GLOBAL_CLASS に入れると巧くいかない */
	/* 意図的に入れないもの */
...
extern PSP_PAD_GLOBAL_CLASS psp_pad;
		...
		/*(共通)*/
		#define psp_pad cg/*_global_pad_*/
	#endif/*(USE_PAD_STRUCT)*/
...
#define GAME_COMMAND_FUNC(nnn)		void game_command_##nnn(GAME_COMMAND *gcmd)
#define GAME_COMMAND_OBJ_FUNC(nnn)	void game_command_##nnn(GAME_COMMAND *gcmd, OBJ *gcmd_obj)
#define GAME_COMMAND_OBJ_CALL(nnn)	game_command_##nnn(gcmd, gcmd_obj)
...
{
/*中間コード*/
/*0x00*/		GC_CODE_00_ZAKO = 0,			/* 敵 ザコ */
/*0x01*/		GC_CODE_01_CHUU_BOSS_COMMON,	/* 敵 ザコ中-ボス(スク リプト起動しない) */
/*0x02*/		GC_CODE_02_BOSS_COMMON, 		/* 敵 (ボス / 中-ボス)用 スク リプト起動 */
/*0x03*/		GC_CODE_03_BG_CONTROL,			/* 背景コントロール。(スクロール速度等) */
//
/*0x04*/		GC_CODE_04_ALL_CLEAR,			/* ゲーム 全ステージ クリアー */
/*0x05*/		GC_CODE_05_RESURVED,			/* 予約(雑魚スペカの種類割り当て選択命令) */
/*0x06*/		GC_CODE_06_RESURVED,			/* 予約(雑魚発弾タイミング選択命令) */
/*0x07*/		GC_CODE_07_RESURVED,			/* 予約(雑魚発弾タイミング設定命令) */
//
/*0x08*/	//	GC_CODE_00_SJIS_TEXT = 0,		/* 漢字表示[番兵区切りとしても必要] */
/*0x09*/	//	GC_CODE_03_CHECK_SECRET_BONUS,	/* 隠しボーナスチェック "出力" */
/*0x0a*/	//	GC_CODE_04_BEGIN_SECRET_BONUS,	/* 隠しボーナスチェック "確認" */
			//
/*0x0b*/	/* 拡張可能 */
/*...*/ 	/* ... */
/*0x2f*/	/* 拡張可能 */
