...
/* 0:しない。 1:する。 (r41)現在0固定。現状 1は無理なので、改良予定。 */
#define USE_VS3 (0)/*(ボス1人まで対応)*/
//#define USE_VS3 (1)/*(ボス3人モード対応)*/
/*(
	ボス3人モードに現在あえて対応していない理由は、
	純粋にpspでメモリが確保できるかどうかです。
	(r41-)ボス3人モードは一応動くみたい。
)*/
...
/*---------------------------------------------------------
	基本カードスクリプト用命令(opecode)型
---------------------------------------------------------*/
#define CPU_FUNC(aaa)			void op_##aaa(void)
#define CPU_CALL(aaa)			op_##aaa()
