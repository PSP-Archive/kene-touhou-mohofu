...
extern GAME_COMMAND_OBJ_FUNC(07_game_all_clear)
{
	#if 0
	/* なんか知らんが、Lunaticでちょっとチェックしたら、オーバーフローするｗ。 */
	player_dummy_add_score(adjust_score_by_difficulty((
				(cg_zanki * score(3000000)) +	/* 残機   x 300万 */
				(cg_bombs * score(1000000)) 	/* 残ボム x 100万 */
			)));
	#else
	/* なんか知らんが、オーバーフローするので、個別に足してみる。 */
	player_dummy_add_score(adjust_score_by_difficulty(( (((u32)cg_zanki) * score(3000000)) ))); 	/* 残機   x 300万 */
	player_dummy_add_score(adjust_score_by_difficulty(( (((u32)cg_bombs) * score(1000000)) ))); 	/* 残ボム x 100万 */
	#endif
	/* 清算して消える */
	cg_zanki = 0;
	cg_bombs = 0;
