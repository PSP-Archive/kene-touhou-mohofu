...
/*key_config.c my_file.c my_math.c*/global u32 pad_config[KINOU_08_WARIATE_MAX];
...
//extern SDL_Surface *load_chache_bmp2(char *set_filename)
extern SDL_Surface *load_chache_bmp(void)/*char *file_name*/
{
	SDL_Surface *s1;
	SDL_Surface *s2;
	//
...
//	s1 = SDL_LoadBMP(my_file_common_name);
...
extern void unloadbmp_by_surface(SDL_Surface *img_surface)
{
	MY_IMAGE_LIST	*tmp_list;
...
/*---------------------------------------------------------
	ファイル名で指定
---------------------------------------------------------*/
extern void psp_load_bg_file_name(void)/*char *file_name*/
{
	...
	}
	{
		SDL_Surface *loadpic;
		#if (1==USE_KETM_IMAGE_CHACHE)
		loadpic 	= load_chache_bmp();
		#else
		loadpic 	= IMG_Load(my_file_common_name);
		#endif
	//	psp_clear_screen();
	//	SDL_SetAlpha(loadpic, SDL_SRCALPHA, 255);
	//	PSPL_UpperBlit(loadpic, NULL, cg.sdl_screen[SDL_00_VIEW_SCREEN], NULL);
		PSPL_UpperBlit(loadpic, NULL, cg.sdl_screen[SDL_01_BACK_SCREEN], NULL);
		#if (1==USE_KETM_IMAGE_CHACHE)
...
---------------------------------------------------------*/
extern char get_stage_chr(int stage_type_number)
{
