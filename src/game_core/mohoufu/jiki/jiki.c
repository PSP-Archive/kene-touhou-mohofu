...
//global   signed int cg_VR20_game_difficulty;	/* GAME_CORE_GLOBAL_CLASSに、意図的に入れないもの */
global u32 cg_game_select_player;	/* GAME_CORE_GLOBAL_CLASSに、意図的に入れないもの */
#endif
...
static /*(汎用)*/OBJ_CALL_FUNC(player_move_tachie)
{
	// ボス(立ち絵移動)
	#if 0/*(等速移動)*/
	kaiwa_sprite[0].center.y256 	= t256(272+128) - ((src->jyumyou)<<9);
	#else/*(加減速移動)*/
	{
		u32 aaa = (src->jyumyou)+(src->jyumyou);
		aaa += 128;/*(上下入れ替え)*/
		aaa &= 0xff;/*(0...255)*/
...
static void player_create_bomber_tachie(void/* OBJ *src */)
{
	...
	#if 0
		kaiwa_sprite[0].color32 		= MAKE32RGBA(0xff, 0xff, 0xff, 0xff);
		/* 立ち絵初期位置 */
		kaiwa_sprite[0].center.x256 	= -t256(150);	// (200)	/* 0 */  /* (src->center.x256) */
		kaiwa_sprite[0].center.y256 	=  t256(150);	// (100)	/* GAME_HEIGHT+1 */ /* (src->center.y256) */
	...
	//	kaiwa_sprite[0].center.x256 	= t256(256);
		...
		h->jyumyou						= (127);/*on*/	/* 時間制限でおしまい */
	#endif
		/* 描画用角度 */
		h->rotationCCW65536		= (0);// (256);
...
extern PLAYER_FUNC(reimu_yuyuko_create_bomber_kekkai_parrent);			/* 霊夢専用 ボムの親 */
//extern PLAYER_FUNC(yuyuko_create_bomber_gyastry_dream_parrent);		/* 幽々子専用 */
#define yuyuko_create_bomber_gyastry_dream_parrent reimu_yuyuko_create_bomber_kekkai_parrent

extern PLAYER_FUNC(marisa_create_bomber_master_spark_parrent);			/* 魔理沙 B専用 ボムの親 */
extern PLAYER_FUNC(chrno_q_create_bomber_gyastry_dream_parrent);		/* チルノ⑨専用 */
extern PLAYER_FUNC(marisa_create_bomber_star_dust_levarie_parrent); 	/* 魔理沙 A専用 低速ボムの親 */
extern PLAYER_FUNC(remilia_create_bomber_cross_shot_parrent);			/* レミリアボムの親 */
extern PLAYER_FUNC(yuyuko_create_bomber_meifu_parrent); 				/* 幽々子専用 低速ボムの親 */
extern PLAYER_FUNC(marisa_create_bomber_homing_parrent);				/* 低速ボムの親 */
extern PLAYER_FUNC(reimu_create_bomber_homing_shot_parrent);			/* 霊夢専用 低速ボムの親 */
/*---------------------------------------------------------
	プレイヤーキー操作
---------------------------------------------------------*/
...
static void player_keycontrol(OBJ *s1)
{
	/*---------------------------------------------------------
		プレイヤーボム関連処理
	---------------------------------------------------------*/
//	if (STATE_FLAG_0x0200_IS_KAIWA_MODE==(cg_state_flag & STATE_FLAG_0x0200_IS_KAIWA_MODE))
//	{
//		;	/* 会話イベント中はボム関連の処理はしない */
//	}
//	else
	/*(ボンバー有効期間中)*/
	if (0 < cg_bomber_time) 	/* ボムウェイト処理 */
	{
		/* シナリオ時にも自機ボンバー時間経過する。(現在my_special()で自機ボム終了しないとシナリオ読まない仕様の為) */
		cg_bomber_time--; /*fps_factor*/
		if (2 > cg_bomber_time)
		{
...
//	if (0 >= cg_bomber_time)
	{
		...
		if (IS_SCENE_KAIWA_TATAKAU)
		{
			...
			{
				if (0 < cg_bombs)/*(残ボンバーがある場合)*/
				{
					cg_bombs--;
					cg_player_data_used_bomber++;	/* 集計システム(player_data)ボム使用回数 */
					cg_state_flag	|= JIKI_FLAG_0x0020_BOMBER_SYUU_SYUU; /*(ボムによる自動収集可能)*/
...
//					if (PLAYER_STATE_01_KURAI_BOMB_UKETUKE==cg.special_ivent_type)/* 喰らいボム受付中 */
					...
					{
						cg_player_data_use_kurai_bomb++;/* 集計システム(player_data)喰らいボム成功 */
					}
//					pds_status_timer = LIMIT_128;// cg.special_ivent_type	= PLAYER_STATE_03_SAVE_02;
					/* cg_bomber_time と ->jyumyouで同時に動作 */
					jiki_shot_set_bomber_time();
					//	#define USER_BOMOUT_WAIT (30)
					pds_status_timer	= cg_bomber_time + USER_BOMOUT_WAIT;/* 実質無敵時間 */
				...
				//
					...
					kanji_cursor_move_home_position();		/* カーソルを1行目(==0)へ移動 */
					{
						...
						cg.msg_time 				= cg_bomber_time;	/* byou60(5) */ /* 約 5 秒 */
						typedef struct
						{
...
//				zzz_obj_maru					= &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_01_JIKI_MARU];
				...
				obj_tei_soku->rotationCCW65536 += (10<<6);
				#endif
			}
	...
	if (psp_pad.pad_data & PSP_KEY_UP/* PSP_CTRL_UP */) /* 註：斜め上でも回収可能 */ /* && (s1->y>0) */
	{
		...
		#else
		if ((cg_weapon_power==MAX_POWER_IS_128) 	// 128 変更 /* max==MAX_POWER_IS_128==「129段階」 */
			/* 魔理沙 特殊能力：アイテム上部収集がいつでも可能 */
			|| (MARISA_A==(cg_game_select_player))
			|| (MARISA_B==(cg_game_select_player))
		//	|| (MARISA_C==(cg_ga me_select_player))
		)	/* 魔理沙は常に上部自動収集が可能 */
		#endif
		{
			if (s1->center.y256 < ITEM_GET_BORDER_LINE256)/* [アイテム引き寄せライン] */
			{
				cg_state_flag |= JIKI_FLAG_0x0010_JYOU_BU_SYUU_SYUU;	/* 上部自動収集可能 */
			}
		}
	}
	else
	...
	{
		//if (cg_state_flag & (JIKI_FLAG_0x0010_JYOU_BU_SYUU_SYUU /* | JIKI_FLAG_0x0020_BOMBER_SYUU_SYUU */) )/* (?)たぶん */
		{
			cg_state_flag &= (~JIKI_FLAG_0x0010_JYOU_BU_SYUU_SYUU); 	/* 上部自動収集不可 */
		}
	}
//
		/*---------------------------------------------------------
	...
	if (IS_SCENE_KAIWA_TATAKAU)
	{
		...
		{
			cg_state_flag |= JIKI_FLAG_0x0080_SHOT_KEY_SEND_FOR_OPTION; 	/* on(オプションがショットを撃つ) */
	...
	{static int anime_delay;
		...
		if (0 > anime_delay)
		{
			...
			{
				if (auto_anime_frame > 4) {auto_anime_frame--;}
				if (auto_anime_frame < 4) {auto_anime_frame++;}
			}
			{
				...
				auto_aniime &= 3;
				#if (1)
				register int now_anime_frame;
				now_anime_frame = (auto_anime_frame);//
					 if (4 == auto_anime_frame) 	{now_anime_frame += (auto_aniime);	}	/* 中心1-4 */
				else if (4 < auto_anime_frame)		{now_anime_frame += ((3) + (8)); 	}	/* 右側1-4 */	/* +(8) == オプションインターリーブ */
			//	else								{;									}	/* 左側1-4 */
...
/*
0 1 2 3 4 5 6  7  8  :: auto_anime_frame[0 ... 8]
0 1 2 3 4 8 9 10 11  :: now_anime_frame [0 ... 11]
		5
		6
		7
 */
...
---------------------------------------------------------*/
/* [cg_weapon_powerが変更された場合に必ず行う後チェック] */
static void check_weapon_level(void)
{
	...
	u8 pds_weapon;	pds_weapon = cg_weapon_power;
	...
	/* 紅、調べたら丁度2倍っぽい。 */
	{	/* 丁度2倍だとこんな感じで簡略化できる。 */
	//	cg_jiki_weapon_level_offset = ((cg_ga me_select_player)/* 0 */);
	//	cg_jiki_weapon_level_offset += (WEAPON_L5<<3);
	//	cg_jiki_weapon_level = ((0)/* 0 */);
		cg_jiki_weapon_level = (WEAPON_L5);
		int jj;
		for (jj=(0); jj<(5); jj++)
		{
...
static int player_add_power_ttt_type(OBJ/**/ *src, u8 ttt_type)
{
	...
	//	const int add_power = aaa[/* ((ttt_type)) */((NULL==ttt)?(0/* 3 チルノ用 */):((ttt->type)&0x03))];
		...
		int previous_power; previous_power = cg_weapon_power;		/* アイテム取得する直前のパワー値 */
		cg_weapon_power += add_power;	/* パワー値、加算 */
		/* パワー値、超えてたら修正 */
		#if (0)
		if ((MAX_POWER_IS_128-1) < cg_weapon_power )/* 127==128段階目 */	{	cg_weapon_power = MAX_POWER_IS_128;/* 最大値 */ 	}
		#else/*(ok?)*/
		cg_weapon_power = psp_min(cg_weapon_power, (MAX_POWER_IS_128));
		#endif
		if (MAX_POWER_IS_128 == cg_weapon_power/* previous_power */) /* maxの場合 */
		{
			if (previous_power != cg_weapon_power)/* 直前がMAXじゃなかったのに、今回MAXになった場合 */
			{
				#if 1
				/* 特殊機能 */
			...
			cg_chain_point += add_power;	/* チェイン、加算 */
			/* チェイン、超えてたら修正 */
			#if (0)
			if (31 < cg_chain_point)	{	cg_chain_point = 31;/* 最大値 */	}
			#else/*(ok?)*/
			cg_chain_point = psp_min(cg_chain_point, (31));
			#endif
			/* (ノーミスボーナス)ウェポンボーナス得点計算 */
			add_score_point = (/* score_type */(cg_chain_point));
			#if 1
		...
		//	if (NULL==ttt)/* チルノなら */
			...
			if ((REMILIA-1) < (cg_game_select_player))/* レミリア & 幽々子 & チルノなら */
				#endif
			{
				if ((8-1) < cg_bombs)	{	;	}/* 既に最大値(8)ならば、なにもせず。 */
				else
				{
					cg_bombs++; 	/* ボム追加 */
					cg_weapon_power = (MAX_POWER_IS_128>>1);/* 最大値の半分 */
				}
			}
			#endif
		}
	...
	check_weapon_level();/* [cg_weapon_powerが変更された場合に必ず行う後チェック] */
//
 ...
 */
static PLAYER_S1_FUNC(player_collision_check_item)/* , int mask */ /* , OBJ_Z03_ITEM */
{
	/* 自機がアイテムにあたった場合 */
	OBJ *ttt;	//対象
	ttt = obj_collision_check_00_tama(s1, OBJ_Z03_ITEM); /* 弾専用(アイテム) */ /* mask */
	if (NULL != ttt)
	{
	...
	//	case_SP_ITEM_00_P001:	//	player_add_power(s1, ttt, 1);		break;	// ウェポンアイテム[小p]
	//	case_SP_ITEM_01_P008:	//	player_add_power(s1, ttt, 8);		break;	// ウェポンアイテム[大P]
	//	case_SP_ITEM_02_P128:	//	player_add_power(s1, ttt, 127); break;	// ウェポンアイテム[F]
		case_POWER_UP:	// ウェポンアイテム[小p][大P][F]
			;
	//	if (NULL!=ttt)/* チルノ以外なら */
		{	const int add_score_point = player_add_power_ttt_type(s1, ttt->obj_type_set);
		...
		case_SP_ITEM_04_BOMB:
			if ((8-1) < cg_bombs)	{goto add_10000pts; }/* 既に最大値(8)ならば、10000+ [pts] */
			cg_bombs++;
			#if 1/* バグfix? */
			ttt->jyumyou = JYUMYOU_NASI;/* おしまい */
			#endif
			...
			goto case_break;
		case_SP_ITEM_03_1UP:
			if ((8-1) < cg_zanki)	{goto add_10000pts; }/* 既に最大値(8)ならば、10000+ [pts] */
			cg_zanki++;
			#if 1/* バグfix? */
			ttt->jyumyou = JYUMYOU_NASI;/* おしまい */
			#endif
		...
		case_SP_ITEM_05_TENSU:
			...
			else/*(通常)*/
			{
				...
				int add_score_point;
				if (ITEM_GET_BORDER_LINE256 < s1->center.y256) /* ITEM_GET_BORDER_LINE256 未満の場合は、ITEM_GET_BORDER_LINE256までの距離に応じて減点 */
				{
				...
				//	add_score_point = (SCORE_10000);	/* 基本点 10000[pts] */
					add_score_point = (SCORE_10000+(cg_VR20_game_difficulty));	/* 基本点 10000[pts](easy), 11000(normal), 12000(hard), 512000(lunatic). */
				}
...
static PLAYER_S1_FUNC(player_collision_check_tama)
{
	//static PLAYER_FUNC(player_collision_check_graze)
	/*---------------------------------------------------------
	...
	---------------------------------------------------------*/
	#if (1)
	s1->m_Hit256R		= TEST_FIX_GRAZE048;/* てすと、グレイズ判定。 */
	#endif
	OBJ *ttt;	/* 敵弾 */
	ttt = NULL;
	ttt = obj_collision_check_00_tama(s1, OBJ_Z04_TAMA);/* 弾専用 */
	if (NULL != ttt)
	{
		/* プレイヤーに敵弾があたった場合はグレイズする */
		if (0==(ttt->TR00_hatudan_spec_data & TAMA_SPEC_4000_GRAZE))/* グレイズ済? */
		{
			ttt->TR00_hatudan_spec_data |= TAMA_SPEC_4000_GRAZE;/* グレイズ済 */
			cg_graze_point++;/* グレイズ数 */
			// チルノAとQの場合、グレイズでパワーアップ。
		//	if ((CIRNO_A-1) < cg_ga me_select_player)/* 6:CIRNO_A or 7:CIRNO_Q (-r34) */
			// チルノAの場合のみ、グレイズでパワーアップ。
			if (CIRNO_A == cg_game_select_player)/* 6:CIRNO_A only. (r35-) */
			{
				player_add_power_ttt_type(s1, /* ttt->type */(3)/*(3==チルノA専用)*/);
			}
			else
			{
				player_dummy_add_score((score(100)+score(200)*(cg_VR20_game_difficulty)));/* score(500) */
			}
			/* グレイズ音 */
...
//			#else
			...
			chache_m_Hit256R	= s1->m_Hit256R;/* 自機のあたり判定サイズ */
			s1->m_Hit256R		= zzz_obj_maru->m_Hit256R;/* ○のあたり判定サイズ */
			uuu = obj_collision_check_00_tama(s1, OBJ_Z04_TAMA);/* 弾専用 */
			s1->m_Hit256R		= chache_m_Hit256R;/* 戻す */
//			#endif
		#else
			s1->m_Hit256R		= TEST_FIX_DEATH001;/* てすと、死亡判定。 */
			/* 自機で判定 */
			uuu = obj_collision_check_00_tama(s1, OBJ_Z04_TAMA);/* 弾専用 */
			s1->m_Hit256R		= TEST_FIX_GRAZE048;/* てすと、グレイズ判定。 */
		#endif
...
static PLAYER_S1_FUNC(player_collision_check_teki)
{
	#if (1)
	s1->m_Hit256R		= TEST_FIX_TEKI001;/* てすと、敵判定。(敵との当たり判定で死ぬ) */
	#endif
	//	static PLAYER_FUNC(player_collision_check_teki)
	{
		OBJ *ttt;	/* 敵自体(ボス/中型/ザコ) */
		/* */  // (OBJ_Z02_TEKI );
		ttt = obj_collision_check_A01_A02_teki(s1);// /* 敵専用 */
		if (NULL != ttt)
		{
			...
			//	{
			//		雑魚がダメージを受ける。
					player_weapon_collision_check(s1, PLAYER_WEAPON_TYPE_01_BOMB_PLAYER);/* ボム/playerで敵を倒すあたり判定 */
			//	}
			}
		}
	}
...
//	cg_state_flag		&= (~SCENE_NUMBER_MASK);	/*(シーンを消す)*/
//ダメ。理由==continue(?)	cg_state_flag		&= (0xffff00ffu);	/*(シーンを消す)*/
	cg_state_flag		|= (0x00008000u); //プレイヤーループを抜ける処理(とりあえず)
	#endif
...
//	if ((CIRNO_A-1) < cg_ga me_select_player)/* 6:CIRNO_A or 7:CIRNO_Q (-r34) */
	...
	if (CIRNO_A == cg_game_select_player)/* 6:CIRNO_A only. (r35-) */
	{
		cg_bombs			= (0);	/* ボム所持数の初期化 */
	}
	else/* チルノ以外 */
	{
		cg_bombs			= option_config[OPTION_CONFIG_01_BOMB]; 	/* ボム所持数の初期化 */
		/* static_fix_status[BASE_BOMBS+(cg_ga me_select_player)] */  /* 3 */
	}
	cg_chain_point			= (0);										/* ノーミスボーナスの初期化 */
	// cg_chain_point(ノーミスボーナス)の初期化 // どれだけ連続(ノーミス)でwepon_upを取ったか
//
	check_weapon_level();/* [cg_weapon_powerが変更された場合に必ず行う後チェック] */
}
...
static void player_explode(OBJ *s1)
{
 ...
 WEAPON_L5(P128)
			 ...
			 */
		//	item_create(s1, (((30-1) > cg_weapon_power)?(SP_ITEM_01_P008):(SP_ITEM_00_P001)), (7)/* 5 */, ITEM_MOVE_FLAG_06_RAND_XY);
		//	item_create(s1, (((16) > cg_weapon_power)?(SP_ITEM_01_P008):(SP_ITEM_00_P001)), (6), ITEM_MOVE_FLAG_06_RAND_XY);/* いくら下手でも、多少多いかな? */
		//	item_create(s1, SP_ITEM_01_P008, 1, ITEM_MOVE_FLAG_06_RAND_XY);
			#else
			/* 多少修正してみる */
			item_create_flags_num(s1, (((16) > cg_weapon_power)?(SP_ITEM_01_P008):(SP_ITEM_00_P001)), (3), (ITEM_MOVE_FLAG_02_WAIT|ITEM_MOVE_FLAG_06_RAND_XY) );/* 下手な人ががんばれば3つ取れる事が前提 */
		...
		if (REMILIA==(cg_game_select_player))	/* レミリアの場合 */
		{
			...
			{
				/* 使ってない分の持ってるボムを吐き出す */
				item_create_flags_num(s1, SP_ITEM_04_BOMB, (cg_bombs), (ITEM_MOVE_FLAG_02_WAIT|ITEM_MOVE_FLAG_06_RAND_XY) );
			}
		}
	}
//		cg.explode		= 0;
//	if (0==cg.explode)
	...
	//	cg.bonus=0;
		...
		#endif
		/*	 */
		cg_bomber_time	= 0;			/* ←ここが無いせいで魔理沙(&チルノ)ボム撃てなくなってた */
		#if 1
	//	cg_bomber_time = 0;/* 都合上 */
		set_bg_alpha(255);/* 画面を明るくする */
		#endif

	//	cg_state_flag		= JIKI_FLAG_0x00_ALL_OFF;/* (?)*/		/*(全フラグを消す特殊機能)*/
		cg_state_flag		&= (~(	JIKI_FLAG_0x0010_JYOU_BU_SYUU_SYUU			|		/* 終わり */
...
//			cg.special_ivent_type	= 0;/* (?)*/
			...
			{
				cg_player_data_use_continue++;	/* コンティニュー回数 */
			//	render_continue(/* cg_game_now_max_continue */);
				cg.main_call_func = ask_continue_menu_start;
			}
			else
			{
				cg.main_call_func = gameover_start;
			}
//			break;
	}
	else
	...
	if (LIMIT_m16_GAME_OUT == pds_status_timer) /* プレイヤー、ガッツ減少処理 */
	{
		...
		option[3]->PL_OPTION_DATA_yuukou_flag	= (PL_OPTION_FLAG_00_OPTION_OFF);	/* オプションを消す */
		#endif/* 0000 */
		cg_player_data_count_miss++;	/* ミス回数 */
		/*(ゲームランクを下げる)*/
		cg_game_rank >>= 1;
		/* 残チャンス減らす */
		cg_zanki--;
		if (0 > cg_zanki)	/* 残チャンスがなければ */
		{
			player_loop_quit();
		}
		else
		...
		/*	MAX(128)->死ぬ->(112)->死ぬ->(96)->死ぬ->(80)->死ぬ->(64)	(常に16減で一定)	 */
			cg_weapon_power -= (16);	/* 武器レベルの低下(紅、調べたら難易度に関係なくP16減だった) */
			#if (0)
			if (0 > cg_weapon_power)	{	cg_weapon_power = 0;	}
			#else/*(ok?)*/
			cg_weapon_power = psp_max(cg_weapon_power, (0));
			#endif
			player_fukkatsu_aaa(s1);	/* プレイヤー位置の初期化 */
...
static PLAYER_S1_FUNC(any_player_move)
{
	cg_state_flag &= (~JIKI_FLAG_0x0080_SHOT_KEY_SEND_FOR_OPTION);	/* off(オプションがショットを撃つのを止める) */
	...
	if (0 == pds_status_timer)	/* 通常時の場合 */
	{
		...
		player_collision_check_tama(s1);
		{
			/* [0] ゲームランクを上げる。 */
			cg_game_rank++;
			cg_game_rank = psp_min(cg_game_rank, 65535);/*(最大ランク)*/
			#if (0)/*デバッグ用(スコア欄にゲームランクを表示)*/
			cg_game_score	= cg_game_rank;
			#endif
		}
	}
	else	/* 例外時(特殊処理)の場合 */
...
{
	cg_zanki							= (1+option_config[OPTION_CONFIG_00_PLAYER]);
	cg_game_score						= score(0);
	#if (1==USE_EXTEND_CHECK)
...
//	#define TEISOKU_EFFECT_01_OBJ (7)
	...
	//	s1->m_Hit256R		= graze_atari[(cg_ga me_select_player)];/* グレイズ判定 */
...
extern void obj_initialize_one(OBJ *obj);
...
extern void jiki_class_initialize(void)
{
	...
	//
	#if (1)
	for (VR03_i=0; VR03_i<OBJ_POOL_04_PANEL_0056_MAX; VR03_i++)/* 全部 */
	{
		OBJ *obj;/* 初期化するスプライト */
		obj = &obj99[OBJ_HEAD_04_0x0a10_PANEL+VR03_i];
		obj_initialize_one(obj);/* 1枚のスプライトオブジェクトを初期化 */
	}
	#endif
	//
	#if (1)
	VR02_j = t256(((GAME_WIDTH)+(5*8)+(4)));/*[t256(x位置pixel)]*/
	for (VR03_i=0; VR03_i<(10); VR03_i++)/*(2)*/
	{
		OBJ *obj;/* 初期化するスプライト */
		/* スコア */
		obj = &obj99[OBJ_HEAD_04_0x0a10_PANEL+VR03_i+PANEL_OBJ_00_S00];
		obj->center.x256 = (VR02_j);
		obj->center.y256 = ((6*8+7)<<8);
		/* ハイスコア */
		obj = &obj99[OBJ_HEAD_04_0x0a10_PANEL+VR03_i+PANEL_OBJ_10_H00];
		obj->center.x256 = (VR02_j);
	...
	#if (1)
	//----[EFFECT]	/* ボスeffect を作成 */
	for (VR03_i=0; VR03_i<(5+3/*+3*/); VR03_i++)
	{
		OBJ *h;
		h						= obj_regist_direct_number(OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_08_EFFECT01+VR03_i); /* 必ず登録できる。 */
	...
	//
	cg_player_data_use_continue 		= 0;	/* コンティニュー回数 */
	cg_player_data_count_miss			= 0;	/* ミス回数 */
	cg_player_data_used_bomber			= 0;	/* ボム使用回数 */
	cg_player_data_use_kurai_bomb		= 0;	/* 喰らいボム使用回数 */
//	cg_player_data_count_bonus			= 0;	/* カードボーナス回数 */
//
	cg_bomber_time						= 0;	/* ==bomb_wait */
	cg_graze_point						= 0;
	cg_game_rank						= 0;	/* ゲームランク */
//
	/* (r32) */cg_state_flag			= JIKI_FLAG_0x00_ALL_OFF;/*(全フラグを消す特殊機能)*/
//
	card.boss_hp_dec_by_frame			= 0;/* ボス攻撃減少値、フレーム単位 */
//
//	cg_jiki_weapon_level_offset 		= (cg_ga me_select_player) + (/* 0==武器レベル */0<<3);
	cg_jiki_weapon_level				= (/* 0==武器レベル */0<<3);/*(r35u2)*/
	/* 練習モードの場合はフルパワーで始める(その代わりクリア後のイベントが見れない) */
	cg_weapon_power 					= (0==cg.game_practice_mode)?(0):(MAX_POWER_IS_128);
	//
	#if 1/* どこかで初期化が必ず必要(ここでない方が良いかも？) */
	HATSUDAN_R14_tama_spec				= (DANMAKU_LAYER_00)|(0);/* どこかで初期化が必ず必要 */
	cg_BOSS_DATA_32_chuu_boss_mode		= (chuu_boss_00_mode_off);/* どこかで初期化が必ず必要 */
	#endif
	//
...
---------------------------------------------------------*/
extern void jiki_eien_muteki_on(void)
{
//	cg.special_ivent_type	= PLAYER_STATE_02_SAVE_01;	// 無敵状態？
