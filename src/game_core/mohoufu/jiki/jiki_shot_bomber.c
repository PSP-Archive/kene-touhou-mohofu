...
/* static */extern void player_weapon_collision_check(OBJ *shot, int erase_shot_type)
{
//	OBJ *shot;	/* 自弾 */
...
static SHOT_FUNC(check_collision_gamengai)
{
	if (((src_shot->center.y256)+(t256(-10))) < t256(0))/*(画面外に出たら)*/
	{
		src_shot->jyumyou = JYUMYOU_NASI;/*(ショットおしまい)*/
	}
	else/*画面内なら*/
	{
		player_weapon_collision_check(src_shot, PLAYER_WEAPON_TYPE_00_SHOT);/*(あたり判定チェック)*/
	}
}
...
static SHOT_FUNC(other_move_object)
{
	src_shot->center.x256 += src_shot->math_vector.x256;		/*fps_factor*/
	src_shot->center.y256 += src_shot->math_vector.y256;		/*fps_factor*/
	check_collision_gamengai(src_shot);
}
/*---------------------------------------------------------
	レミリア専用、ショット弾の移動
---------------------------------------------------------*/

static SHOT_FUNC(remilia_move_object)
{
//	if (REMILIA==(cg_ga me_select_player))	/* レミリアはゆらゆら */
	{	u16 rand_int;
		rand_int = ra_nd(); 	/*(リプレイ時に再現性が必要)*/
	//	rand_int = vfpu_rand_8888(0, 255);
		src_shot->center.x256 -= (rand_int&0x0200);/*0x0100*/
		src_shot->center.x256 += (rand_int&0x0100);/*0x0080*/
		src_shot->center.x256 += (rand_int&0x0100);/*0x0080*/
	}
	#if (0)
	src_shot->center.x256 += src->math_vector.x256; 	/*fps_factor*/
	src_shot->center.y256 += src->math_vector.y256; 	/*fps_factor*/
	check_collision_gamengai(src_shot);
	#else
	other_move_object(src_shot);
	#endif
}
...
/*---------------------------------------------------------
	誘導サブ。
---------------------------------------------------------*/
static void yuudo_aaa(OBJ/**/ *src_shot, int bbb)
{
	#if (1)
	/* パワーの量に比例してショットの軌跡を曲げる。 */
	src_shot->math_vector.x256 += ((src_shot->math_vector.x256 * bbb)>>11);
	#endif
	//
	#if (1)
	/* ショットを誘導する。 */
	...
	if (obj_boss->center.x256 < src_shot->center.x256)/*(ボスが左 < 幽々子が右)*/
	{
		if (0 < src_shot->math_vector.x256)/*(正==幽々子ショットが右へ移動中)*/
		{
			src_shot->math_vector.x256 -= src_shot->math_vector.x256;/* 符号反転(負方向==幽々子ショットを左へ誘導) */
		}
	...
	{
		if (0 > src_shot->math_vector.x256)/*(負==幽々子ショットが左へ移動中)*/
		{
			src_shot->math_vector.x256 -= src_shot->math_vector.x256;/* 符号反転(正方向==幽々子ショットを右へ誘導) */
		}
	...
	#if 0
	/* 誘導する */
	if (src->center.x256 < src_shot->PL_SHOT_DATA_target_x256 )
	{
	//	src_shot->center.x256 += src_shot->math_vector.x256;		/*fps_factor*/
	//	src_shot->center.x256 -= src_shot->math_vector.x256;		/*fps_factor*/
	}
...
/*---------------------------------------------------------
	霊夢Aザブトン専用、ショット弾の移動
---------------------------------------------------------*/
static SHOT_FUNC(zabuton_move_object)
{
	//[旧位置]
...
//	if ((REIMU_B+1) > (cg_ga me_select_player)) 	/* 霊夢の回転ショット */
	...
	//ザブトン専用	if (((JIKI_SHOT_01)|OBJ_Z80_SHOT_SPECIAL)==src->obj_type_set)
		{
			/* 描画用グラ回転 */
			src_shot->rotationCCW65536 += ((32)<<6);	/* cv65536r(10) */
			mask65536(src_shot->rotationCCW65536);
		}
	}
	yuudo_aaa(src_shot, (128) );
	#if (0)
	src_shot->center.x256 += src_shot->math_vector.x256;		/*fps_factor*/
	src_shot->center.y256 += src_shot->math_vector.y256;		/*fps_factor*/
	check_collision_gamengai(src_shot);
...
static SHOT_FUNC(yuyuko_move_object)
{
	static u16 pow_low;
	pow_low++;
//	if ((1024) < pow_low)/* 1024発移動するごとに御腹が空く */
	if ((4096) < pow_low)/* 1024発移動するごとに御腹が空く */
	{
		pow_low = 0;
		if (0 < cg_weapon_power)/* パワーがあれば */
		{
			cg_weapon_power--;/* 減らす */
		}
	}
	//[旧位置]
	yuudo_aaa(src_shot, (cg_weapon_power) );
	//
	#if (0)
	src_shot->center.x256 += src->math_vector.x256; 	/*fps_factor*/
	src_shot->center.y256 += src->math_vector.y256; 	/*fps_factor*/
...
//extern void player_register_shot_object(OBJ/**/ *src, int /*set_*/shot_type);
/*static*/static/*extern*/ void player_register_shot_object(OBJ/**/ *src, int /*set_*/shot_type)
{
	OBJ *h;/* shot */
	h					= obj_regist_teki();
	if (NULL!=h)/* 登録できた場合のみ */
	...
	//		/* CCWの場合 */
			{ (  0),	( -7),	(	0), 	0x01}, // NEEDLE_ANGLE_270_M	cv65536r((180)), 			/* 魔理沙 A/B 霊夢 レミリア ＆ チルノ */
			{ ( -5),	( +4),	(	0), 	0x01}, // NEEDLE_ANGLE_270_L	cv65536r((180)), 			/* オプション左から発弾 (霊夢B 専用) */
			{ ( +5),	( +4),	(	0), 	0x01}, // NEEDLE_ANGLE_270_R	cv65536r((180)), 			/* オプション右から発弾 (霊夢B 専用) */
			//
			{ (  0),	( -7),	(-108), 	0x01}, // NEEDLE_ANGLE_263	cv65536r((180-25)), /*263=270-7*/ /* レミリア ＆ チルノ */
			{ (  0),	( -7),	( 108), 	0x01}, // NEEDLE_ANGLE_277	cv65536r((180+25)), /*277=270+7*/ /* レミリア ＆ チルノ */
			{ (  0),	(-47),	(	5), 	0x01}, // NEEDLE_ANGLE_271	cv65536r((181)), 				/* 幽々子専用 */	/*-48*/ /*t256( 0), t256(-40), */
		};
	//	int shot_type;
		...
		if (0x00 == shot_status_hari)	/* 針以外 */
		{
			...
			//	if (0x02== shot_status_hari) /* 霊夢の回転ショット */
				if (REIMU_SHOT_TYPE_00 != shot_type)/* 誘導ショット以外？ */
				{
					...
					/* 描画用グラ回転 */
					/*(リプレイ時に再現性が必要ない)*/
				//	h->rotationCCW65536 	= (ra_nd() & (65536-1));/* 出だしの角度はランダムでないと(レーザーみたいな)変な画面になる */
				//	h->rotationCCW65536 	= (vfpu_rand_8888(0, 255) & (65536-1));/* 出だしの角度はランダムでないと(レーザーみたいな)変な画面になる */
					h->rotationCCW65536 	= (vfpu_rand_8888(0, 255) << 6);/* 出だしの角度はランダムでないと(レーザーみたいな)変な画面になる */
				}
	...
	//	h->kougeki_ti = ddd_tbl[DDD_STRENGTH_L1+(cg_jiki_weapon_level_offset)];
	//	h->kougeki_ti = ccc_tbl[CCC_STRENGTH_L1+(cg_jiki_weapon_level_offset)+(is_needle<<(4+2))];/* 64 == 8[players]*8[line] */
		/*(r35u2-)*/
		h->kougeki_ti = (((int)current.shot_status[SHOT_STATUS_00_SHOT_STRENGTH_L1+(cg_jiki_weapon_level)+(is_needle<<(3))])<<2);/* 64 == 8[players]*8[line] */
		/*
			攻撃値: ボンバーの威力。
		*/
	//
	...
	//	ok=0;
		...
		//	if (REIMU_SHOT_TYPE_01 > shot_type)/* 誘導ショットか？ */
			...
			#endif
			{
				...
				//	if (REIMU_SHOT_TYPE_00 == shot_type)/* 誘導ショットか？ */
					{
						VR04	= (target->center.x256);			/*(狙い先)*/
						VR05	= (target->center.y256);			/*(狙い先)*/
						VR06	= (zzz_player->center.x256);		/*(狙い元)*/
						VR07	= (zzz_player->center.y256);		/*(狙い元)*/
						CPU_CALL(cpu_VR_angleCCW65536_src_nerai);
				//(同じ)VR13	= HATSUDAN_R13_angle65536;
				//		int_angle1024 >>= (6);
					//	if (511 < int_angle1024 )/* 上方180-360度[/360度]の領域のみ追尾する */
					//	if (/*(384)*/(256*3) < (( int_angle1024+128+256)&(1024-1)) )/* 上方225-315度[/360度]の領域のみ追尾する */	/* CCWの場合 */
					/*r42とりあえず*/if (/*(384)*/(256*3) < (( (VR13>>6)+128+256)&(1024-1)) )/* 上方225-315度[/360度]の領域のみ追尾する */	/* CCWの場合 */
						{
							/*16==ccc_tbl[CCC_SPEED+(cg_ga me_select_player)]*/
							/*16==ccc_tbl[CCC_SPEED+(cg_ga me_select_player)]*/
							#if (0)//
							h->math_vector.x256  = si n65536((int_angle65536))*(/*p->speed*/(16));/*fps_factor*/		/* CCWの場合 */
							h->math_vector.y256  = co s65536((int_angle65536))*(/*p->speed*/(16));/*fps_factor*/
							#else
						//	{
						...
						//	}
							//------------------
							HATSUDAN_R11_speed256	= (t256(16));/*t256(p->speed)*/
					//(同じ)HATSUDAN_R13_angle65536 = (VR13);
							...
							{	VR13 += cv65536r((180));/*(省略可)mask65536(VR13);*/}/* 描画用グラ回転(ショットは上が正面なので、回転角を計算)[180/360度]回す。 */
							h->rotationCCW65536 	= VR13;/* 下が正面の角度が描画用角度 */
							#endif
						//	ok=1;/* 自動追尾可能 */
							return;
						}
...
//	if (REMILIA==(cg_ga me_select_player))/* */
	{	/*(左右に分かれる)*/	/* レミリアのみ */
...
/*
 ...
 WEAPON_L5(P128)
*/
/*static*/static SHOT_FUNC(shot_regist_option_glow)
{	/* 攻撃支援 */
	if (src_shot->PL_OPTION_DATA_opt_anime_add_id < OPTION_C3)
	{
	// OPTION_C1: OPTION_C2:
	//	if (cg_weapon_power > (100-1))	/* 旧システムの調整値 100 */
		if (cg_weapon_power > (96)) 	/* システムが変わったので修正してみる(96==128-32) */
		{
			shot_regist_ci_gggg(src_shot);	//	src_shot->PL_OPTION_DATA_opt_shot_interval=(7+3);
		}
		else
		{
			shot_regist_re_hari(src_shot);	//	src_shot->PL_OPTION_DATA_opt_shot_interval=(5+3);
		}
	}
	else
	{
...
static OPTION_FUNC(player_animate_option)
{
//	if (0==anime_fix_status[BASE_OPT_ANIM_TYPE+(cg_ga me_select_player)])
	...
	{	/* オプションが回転する場合 */
		...
		src_option->rotationCCW65536 += (((u32)(current.shot_status[SHOT_STATUS_27_OPTION_ANIME_SPEED]))<<6);/*r42とりあえず*/	/* 描画用グラ回転速度 */
		mask65536(src_option->rotationCCW65536);
	}
	else
	{	/* オプションがアニメーションする場合 */
		src_option->PL_OPTION_DATA_anime_wait--;
		if (src_option->PL_OPTION_DATA_anime_wait <= 0)
		{
		//	src_option->PL_OPTION_DATA_anime_wait = (anime_fix_status[BASE_OPT_ANIM_SPEED+(cg_ga me_select_player)]);
			src_option->PL_OPTION_DATA_anime_wait = (current.shot_status[SHOT_STATUS_27_OPTION_ANIME_SPEED]);
			#if 1
			src_option->obj_type_set++;
			src_option->obj_type_set &= (0xfffffff7); /* 上位ビットに影響を与えないでアニメーション */
			#endif
		}
	}
}
...
/*---------------------------------------------------------
	プレイヤー、オプションの移動制御
---------------------------------------------------------*/
/*static*/extern SHOT_FUNC(player_control_option);	/* 全員 */
/*static*/extern SHOT_FUNC(player_move_option)		/* 全員 */
{
	/* オプションが非表示の場合、何もしない。 */
	if (PL_OPTION_FLAG_00_OPTION_OFF==(src_shot->PL_OPTION_DATA_yuukou_flag)) {return;}
//
	/* アニメーション */
	player_animate_option(src_shot);
//
	/* シナリオ会話モードではショットボタン入力無効 */
//	if (0==(cg_state_flag & STATE_FLAG_0x0200_IS_KAIWA_MODE))
	if (cg_state_flag & JIKI_FLAG_0x0080_SHOT_KEY_SEND_FOR_OPTION)/* off(オプションがショットを撃つ場合) */
	{
		src_shot->PL_OPTION_DATA_opt_shot_interval--;
		if ((src_shot->PL_OPTION_DATA_opt_shot_interval) < 0)
		{
		//	src_shot->PL_OPTION_DATA_opt_shot_interval = player_bbb_status[(cg_jiki_weapon_level_offset)];/* オプションのショット間隔 */
			src_shot->PL_OPTION_DATA_opt_shot_interval = current.shot_status[SHOT_STATUS_18_OPTION_SHOT_INTERVAL_L1+(cg_jiki_weapon_level)];/* オプションのショット間隔 */
			#if 1//
			...
			(ggg[(cg_game_select_player)])(src_shot);
			#endif
		}
	}
	player_control_option(src_shot);
}
...
extern void register_main_shot(OBJ *s1)
{
	...
	else
	{
		...
		//	/*cg.*/weapon_interval = jiki_auto_shot_interval_table[(cg_jiki_weapon_level_offset)];
			/*cg.*/weapon_interval = current.shot_status[SHOT_STATUS_10_JIKI_SHOT_INTERVAL_L1+(cg_jiki_weapon_level)];
		...
		//	(bbb[(cg_jiki_weapon_level_offset)])(s1);
			(bbb[(cg_jiki_weapon_level<<3)+cg_game_select_player])(s1);
		}
	}
}
...
//efine PL_HOMING_KODOMO_DATA_color256			user_data02
...
//#define PL_HOMING_KODOMO_DATA_color256		user_data02
//	OBJ *target_obj;	/* int target_id */
//} PL_HOMING_KODOMO_DATA;	/* == PL_HLASER_DATA */

//typedef struct
//{
#define PL_KEKKAI_DATA_angleCCW65536 			user_data00
#define PL_KEKKAI_DATA_add_r65536				user_data01
#define PL_KEKKAI_DATA_radius256				user_data03 		/* 半径(radius) */
//} PL_KEKKAI_DATA;
...
---------------------------------------------------------*/
static SHOT_FUNC(player_bomber_out_collision_check)
{
	#if 1/*Gu(中心座標)*/
	if ((src_shot->center.x256 < t256(0))||(src_shot->center.x256 > t256(GAME_WIDTH))||
		(src_shot->center.y256 < t256(0))||(src_shot->center.y256 > t256(GAME_HEIGHT)))
	#endif
	{
		src_shot->jyumyou = JYUMYOU_NASI;/* 画面外に出たらおしまい */
	}
	else
	{
		player_weapon_collision_check(src_shot, PLAYER_WEAPON_TYPE_01_BOMB_PLAYER);/* ボムで敵を倒すあたり判定 */
	}
}
...
static SHOT_FUNC(reimu_yuyuko_move_rotate_kekkai) /* 霊夢 */
{
	src_shot->PL_KEKKAI_DATA_angleCCW65536 += ((src_shot->PL_KEKKAI_DATA_add_r65536) );/*(r42)*/	/*fps_factor*/	/*...*/
	mask65536(src_shot->PL_KEKKAI_DATA_angleCCW65536);
	#if 1
	/* 描画用角度(下が0度で左回り(反時計回り)) */
	src_shot->rotationCCW65536 = (src_shot->PL_KEKKAI_DATA_angleCCW65536);
	#endif
//
	int genten_cx256;
	int genten_cy256;
	...
	#if (0)//
	src_shot->center.x256 = genten_cx256 + ((si n65536((src_shot->PL_KEKKAI_DATA_angleCCW65536))*(src_shot->PL_KEKKAI_DATA_radius)) );/*fps_factor*/	/* CCWの場合 */
	src_shot->center.y256 = genten_cy256 + ((co s65536((src_shot->PL_KEKKAI_DATA_angleCCW65536))*(src_shot->PL_KEKKAI_DATA_radius)) );/*fps_factor*/
	#else
//	{
//		int sin_value_t256; 	//	sin_value_t256 = 0;
//		int cos_value_t256; 	//	cos_value_t256 = 0;
//		int256_sin cos65536((src_shot->PL_KEKKAI_DATA_angleCCW65536), &sin_value_t256, &cos_value_t256);
//		src_shot->center.x256 = genten_cx256 + ((sin_value_t256*(src_shot->PL_KEKKAI_DATA_radius)) );/*fps_factor*/
//		src_shot->center.y256 = genten_cy256 + ((cos_value_t256*(src_shot->PL_KEKKAI_DATA_radius)) );/*fps_factor*/
//	}
	//------------------
	HATSUDAN_R11_speed256	= (src_shot->PL_KEKKAI_DATA_radius256);
	HATSUDAN_R13_angle65536 = (src_shot->PL_KEKKAI_DATA_angleCCW65536);
	CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
	src_shot->center.x256 = genten_cx256 + VR07;//sin_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
	src_shot->center.y256 = genten_cy256 + VR06;//cos_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
	//------------------
	#endif

	/* スクエア結界(霊夢B) */
//	if (REIMU_A != (cg_ga me_select_player))
	if (REIMU_B == (cg_game_select_player)) /* スクエア結界(霊夢B) */
	{
		src_shot->rotationCCW65536 		= 0;/* 下が正面の角度が描画用角度 */
		if (0 < src_shot->PL_KEKKAI_DATA_add_r65536)
		{
			if (genten_cx256 < src_shot->center.x256)
					{src_shot->center.x256 = genten_cx256 + (src_shot->PL_KEKKAI_DATA_radius256);}
			else	{src_shot->center.x256 = genten_cx256 - (src_shot->PL_KEKKAI_DATA_radius256);}
		}
		else
		{
			if (genten_cy256 < src_shot->center.y256)
					{src_shot->center.y256 = genten_cy256 + (src_shot->PL_KEKKAI_DATA_radius256);}
			else	{src_shot->center.y256 = genten_cy256 - (src_shot->PL_KEKKAI_DATA_radius256);}
		}
	}
//	if (REIMU==(cg_ga me_select_player))
	{
		if (src_shot->PL_KEKKAI_DATA_radius256 < t256(255) )
		{
			src_shot->PL_KEKKAI_DATA_radius256 += ((ra_nd() & (1<<8)));
		}	/* 少しずつ広がるようにした */
		else/* 回転が広がったらおしまい */
		{
			src_shot->jyumyou = JYUMYOU_NASI;
			cg_state_flag &= (~JIKI_FLAG_0x0020_BOMBER_SYUU_SYUU);	/* ボムによる自動収集は終わり */
		}
	//	src_shot->alpha 			= ((int)(255-src_shot->PL_KEKKAI_DATA_radius))&(256-1); /* だんだん消えるようにした */
	//	src_shot->color32			= ((((255-src_shot->PL_KEKKAI_DATA_radius))&(0xff))<<24)|0x00ffffff; /* だんだん消えるようにした */
	//	src_shot->color32			= ((((255-src_shot->PL_KEKKAI_DATA_radius))&(0xff))<<24)|0x00ffffff; /* だんだん消えるようにした */
		src_shot->color32			= (((((255<<8)-src_shot->PL_KEKKAI_DATA_radius256))&(0xff00))<<(24-8))|0x00ffffff; /* だんだん消えるようにした */
	}
//	else //if (YUYUKO==(cg_ga me_select_player))
//	{
//		if (pd_bomber_time<=0)	{	src_shot->jyumyou = JYUMYOU_NASI;}/* 時間制限でおしまい */
//	}
	if ((ra_nd()&0xff) < cg_bomber_time)
	{
		if (0==((src_shot->PL_KEKKAI_DATA_radius256) & (0x07<<8) ))
		{
			//player_weapon_collision_check_kekkai(src_shot, src->PL_KEKKAI_DATA_radius);
...
static SHOT_FUNC(marisa_move_master_spark)
{
	OBJ *zzz_player;
	zzz_player = &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_00_PLAYER];
//	src_shot->center.x256 += src->math_vector.x256; /*fps_factor*/
//	src_shot->center.y256 += src->math_vector.y256; /*fps_factor*/
	src_shot->center.x256 = zzz_player->center.x256;			/*fps_factor*/
	src_shot->center.y256 = zzz_player->center.y256-t256(MASKER_SPARK_LENGTH144);	/*fps_factor*/
	{
		...
		/* 表示角度(下が0度で左回り(反時計回り)) */
	//	src_shot->tmp_angleCCW1024	+= maspa_kaiten[((src_shot->obj_type_set)& (0x03))];
		src_shot->tmp_angleCCW65536	+= (((u32)maspa_kaiten[((src_shot->obj_type_set)& (0x03))])<<6);/*(とりあえず)*/
	//	mask65536((src->tmp_angleCCW65536));
	//	{
	//		int sin_value_t256; 		//	sin_value_t256 = 0;
	//		int cos_value_t256; 		//	cos_value_t256 = 0;
	//		int256_sin cos65536(src_shot->tmp_angleCCW65536, &sin_value_t256, &cos_value_t256);
	//		src_shot->rotationCCW65536 = ((sin_value_t256)<<(6-2));
	//	}
		//------------------
		HATSUDAN_R11_speed256	= (t256(16));
		HATSUDAN_R13_angle65536 = (src_shot->tmp_angleCCW65536);
		CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
		src_shot->rotationCCW65536 = VR07;//sin_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
	//	src_shot->rotationCCW65536 = VR06;//cos_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
		//------------------
	}
	/* 時間でおしまい */
	if (0 >= cg_bomber_time)	{src_shot->jyumyou = JYUMYOU_NASI;}/* 時間制限でおしまい */
	else
	if (0x40 > cg_bomber_time)	/* 0x3f ... 0x01 */
	{
		src_shot->color32			= ((((cg_bomber_time))/*&(0x3f)*/)<<(24+2))|0x00ffffff; /* だんだん消えるようにした */
		/* (この値の範囲なら)実はマスク必要ない。 */
	}
	player_weapon_collision_check(src_shot, PLAYER_WEAPON_TYPE_01_BOMB_PLAYER);/* ボムで敵を倒すあたり判定 */
//	player_bomber_out_collision_check(src_shot);/* 画面外に出たらおしまい & ボムで敵を倒すあたり判定 */
}
...
static SHOT_FUNC(marisa_yuyuko_move_levarie_gyastry_dream)
{
	src_shot->center.x256 += src_shot->math_vector.x256;	/*fps_factor*/
	src_shot->center.y256 += src_shot->math_vector.y256;	/*fps_factor*/
	#if 1
	/* YUYUKO MARISA_A 以外 */
	if (
	//	(CIRNO_A==(cg_ga me_select_player)) || (CIRNO_Q==(cg_ga me_select_player)) ||
		((CIRNO_A-1)<(cg_game_select_player)) //||
	//	(MARISA_B==(cg_ga me_select_player))/* マスパ(もどき) */
	)
	#endif
	{	/*チルノ用(暫定的)*/	/*1.5*(d->speed);*/ 	/*fps_factor*/
		src_shot->center.y256 -= (abs((src_shot->math_vector.x256+(src_shot->math_vector.x256>>1) )));
		src_shot->center.y256 -= (abs((src_shot->math_vector.y256+(src_shot->math_vector.y256>>1) )));
		src_shot->center.y256 -= t256(1.0);
	}
	{

		/* 表示角度(下が0度で左回り(反時計回り)) */
		src_shot->rotationCCW65536	+= ((current.shot_status[SHOT_STATUS_20_BOMBER_KAITEN_SOKUDO])<<6);/*r42とりあえず*/
		mask65536((src_shot->rotationCCW65536));
		if (t256(4.0) > src_shot->m_zoom.x256)
		{
		//	src_shot->m_zoom.y256 += aaa_sss[(cg_ga me_select_player)+(PLAYERS8)];
		//	src_shot->m_zoom.x256 += aaa_sss[(cg_ga me_select_player)+(PLAYERS8)];
			src_shot->m_zoom.x256 += current.shot_status[SHOT_STATUS_21_BOMBER_KAKUDAI_SOKUDO];
			src_shot->m_zoom.y256 = src_shot->m_zoom.x256;
		}
	}
	player_bomber_out_collision_check(src_shot);
}
...
static SHOT_FUNC(remilia_move_burn_fire)
{
	#if 1
	// 加速
	// x1.10
//	src_shot->math_vector.x256 = ((src_shot->math_vector.x256 * t256(1.1))>>8);//co_s65536((data->angle65536))*p->speed;/*fps_factor*/
//	src_shot->math_vector.y256 = ((src_shot->math_vector.x256 * t256(1.1))>>8);//si_n65536((data->angle65536))*p->speed;/*fps_factor*/
	// x1.125
	src_shot->math_vector.x256 += ((src_shot->math_vector.x256)>>3);//co_s65536((data->angle65536))*p->speed;/*fps_factor*/
	src_shot->math_vector.y256 += ((src_shot->math_vector.y256)>>3);//si_n65536((data->angle65536))*p->speed;/*fps_factor*/
	// x1.0625
//	src_shot->math_vector.x256 += ((src_shot->math_vector.x256)>>4);//co_s65536((data->angle65536))*p->speed;/*fps_factor*/
//	src_shot->math_vector.y256 += ((src_shot->math_vector.y256)>>4);//si_n65536((data->angle65536))*p->speed;/*fps_factor*/
	/* 描画が重すぎるので加速性能、若干速く( x1.0625→ x1.125)する */
	//
	#endif
	src_shot->center.x256 += src_shot->math_vector.x256;	//	src_shot->math_vector.x256;//co_s65536((data->angle65536))*p->speed;/*fps_factor*/
	src_shot->center.y256 += src_shot->math_vector.y256;	//	src_shot->math_vector.y256;//si_n65536((data->angle65536))*p->speed;/*fps_factor*/
	//
	player_bomber_out_collision_check(src_shot);
}
...
static SHOT_FUNC(yuudou_idou)
{
	OBJ *target;	/* 目標 */
	target = src_shot->target_obj;
	/* 目標が画面内ならば目標に向かう */
	if (
		...
		(target->center.y256 < (t256(GAME_HEIGHT)) ))	/*Gu(中心座標)*/
	{
		...
		/* CCWの場合 */
			VR04	= (target->center.x256);		/*(狙い先)*/
			VR05	= (target->center.y256);		/*(狙い先)*/
			VR06	= (src_shot->center.x256);		/*(狙い元)*/
			VR07	= (src_shot->center.y256);		/*(狙い元)*/
			CPU_CALL(cpu_VR_angleCCW65536_src_nerai);
			ta1024 = HATSUDAN_R13_angle65536;
			ta1024 >>= (6);
			#if 1
			/* 遅れ誘導 */
			/* KETMのは考え方が複雑なので破棄、0度からの差分で考え180度を境に分岐すれば同等(1周は360度しかないので) */
		//	const int sabun_aaa = (src_shot->tmp_angleCCW1024-src_shot->rotationCCW1024);
			const int sabun_aaa1024 = (ta1024-((src_shot->rotationCCW65536)>>6));/*r42とりあえず*/
			if (0 > sabun_aaa1024/*(src_shot->tmp_angleCCW1024-src->rotationCCW1024)*/ )/* 0レジスタ+共通最適化 */
			{	/* 目標角度 < 制御角度 */
				if ((1024/2) > sabun_aaa1024/*(src->tmp_angleCCW1024-src->rotationCCW1024)*/)/* 共通最適化 */
						{	src_shot->rotationCCW65536 -= (10<<6); }	/* 180度以内なら近いほうから回る */
				#if 0	/* 省略して問題ない？ */
				else	{	src_shot->rotationCCW65536 += (10<<6); }	/* 180度以上なら反対周りのほうが近い */
				#endif
			}
			else	/* 目標角度 > 制御角度 */
			{
				if ((1024/2) > -(sabun_aaa1024)/*(src_shot->rotationCCW1024-src->tmp_angleCCW1024)*/)
						{	src_shot->rotationCCW65536 += (10<<6); }	/* 180度以内なら近いほうから回る */
				#if 0	/* 省略して問題ない？ */
				else	{	src_shot->rotationCCW65536 -= (10<<6); }	/* 180度以上なら反対周りのほうが近い */
				#endif
			...
			#else
			/* 直接誘導 */
			src_shot->rotationCCW65536/*src_shot->YUUDOU_BOMBER_angleCCW65536*/ = (ta1024<<6);/*r42とりあえず*/
			#endif
		#endif
		}
		mask65536(src_shot->rotationCCW65536);/*src_shot->YUUDOU_BOMBER_angleCCW65536*/
		#endif/*誘導(000)*/
	}
	else	/* 目標が画面外ならば新しい目標を探す。 */
	{
		/* Ziel wurde anderweitig vernichtet, neues Ziel suchen */
		src_shot->target_obj = &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_00_PLAYER];		/* 画面外に逃げられた */
	}
}
...
static SHOT_FUNC(marisa_move_parrent_hlaser)
{
	/* 他の誘導ボムが、既に倒したか？ */
	if (&obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_00_PLAYER] == src_shot->target_obj) /* 見つからない(他の誘導ボムが倒してしまった場合) */
	{
		/* ターゲット検索、敵を探す。 */
		src_shot->target_obj = obj_teki_search();
	}
	else	/* まだターゲットが生きてる */
	{
		if (HOMING_096_TIME_OUT < src_shot->jyumyou)
		{
			/* 一定時間減速 */
			src_shot->YUUDOU_BOMBER_speed256 -= t256(0.5);/*減速*/
			// 止まると困る
			src_shot->YUUDOU_BOMBER_speed256 = psp_max((src_shot->YUUDOU_BOMBER_speed256), t256(0.75)); /* 最小値 */
		}
		else
		{
			/* 一定時間を超えたら加速 */
			src_shot->YUUDOU_BOMBER_speed256 += t256(0.5);/*加速*/
			// 速過ぎるのも困る
			src_shot->YUUDOU_BOMBER_speed256 = psp_min((src_shot->YUUDOU_BOMBER_speed256), t256(16.0)); /* 最大値 */
		}
		//
		yuudou_idou(src_shot);
	}
	#if 0
	/* (多分必要ないけど)念の為マスク */
	mask65536(src_shot->rotationCCW65536);/*src->YUUDOU_BOMBER_angleCCW65536*/
	#endif
	#if 1
	/* 表示用 */
//	src_shot->rotationCCW65536	= src->rotationCCW65536/*src->YUUDOU_BOMBER_angleCCW65536*/;
	#endif
	/*src->YUUDOU_BOMBER_angleCCW65536*/
	#if (0)//
	src_shot->center.x256 += ((si n65536((src->rotationCCW65536))*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/		/* CCWの場合 */
	src_shot->center.y256 += ((co s65536((src->rotationCCW65536))*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/
	#else
//	{
//		int sin_value_t256; 	//	sin_value_t256 = 0;
//		int cos_value_t256; 	//	cos_value_t256 = 0;
//		int256_sin cos65536(src_shot->rotationCCW65536, &sin_value_t256, &cos_value_t256);
//		src_shot->center.x256 += ((sin_value_t256*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/
//		src_shot->center.y256 += ((cos_value_t256*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/
//	}
	//------------------
	HATSUDAN_R11_speed256	= (src_shot->YUUDOU_BOMBER_speed256);
	HATSUDAN_R13_angle65536 = (src_shot->rotationCCW65536);
	CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
	src_shot->center.x256 += VR07;//sin_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
	src_shot->center.y256 += VR06;//cos_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
	//------------------
	#endif
	player_weapon_collision_check(src_shot, PLAYER_WEAPON_TYPE_00_SHOT);
//
//	src_shot->jyumyou--;/*fps_factor*/
//	if (0 > src_shot->jyumyou ) /* 時間切れ？ */
//	{
//		src_shot->jyumyou = JYUMYOU_NASI;		/* 時間切れなのでおしまい */
//	}
}
...
static SHOT_FUNC(move_reimu_musou_fuuin)
{
	/* 他の誘導ボムが、既に倒したか？ */
	if (&obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_00_PLAYER] == src_shot->target_obj) /* 見つからない(他の誘導ボムが倒してしまった場合) */
	{
	...
	{
		if (t256(3.0) > src_shot->YUUDOU_BOMBER_speed256)
		{
			src_shot->YUUDOU_BOMBER_speed256 += /*(int)*/t256(0.5);/*加速*/
		}
		//
		yuudou_idou(src_shot);
	}
// ボムは最大 7 個(8)
//	if ((cg_bomber_time&0x0f)==(src_shot->YUUDOU_BOMBER_reimu_musou_id&0x0f))
	if ((cg_bomber_time&0x07)==(src_shot->YUUDOU_BOMBER_reimu_musou_id&0x07))
	{
		/*src->YUUDOU_BOMBER_angleCCW1024*/
		#if (0)//
		src_shot->math_vector.x256 = ((si n65536((src->rotationCCW65536))*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/		/* CCWの場合 */
		src_shot->math_vector.y256 = ((co s65536((src->rotationCCW65536))*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/
		#else
	//	{
	//		int sin_value_t256; 	//	sin_value_t256 = 0;
	//		int cos_value_t256; 	//	cos_value_t256 = 0;
	//		int256_sin cos65536(src_shot->rotationCCW65536, &sin_value_t256, &cos_value_t256);
	//		src_shot->math_vector.x256 = ((sin_value_t256*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/
	//		src_shot->math_vector.y256 = ((cos_value_t256*(src_shot->YUUDOU_BOMBER_speed256))>>8);/*fps_factor*/
	//	}
		//------------------
		HATSUDAN_R11_speed256	= (src_shot->YUUDOU_BOMBER_speed256);
		HATSUDAN_R13_angle65536 = (src_shot->rotationCCW65536);
		CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
		src_shot->math_vector.x256 = VR07;//sin_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
		src_shot->math_vector.y256 = VR06;//cos_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
		//------------------
		#endif
	}
		src_shot->center.x256 += (src_shot->math_vector.x256);/*fps_factor*/
		src_shot->center.y256 += (src_shot->math_vector.y256);/*fps_factor*/
	player_weapon_collision_check(src_shot, PLAYER_WEAPON_TYPE_01_BOMB_PLAYER/*PLAYER_WEAPON_TYPE_00_SHOT*/);
//
	if (0 >= cg_bomber_time)	/* 時間切れの場合 */
	{
		gamen_gai_nara_zako_osimai(src_shot);/* 画面外ならおしまい */
		src_shot->kougeki_ti--;/* 時間切れの場合、ボンバー(お札)の効力(威力)を減らす */
	}
//
	if (0 > src_shot->kougeki_ti)	{src_shot->jyumyou = JYUMYOU_NASI;	}	/* ボンバー(お札)の効力(威力)切れでおしまい */
}
/*---------------------------------------------------------
	プレイヤー、誘導弾の移動(子供)
---------------------------------------------------------*/

static SHOT_FUNC(marisa_move_kodomo_hlaser)
{
	#if 1
	OBJ *oya;
	oya = src_shot->target_obj;
	if (
	...
	{
		src_shot->jyumyou = JYUMYOU_NASI;/*おしまい*/
	}
//
	#if 1
	/* 表示用 */
	src_shot->rotationCCW65536	= oya->rotationCCW65536;/*src->PL_HOMING_KODOMO_DATA_angleCCW1024*/
	#endif
	src_shot->center.x256 = oya->center.x256;
	src_shot->center.y256 = oya->center.y256;
//	src_shot->PL_HOMING_KODOMO_DATA_color256 = ((oya->PL_HOMING_KODOMO_DATA_color256*t256(0.96))>>8)&0xff;
//	src_shot->PL_HOMING_KODOMO_DATA_color256 = ((oya->PL_HOMING_KODOMO_DATA_color256*t256(0.90))>>8)&0xff;
//	src_shot->PL_HOMING_KODOMO_DATA_color256 = ((oya->PL_HOMING_KODOMO_DATA_color256 - 0x10)&0xff);
//	src_shot->color32 = (src->PL_HOMING_KODOMO_DATA_color256<<24)|(0x00ffffff);
	#if (1)/*(?)*/
	/* 数フレーム動かない場合は、自動消去する */
	if ((src_shot->PL_HOMING_KODOMO_DATA_check_x256 != src_shot->center.x256) ||
		(src_shot->PL_HOMING_KODOMO_DATA_check_y256 != src_shot->center.y256))
	{
		src_shot->PL_HOMING_KODOMO_DATA_time_out = 8;/* 動いたら自動消去しない */
	}
	src_shot->PL_HOMING_KODOMO_DATA_time_out--;
	if (0 > src_shot->PL_HOMING_KODOMO_DATA_time_out)
	{
		src_shot->jyumyou = JYUMYOU_NASI;/*おしまい*/
	}
	#endif
	src_shot->PL_HOMING_KODOMO_DATA_check_x256 = src_shot->center.x256;
	src_shot->PL_HOMING_KODOMO_DATA_check_y256 = src_shot->center.y256;
	#endif
}
...
add1-------------------------------------------------------------------------
*/
static SHOT_FUNC(set_bomb_str_STD)
{
	src_shot->m_Hit256R 	= ((current.shot_status[SHOT_STATUS_1E_STANDARD_BOMB_ATARI_HANKEI])<<8);
	src_shot->kougeki_ti	= current.shot_status[SHOT_STATUS_16_STANDARD_BOMB_STRENGTH];
	/* 攻撃値: ==ボンバーの威力。 */
}
static SHOT_FUNC(set_bomb_str_LOW)
{
	src_shot->m_Hit256R 	= ((current.shot_status[SHOT_STATUS_1F_LOWER_BOMB_ATARI_HANKEI])<<8);
	src_shot->kougeki_ti	= current.shot_status[SHOT_STATUS_17_LOWER_BOMB_STRENGTH];
	/* 攻撃値: ==ボンバーの威力。 */
}
/* (幽々子低速)十字炎ボムの炎の部分 */

static TUIKA_FUNC(yuyuko_add_meifu)
{
		/* (24+(1))  、ここでの cg_bomber_time は、最大0x7fなので。 */
		src_tuika->color32		= (cg_bomber_time<<(24+(1)))|0x00ffffff;
		src_tuika->m_zoom.x256	+= (10);
		src_tuika->m_zoom.y256	+= (10);
//	int j;
//	for (j=0; j<(1); j++)
	{
		OBJ *h;
		...
		u32 my_rand;
			my_rand = ra_nd();
			{
				h->center.x256			= (src_tuika->center.x256) - t256(16) + (((my_rand<<8)&0x1fff));//+ ((0==j)?(-t256(32)):t256(32));
				h->center.y256			= (src_tuika->center.y256) - t256(16) + (((my_rand	 )&0x1fff));// (u16)((my_rand>>16)) + t256((GAME_HEIGHT-256)/2);
			}
		...
		int ang_aaa_1024;
			...
			//------------------
			HATSUDAN_R11_speed256	= ((rnd_spd256));/*(t256形式)*/
			HATSUDAN_R13_angle65536 = deg1024to65536(ang_aaa_1024);
			CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
			h->math_vector.x256 = VR07;//sin_value_t256 // 縦
			h->math_vector.y256 = VR06;//cos_value_t256 // 横
			}
		}
	}
}
...
static TUIKA_FUNC(remilia_add_burn_shot)
{
	{
		OBJ *h;
		h = obj_regist_teki();
...
//			h->color32			= MAKE32RGBA(0xff, 0xff, 0xff, 0x64);		/*	h->alpha			= 0x64 100;*/
			...
			#if 1/*Gu(中心座標)*/
			h->center.x256			= src_tuika->center.x256;
			h->center.y256			= src_tuika->center.y256+t256(15);
			#endif
			{
			#if 1
			/* 描画用角度(下が0度で左回り(反時計回り)) */
//			h->rotationCCW1024		= ang_aaa_1024;
			h->rotationCCW65536		= (0);
			#endif
			h->math_vector.x256 	= (src_tuika->math_vector.x256);/*fps_factor*/ /* CCWの場合 */
			h->math_vector.y256 	= (src_tuika->math_vector.y256);/*fps_factor*/
			u32 rnd_spd 		= (ra_nd() ) ;
...
static PLAYER_FUNC(reimu_add_reifu_musofuuin)
{
	{
		static int musou_id;
		musou_id++;
	...
	//			h->obj_type_set 			= (OBJ_Z01_JIKI_GET_ITEM|OBJ_Z80_SHOT_SPECIAL)/*ボスに有効*/;
				h->obj_type_set 			= (/*OBJ_Z01_JIKI_GET_ITEM*/JIKI_BOMBER_01|OBJ_Z80_SHOT_SPECIAL);/*ボスに有効*/
				#if 1/*Gu(中心座標)*/
			//	h->center.x256			= src_player->center.x256 - t256(5);
			//	h->center.y256			= src_player->center.y256 + t256(15);
				h->center.x256			= src_player->center.x256;
				h->center.y256			= src_player->center.y256;
				#endif
	//			if (0==i)
				{
	...
	//				h->rotationCCW1024	= (i<<6);
					/* 180 ... 0 (1024/128==8) */
				//	h->rotationCCW1024	= ( (((int)cg_bomber_time)<<3)+(512));/* 射出する方向 */
					h->rotationCCW65536	= ( (((int)cg_bomber_time)<<(3+6))+(512<<6));/* 射出する方向 */
					#if 0
					/* (多分必要ないけど)念の為マスク */
					mask65536(src->rotationCCW65536);/*src->YUUDOU_BOMBER_angleCCW1024*/
					#endif
...
static PLAYER_FUNC(marisa_add_teisoku_yuudoudan)
{
...
//					h->rotationCCW1024						= (j&1)?(cv1024r(  0+10)):(cv1024r(360-10));
					h->rotationCCW65536						= (j&1)?(cv65536r(  0+10)):(cv65536r(360-10));
...
static TUIKA_FUNC(bomber_hontai_off_check)
{
	if (0 >= cg_bomber_time)
	{
		src_tuika->callback_mover = NULL;/*(off!)*/
	}
/* 時間制限でおしまい */	//	if (0 >= cg_bomber_time)	{ src_tuika->jyumyou = JYUMYOU_NASI;}
}
...
static TUIKA_FUNC(bomber_hontai_reimu_tuika_musofuuin_shot)
{
	/* シナリオ時には誘導弾を追加しない */
	if (IS_SCENE_KAIWA_MODE)	{return;}
	bomber_hontai_off_check(src_tuika);
	//
	/*(r36テスト、32 未満は追加しない)*/
	if (32 > cg_bomber_time)	{return;}
	//
	OBJ *zzz_player;
	zzz_player = &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_00_PLAYER];
	//
...
//	if (0x0f==(((int)pd_bomber_time)&0x0f))
	if (0x0f==((src_tuika->jyumyou)&0x0f))/* 波動を撃つタイミング */
	{
		reimu_add_reifu_musofuuin(zzz_player);/* 波動発動(早口言葉?) */
	}
	#if 1/*Gu(中心座標)*/
	src_tuika->center.x256 = zzz_player->center.x256;
	src_tuika->center.y256 = zzz_player->center.y256;
	#endif
}
...
---------------------------------------------------------*/
static TUIKA_FUNC(bomber_hontai_marisa_tuika_bomber_teisoku_yuudoudan)
{
	/* シナリオ時には誘導弾を追加しない */
	if (IS_SCENE_KAIWA_MODE)	{return;}
	bomber_hontai_off_check(src_tuika);
...
//	if (0==(((int)pd_bomber_time)&0x0f))/*(r31)*/
	if (0==((src_tuika->jyumyou) & 0x07))/*(r32)追加量を2倍にしてみる。*/
	{
		/* シナリオ時には誘導弾を追加しない */
	//	if (0==(cg_state_flag & STATE_FLAG_0x0200_IS_KAIWA_MODE))
		{
...
static TUIKA_FUNC(bomber_hontai_remilia_tuika_cross_shot)
{
	/* シナリオ時には誘導弾を追加しない */
	if (IS_SCENE_KAIWA_MODE)	{return;}
	bomber_hontai_off_check(src_tuika);
...
//	if (0==(cg_state_flag & STATE_FLAG_0x0200_IS_KAIWA_MODE))
	{
		remilia_add_burn_shot(src_tuika);/* 波動発動(早口言葉?) */
	}
	#if 1/*Gu(中心座標)*/
	/* 親移動 */
	src_tuika->center.x256 += src_tuika->math_vector.x256;
	src_tuika->center.y256 += src_tuika->math_vector.y256;
	#endif
}
...
static TUIKA_FUNC(bomber_hontai_yuyuko_tuika_meifu)
{
	/* シナリオ時には誘導弾を追加しない */
	if (IS_SCENE_KAIWA_MODE)	{return;}
	bomber_hontai_off_check(src_tuika);
	//
	yuyuko_add_meifu(src_tuika);/* 波動発動(早口言葉?) */
	#if 1/*Gu(中心座標)*/
//	src_tuika->center.x256 = zzz_player->center.x256;
//	src_tuika->center.y256 = zzz_player->center.y256;
	#endif
...
/*static*/extern PLAYER_FUNC(reimu_yuyuko_create_bomber_kekkai_parrent) 	/* 霊夢 */		//シールドの追加
{
	int ii;//	int i;	/* 半象限ずつ */
	int jj;
	ii = 0;
//	for (ii=0; ii<(8); ii++)//	for (i=0; i<360; i+=45)
	...
	//	if (NULL!=h)/* 登録できた場合のみ */
		if (NULL==h)return;/* 登録できた場合のみ */
		{
			...
			if (YUYUKO==(cg_game_select_player))
			{
				if (0 == (ii&1))// if (i%90 == 0)
				{
					h->PL_KEKKAI_DATA_add_r65536 	= (( 4)<<6); /* 時計回り */
					h->PL_KEKKAI_DATA_radius256		= t256(16); 	/*(48)*/	//d->radius=38; 	/* 少しずつ広がる半径初期値 */
				}
				else
				{
					h->PL_KEKKAI_DATA_add_r65536 	= ((-3)<<6); /* 反時計回り */
					h->PL_KEKKAI_DATA_radius256		= t256(16); 	/*(45)*/	//d->radius=35; 	/* 少しずつ広がる半径初期値 */
				}
			}
			else
			#endif
			{
				if (0 == (ii&1))// if (i%90 == 0)
				{
					h->PL_KEKKAI_DATA_add_r65536 	= (( 46)<<6);	/* 時計回り */
					h->PL_KEKKAI_DATA_radius256		= t256(16); 	/*(48)*/	//d->radius=38; 	/* 少しずつ広がる半径初期値 */
				}
				else
				{
					h->PL_KEKKAI_DATA_add_r65536 	= ((-34)<<6);	/* 反時計回り */
					h->PL_KEKKAI_DATA_radius256		= t256(0);		/*(45)*/	//d->radius=35; 	/* 少しずつ広がる半径初期値 */
				}
			}
			/* 霊夢 ＆ 幽々子 */
		...
		//	h->PL_KEKKAI_DATA_angleCCW1024		= (jj);// (ii<<7);	/* (1024/8) */
			h->PL_KEKKAI_DATA_angleCCW65536		= ((jj)<<6);// (ii<<7);	/* (1024/8) */
			/* 出始めはプレイヤー座標 */
		//	{
		//		OBJ *zzz_player;
		//		zzz_player = &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_00_PLAYER];
		...
		//		h->center.y256					= zzz_player->center.y256;
				h->center.x256					= src_player->center.x256; /* 発弾時のプレイヤーの座標を引き継ぐ。 */
				h->center.y256					= src_player->center.y256; /* 発弾時のプレイヤーの座標を引き継ぐ。 */
		//	}
		}
		/* 霊夢Aは	 8 == 4弾 x 2種類 */
		/* 霊夢Bは	16 == 8弾 x 2種類 */
		/* 幽々子は 16 == 8弾 x 2種類 */
	//	jj += ((YUYUKO==(cg_ga me_select_player))?(1024/16):(1024/8));
	...
	#if (1)/*(幽々子用)*/
	{
		...
		s1->center.x256 		= (src_player->center.x256); /* 発弾時のプレイヤーの座標を引き継ぐ。 */
		s1->center.y256 		= (src_player->center.y256); /* 発弾時のプレイヤーの座標を引き継ぐ。 */
	}
	#endif
}
...
//	/*static*/extern SHOT_FUNC(yuyuko_create_bomber_gyastry_dream_parrent)		/* 霊夢 */		//シールドの追加
//{
...
/*static*/extern PLAYER_FUNC(marisa_create_bomber_master_spark_parrent)
{
	int iii;
	...
	//	if (NULL!=h)/* 登録できた場合のみ */
		...
		{	set_bomb_str_STD(h);
			...
			#if (1)/* 省略可能かも？ */
			h->center.x256					= src_player->center.x256;
			h->center.y256					= src_player->center.y256-t256(MASKER_SPARK_LENGTH144);
			#endif
//			/* 描画用角度(下が0度で左回り(反時計回り)) */
//			h->rotationCCW65536		= (0);
		}
...
/*static*/extern PLAYER_FUNC(chrno_q_create_bomber_gyastry_dream_parrent)
{
	int jjj;		jjj=0;
	int angCCW65536; angCCW65536 = (0);
	int iii;
	...
	//
			...
			#else
		//	if (YUYUKO!=(cg_ga me_select_player))
			{	/*(チルノ用)*/
			...
			#endif
			#if (0)//
			h->math_vector.x256 	= ((si n65536((angCCW65536))*(d_speed256))>>8);/*fps_factor*/ 	/* CCWの場合 */
			h->math_vector.y256 	= ((co s65536((angCCW65536))*(d_speed256))>>8);/*fps_factor*/
			#else
			//------------------
			HATSUDAN_R11_speed256	= (d_speed256);
			HATSUDAN_R13_angle65536 = (angCCW65536);
			CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
			h->math_vector.x256 = VR07;//sin_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
			h->math_vector.y256 = VR06;//cos_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
			//------------------
			...
			#if 1
			/* 描画用角度(下が0度で左回り(反時計回り)) */
			h->rotationCCW65536		= angCCW65536;
			#endif
		//	angCCW65536 += (int)((65536)/16); // 22.5度/360, 2xπ/16 /* 0.392699081698724154810 */
			angCCW65536 += (int)((65536)/18); // 20.0度/360, 2xπ/18 /* 0.349065850398865915384 */
	//
			#if 1/*Gu(中心座標)*/
			h->center.x256			= src_player->center.x256;
			h->center.y256			= src_player->center.y256;
...
//			if (YUYUKO==(cg_ga me_select_player))	/* 幽々子 */
//			{
...
/*static*/extern PLAYER_FUNC(marisa_create_bomber_star_dust_levarie_parrent)
{
	int angCCW65536; angCCW65536 = (0);
	int mmm;
	mmm = (0x100);
	...
	for (kkk=0; kkk<(3); kkk++)/*3サイズ*/
	{
		...
		//	if (NULL!=h)/* 登録できた場合のみ */
			if (NULL==h)return;/* 登録できた場合のみ */ 	/*うーん？？？*/
			{
				...
				//		int sin_value_t256; 				//	sin_value_t256 = 0;
				//		int cos_value_t256; 				//	cos_value_t256 = 0;
				//		int256_sin cos65536((angCCW65536), &sin_value_t256, &cos_value_t256);
				...
				//	}
					...
					/* 描画用角度(下が0度で左回り(反時計回り)) */
					h->rotationCCW65536		= angCCW65536;
					angCCW65536 += (int)((65536)/8);	// 45.0度/360, 64
					#endif
				#if 1/*Gu(中心座標)*/
				{
					h->center.x256			= src_player->center.x256;
...
extern void jiki_shot_oya_hide(void)
{
	OBJ *h;
...
/*static*/extern SHOT_FUNC(remilia_create_bomber_cross_shot_parrent)
{
	OBJ *h;
	h						= &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_02_BOMBER_HONTAI];
	{
		...
		#endif
		{
			OBJ *obj_boss;
			obj_boss			= &obj99[OBJ_HEAD_02_0x0900_TEKI_FIX+TEKI_OBJ_00_BOSS00_HONTAI];/*(ボス本体)*/
			VR04	= (obj_boss->center.x256);			/*(狙い先)*/
			VR05	= (obj_boss->center.y256);			/*(狙い先)*/
			VR06	= (zzz_player->center.x256);		/*(狙い元)*/
			VR07	= (zzz_player->center.y256);		/*(狙い元)*/
			CPU_CALL(cpu_VR_angleCCW65536_src_nerai);
	//(同じ)VR13 = HATSUDAN_R13_angle65536;
			#if (0)//
			int_aaa = si n65536((VR13));	h->math_vector.x256  = int_aaa+int_aaa;/*fps_factor*/	/* CCWの場合 */
			int_aaa = co s65536((VR13));	h->math_vector.y256  = int_aaa+int_aaa;/*fps_factor*/
			#else
		//	{
		//		int sin_value_t256; 		//	sin_value_t256 = 0;
		//		int cos_value_t256; 		//	cos_value_t256 = 0;
		...
		//	}
			//------------------
			HATSUDAN_R11_speed256	= (t256(2));/*t256(2)*/
	//(同じ)HATSUDAN_R13_angle65536 = (VR13);
			CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
			h->math_vector.x256 = VR07;//sin_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
			h->math_vector.y256 = VR06;//cos_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
			...
			VR13 += cv65536r((180));/*(省略可)mask65536(VR13);*/
			h->rotationCCW65536 	= VR13;/* 下が正面の角度が描画用角度 */
			#endif
		}
	}
...
/*---------------------------------------------------------
	魔理沙、チルノ等、低速ボムの親
---------------------------------------------------------*/
/*static*/extern SHOT_FUNC(marisa_create_bomber_homing_parrent)
{
	OBJ *h;
	h						= &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_02_BOMBER_HONTAI];
	{
...
/*---------------------------------------------------------
	霊夢 A (通常ボム) / チルノ A(低速ボム) の親
---------------------------------------------------------*/
/*static*/extern SHOT_FUNC(reimu_create_bomber_homing_shot_parrent) /* reimu_move_add_bomber_hlaser */
{
	OBJ *h;
	h						= &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_02_BOMBER_HONTAI];
	{
		...
		#endif
		{
			#if 1
			/* 描画用グラ回転(ショットは上が正面なので、回転角を計算)[180/360度]回す。 */
			h->rotationCCW65536 	= 0;/* 下が正面の角度が描画用角度 */
			#endif
		}
	}
	#if 0
...
---------------------------------------------------------*/
/*static*/extern PLAYER_FUNC(yuyuko_create_bomber_meifu_parrent)
{
	OBJ *h;
	h						= &obj99[OBJ_HEAD_03_0x0a00_KOTEI+FIX_OBJ_02_BOMBER_HONTAI];
	{
		h->center.x256		= (src_player->center.x256); /* 発弾時プレイヤー座標を引き継ぐ */
		h->center.y256		= (src_player->center.y256); /* 発弾時プレイヤー座標を引き継ぐ */
...
//		h->obj_type_set 	= (/*表示しない*/SP_DUMMY_MUTEKI); /* ボスの直接攻撃は禁止 */
		...
		#if 1/*Gu(中心座標)*/
//		h->center.x256			= t256(GAME_NOT_VISIBLE480);// (src->center.x256); /* 非表示 */
//		h->center.y256			= (src_player->center.y256); /* 非表示 */
		#endif
	}
}
...
/*---------------------------------------------------------
	プレイヤー。 ボムの寿命を設定
--------------------------------------------------------- */
extern void jiki_shot_set_bomber_time(void)
{
	//	#define PD_BOMBER_JYUMYOU180					(180)		/* ボムの寿命==180[frame] */
	//	cg_bomber_time		= (PD_BOMBER_JYUMYOU180);//cg_bomber_time_table[(index_aaa)];
	cg_bomber_time		= (current.shot_status[SHOT_STATUS_24_PLAYER_SET_BOMBER_TIME]);//cg_bomber_time_table[(index_aaa)];
}
/*---------------------------------------------------------
...
/*---------------------------------------------------------
	プレイヤーのアニメーション速度を設定
--------------------------------------------------------- */
extern int jiki_shot_get_player_animation_speed(void)
{
...
/*---------------------------------------------------------
	喰らいボムの入力受け付け時間を設定
---------------------------------------------------------*/
extern int jiki_shot_get_kurai_bombtimer(void)
{
	...
	SHOT_STATUS aaa_fix[(PLAYERS8)] =
	{
		...
		{	/* 幽々子 /  / 扇(赤弾) */
			...
			//	128, 129, 130, 132, 136, 144,		  4,  4,
				//----------------------------(-r40)
			//	 80,  96, 112, 128, 132, 144,		  4,  4,//(-r40)
			//	 64,  64,  64,	65,  65,  66,		  8,  8,//(-r40)
			//	 17,  16,  15,	14,  13,  13,		 48, 127,//(-r40)	// 40(r35u1, 40==8*5)	 48, 160,
			//	 31,  28,  25,	22,  19,  16,		  4, 16,//(-r40)
				//----------------------------(r41-)
			//	144, 132, 128, 112,  96,  50,		  4,  4,//(r41-)
			//	 69,  68,  67,	70,  65,  55,		  8,  8,//(r41-)
			//	 24,  16,  13,	 9,   6,   3,		 48, 127,//(r41-)	// 40(r35u1, 40==8*5)	 48, 160,
			//	 33,  24,  12,	10,   7,   2,		  4, 16,//(r41-)
				//----------------------------(r41-)
				144, 132, 128, 112,  96,  80,		  4,  4,//(r41-)
				 69,  68,  67,	64,  60,  55,		  8,  8,//(r41-)
				 24,  16,  14,	 9,   6,   5,		 48, 127,//(r41-)	// 40(r35u1, 40==8*5)	 48, 160,
				 33,  24,  13,	10,   7,   4,		  4, 16,//(r41-)
				//----------------------------
				  0,   0,	4,	48, (255),	 4, 	  1, 12,
			}
/*
----------------------------ショット
 6.000000000000000 == 144/24
 8.25			   == 132/16
 9.846153846153846 == 128/13
12.444444444444444 == 112/ 9
16				   ==  96/ 6
16.666666666666666 ==  50/ 3
----------------------------サブ
 2.090909090909090 ==  69/33
 2.833333333333333 ==  68/24
 5.583333333333333 ==  67/12
 7				   ==  70/10
 9.285714285714285 ==  65/ 7
27.5			   ==  55/ 2
----------------------------
*/
		},
...
//	/* 低速ボム */			cg_bomber_time = 200/* 320 */ /* 400 */ /* 800 */;		/* ボムの有効時間(設定無敵時間)[※１仕様が変わったので半分にした] */
//	/* 霊夢 */				cg_bomber_time = 180/* 200 */;							/* ボムの有効時間(設定無敵時間) */	/* こちらはボス有効(攻撃型) */
//	/* 魔理沙	チルノ */	cg_bomber_time = 100/* 32 */ /* 100 */; 				/* ボムの有効時間(設定無敵時間) */
//	/* 扇 有効時間 */		cg_bomber_time = 255/* 255==((16*4*2*2)-1) */;			/* ボムの有効時間(設定無敵時間) */
//	レミリアボム			cg_bomber_time = 0x7e	/* 255==0xff==8発	254==0xfe==7発==0xbf=192 4発==0x7f 150==0x96 */;
	static const u8 cg_bomber_time_table[/* 16 */(PLAYERS8*2)] =
	{
