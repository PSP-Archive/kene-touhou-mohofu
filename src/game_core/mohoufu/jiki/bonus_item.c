...
#define ITEM_DATA_02_flags00		user_data03 	/* ( ==TR00_hatudan_spec_data)収集フラグ(TR00_hatudan_spec_dataと合わせる必要がある) */

#define ITEM_DATA_04_y_sum256		user_data02 				/* アイテム投げ出し用 y軸 積算値(y軸、上方がマイナス) */
#define ITEM_DATA_04_count256		ITEM_DATA_04_y_sum256		/* 自動収拾用カウンタ(共用) */
	/* 出現時間 */ /*	自動収集 */
//} ITEM_DATA;
#define ITEM_DATA_07_true_y256		user_data06 	/* 本来のy座標 */
...
//		if (t256(FIX_LABEL_ITEM_HEIGHT_DIV2) >= src->center.y256)	/* 中心座標なので */
		...
		//	toumeido = ( (toumeido));
			...
			toumeido >>= 7; 	/*(>>8だと面白くない)*/
			#if 0/*(デバッグ)*/
			cg_game_score = toumeido;/*[ スコア欄にデバッグ値表示 ]*/
			#endif
...
static OBJ_CALL_FUNC(move_item_type02)	/* 自動収集ならば、自分に集まる */
{
	...
	//	}
		...
		//#define ITEM_DATA_05_x_sa256		user_data04 	/* 差分 x */ /* 自動収集 */
		//#define ITEM_DATA_06_y_sa256		user_data05 	/* 差分 y */ /* 自動収集 */
		...
		/* 自機狙い角を HATSUDAN_R13_angle65536 に作成 */
		VR06	= (src->center.x256);
		VR07	= (src->ITEM_DATA_07_true_y256);
		CPU_CALL(calculate_jiki_nerai);
		/* 「1周が65536分割」から「1周が1024分割」へ変換する。 */
	//	HATSUDAN_R13_angle1024 = (deg65536to1024(HATSUDAN_R13_angle65536));
		/* 自分に集まる */
		int aaa_x256;
		int aaa_y256;
		...
		if ((SP_ITEM_06_HOSI) == (src->obj_type_set))	/* 星点のみ特別処理 */
		{
			if (
				...
				(
					...
					(t256((12)/*(8)*/) > abs(aaa_x256)) &&
					(t256((12)/*(8)*/) > abs(aaa_y256))
					/*(-r40: (8))*/
					/*(r41-: (12) 8だと回収にかなり時間がかかる場合があるので、少し回収判定領域を大きくする。)*/
					#else
...
//		src->ITEM_DATA_07_true_y256 = zzz_player->center.y256 + (aaa_y256); /*fps_factor*/
			#if (0)//
			src->math_vector.x256 = ((si n1024((deg65536to1024(HATSUDAN_R13_angle65536)))*(src->speed256))>>8);/*fps_factor*/	/* CCWの場合 */
			src->math_vector.y256 = ((co s1024((deg65536to1024(HATSUDAN_R13_angle65536)))*(src->speed256))>>8);/*fps_factor*/
			#else
		//	{
		//		int sin_value_t256; 	//	sin_value_t256 = 0;
		//		int cos_value_t256; 	//	cos_value_t256 = 0;
		//		int256_sin cos65536((HATSUDAN_R13_angle65536), &sin_value_t256, &cos_value_t256); /* 「1周が65536分割」 */
		//		src->center.x256				+= ((sin_value_t256*(/*t256*/(5)))/*>>8*/);/*fps_factor*/
		//		src->ITEM_DATA_07_true_y256 	+= ((cos_value_t256*(/*t256*/(5)))/*>>8*/);/*fps_factor*/
		//	}
			//------------------
			HATSUDAN_R11_speed256	= (t256(5));
		//	HATSUDAN_R13_angle65536 = (HATSUDAN_R13_angle65536);
			...
			src->ITEM_DATA_07_true_y256 	+= VR06;//cos_value_t256 // 下CCWの場合(右CWの場合ととxyが逆)
			//------------------
			#endif
	}
...
{
	{
		#if (1)
		{
			...
			obj->jyumyou	= JYUMYOU_1MIN10S;		/* 1分したら勝手に自動消去。 */
			obj->m_zoom.x256			= t256(1.00);	/* 表示拡大率 256 == [ x 1.00 ] */
			obj->m_zoom.y256			= t256(1.00);	/* 表示拡大率 256 == [ x 1.00 ] */
			#if (1)
			/*(レイヤー[0]は(r36以降)必ずstandard_angle_mover_00_callbackなのでレイヤー[0]に強制設定。)*/
			/*(良くわかんない)*/
			obj->TR00_hatudan_spec_data = (DANMAKU_LAYER_00)|(TAMA_SPEC_8000_NON_TILT);/* (r33-)非傾き弾 */
			#endif
	...
	//	if (SP_ITEM_06_HOSI == sel_type)	/* 星点のみ特別処理 */
		{
			/* 紅は、こうらしい */
			if (USER_BOMOUT_WAIT > cg_bomber_time)
			...
			{	/* ボム後の実質無敵期間中はこちら(稼げる) */
				/* ((graze/3)*10)+(500) pts */
				bonus_info_any_score_nodel(obj, (score(500)+(((cg_graze_point*86)>>8)/*(pd->graze_point/3)*/)) );/*自動消去へ仕様変更c->jyumyou = JYUMYOU_NASI;*/
			}
//			/*当たり判定なし*/	/*(?)*/
//			obj->atari_hantei			= (ATARI_HANTEI_OFF/*スコア兼用*/);
		...
		obj->rotationCCW65536				= (0);			/* 描画用角度(下が0度で左回り(反時計回り)) */	/* 0 == 傾かない。下が0度 */
		{
			/* エフェクトの場合半透明なので、白に戻す。 */
...
(SP_ITEM_07_SPECIAL&0xff),	(SP_ITEM_07_SPECIAL&0xff),	(SP_ITEM_07_SPECIAL&0xff),	(SP_ITEM_07_SPECIAL&0xff),	(SP_ITEM_07_SPECIAL&0xff),	(SP_ITEM_07_SPECIAL&0xff),	(SP_ITEM_07_SPECIAL&0xff),	(SP_ITEM_07_SPECIAL&0xff),
	...
	if (NULL != h)/* 登録できた場合 */
	{
		h->obj_type_set 		= set_sel_type; 	/* アイテムの種類 == グラフィックの種類 */
		h->m_zoom.y256			= M_ZOOM_Y256_NO_TILT;/* 特殊機能で傾かないようシステム拡張(r33)。 */
		reflect_sprite_spec(h, OBJ_BANK_SIZE_01_ITEM);
	...
	//
		if (SP_ITEM_06_HOSI == set_sel_type)	/* 星点のみ特別処理 */
		{
			...
			{	/* ボム後の実質無敵期間中はこちら(稼げる) */
				/* ((graze/3)*10)+(500) pts */
				bonus_info_any_score_nodel(h, (score(500)+(((cg_graze_point*86)>>8)/*(pd->graze_point/3)*/)) );/*自動消去へ仕様変更c->jyumyou = JYUMYOU_NASI;*/
			}
//			/*当たり判定なし*/	/*(?)*/
	...
	//	else	{;}
		//
		{
			...
			s32 rnd_spd256				= (my_rand & 0x7f/*angCCW512*/) + t256(0.75);
				...
				HATSUDAN_R13_angle65536 = (my_rand & ((65536/4)-1)) + (65536/4) + (65536/8);
				CPU_CALL(sincos256);/*(破壊レジスタ多いので注意)*/
...
extern void item_create_flags_num(
	OBJ *src,
	int item_type,
	int num_of_creates,
	int set_item_flags
...
---------------------------------------------------------*/
	...
	#if 0/*(旧メモ)*/
	/*mode 1: 撃破中 */
		PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7), (SP_ITEM_00_P001&7),	(SP_ITEM_00_P001&7),	(SP_ITEM_00_P001&7)),
		PACK_NAN_IDO_CONST((SP_ITEM_00_P001&7), (SP_ITEM_00_P001&7),	(SP_ITEM_00_P001&7),	(SP_ITEM_01_P008&7)),
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage1*/
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage2*/
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage3*/
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage4*/
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage5*/
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage6*/
	/*mode 0: 撃破後 */ 	/* easy, normal, hard, lunatic */
		PACK_NAN_IDO_CONST((SP_ITEM_03_1UP&7),	(SP_ITEM_03_1UP&7), 	(SP_ITEM_04_BOMB&7),	(SP_ITEM_04_BOMB&7)),	/* (もしstage7まで作るとしたらstage7クリアー後(クリアー後なのでゲーム的には意味無い)もここを共用する) */
		PACK_NAN_IDO_CONST((SP_ITEM_03_1UP&7),	(SP_ITEM_04_BOMB&7),	(SP_ITEM_04_BOMB&7),	(SP_ITEM_01_P008&7)),
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage1*/	/*alice*/
		PACK_NAN_IDO_CONST((SP_ITEM_00_P001&7), (SP_ITEM_04_BOMB&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage2*/	/*mima*/
		PACK_NAN_IDO_CONST((SP_ITEM_00_P001&7), (SP_ITEM_01_P008&7),	(SP_ITEM_04_BOMB&7),	(SP_ITEM_01_P008&7)),	/*stage3*/	/*kaguya*/
		PACK_NAN_IDO_CONST((SP_ITEM_00_P001&7), (SP_ITEM_04_BOMB&7),	(SP_ITEM_04_BOMB&7),	(SP_ITEM_01_P008&7)),	/*stage4*/	/*aya*/
		PACK_NAN_IDO_CONST((SP_ITEM_00_P001&7), (SP_ITEM_04_BOMB&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage5*/	/*pache*/
		PACK_NAN_IDO_CONST((SP_ITEM_00_P001&7), (SP_ITEM_01_P008&7),	(SP_ITEM_01_P008&7),	(SP_ITEM_05_TENSU&7)),	/*stage6*/	/*sakuya*/		/*---- sakuya撃破後はゲーム的には意味無い */
	#endif
...
/*---------------------------------------------------------
	...
	使い魔 / ボス / 共通
---------------------------------------------------------*/
extern void void_gekiha_keitai(void);
extern OBJ_CALL_FUNC(common_create_item)
{
	enum
	{
		ITEM_00_stage_fantasm = 0,	// (0)
		ITEM_01_stage_1,			// (1)
		ITEM_02_stage_2,			// (2)
		ITEM_03_stage_3,			// (3)
		ITEM_04_stage_4,			// (4)
		ITEM_05_stage_5,			// (5)
		ITEM_06_stage_6,			// (6)
		ITEM_07_stage_extra,		// (7)
		ITEM_MAX					// (8)
	};
	#define ITEM_CREATE_MODE_00 	((8/*ITEM_MAX*/)*0)
	#define ITEM_CREATE_MODE_01 	((8/*ITEM_MAX*/)*1)
	const u32 u32_item_tbl[ITEM_MAX*2] =/* easy, normal, hard, lunatic */
	{
	//	PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7),  (SP_ITEM_04_BOMB&7),  (SP_ITEM_04_BOMB&7),  (SP_ITEM_04_BOMB&7)),	/*[stage 3]*/  /*kaguya*/
	#if 1/*(固定)*/
	/* (r41-) */
	/*mode 00: 撃破中 */
		PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_00_P001&7),  (SP_ITEM_00_P001&7)),	/*[stage fantasm]*/
		PACK_NAN_IDO_CONST((SP_ITEM_00_P001&7),  (SP_ITEM_00_P001&7),  (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7)), /*[stage 1]*/  /*alice*/
		PACK_NAN_IDO_CONST((SP_ITEM_01_P008&7),  (SP_ITEM_00_P001&7),  (SP_ITEM_00_P001&7),  (SP_ITEM_00_P001&7)),	/*[stage 2]*/  /*mima*/
		PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_00_P001&7)),	/*[stage 3]*/  /*kaguya*/
		PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_00_P001&7)),	/*[stage 4]*/  /*aya*/
		PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_00_P001&7)),	/*[stage 5]*/  /*pache*/
		PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_00_P001&7)),	/*[stage 6]*/  /*sakuya*/
		PACK_NAN_IDO_CONST((SP_ITEM_04_BOMB&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_01_P008&7),  (SP_ITEM_00_P001&7)),	/*[stage extra]*/
	/*mode 01: 撃破後 */
		PACK_NAN_IDO_CONST((SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7)), /*[stage fantasm]*/
		PACK_NAN_IDO_CONST((SP_ITEM_03_1UP&7),	 (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7)), /*[stage 1]*/  /*alice*/
		PACK_NAN_IDO_CONST((SP_ITEM_03_1UP&7),	 (SP_ITEM_02_P128&7),  (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7)), /*[stage 2]*/  /*mima*/
		PACK_NAN_IDO_CONST((SP_ITEM_03_1UP&7),	 (SP_ITEM_03_1UP&7),   (SP_ITEM_02_P128&7),  (SP_ITEM_01_P008&7)),	/*[stage 3]*/  /*kaguya*/
		PACK_NAN_IDO_CONST((SP_ITEM_03_1UP&7),	 (SP_ITEM_02_P128&7),  (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7)), /*[stage 4]*/  /*aya*/
		PACK_NAN_IDO_CONST((SP_ITEM_05_TENSU&7), (SP_ITEM_03_1UP&7),   (SP_ITEM_02_P128&7),  (SP_ITEM_02_P128&7)),	/*[stage 5]*/  /*pache*/
		PACK_NAN_IDO_CONST((SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7)), /*[stage 6]*/  /*sakuya*/	   /*---- sakuya撃破後はゲーム的には意味無い */
		PACK_NAN_IDO_CONST((SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7), (SP_ITEM_05_TENSU&7)), /*[stage extra]*/
	#endif
	};
	int item_create_mode = ITEM_CREATE_MODE_00; /* アイテム吐く */
	if (void_gekiha_keitai == card.boss_move_card_callback_void)/*[この条件なら撃破後の筈]*/
	{
		item_create_mode = ITEM_CREATE_MODE_01; /* ボーナスアイテムを出す */
	}
...
//		VR04_NAN_IDO_DECODE = OBJ_Z03_ITEM + u32_item_tbl[item_create_mode+(((cg_VR20_game_difficulty)+i)&((ITEM_MAX)-1))]/*SP_ITEM_03_1UP */
//	for (VR03_i=0; VR03_i<(1)+((((unsigned)cg.game_now_stage>>1))&0x0f); VR03_i++)
	{
		VR04_NAN_IDO_DECODE = u32_item_tbl[item_create_mode + ((cg.game_now_stage) & (ITEM_MAX - 1))];/*SP_ITEM_03_1UP */
		CPU_CALL(nan_ido);
		item_create_mono(src, (OBJ_Z03_ITEM + VR04_NAN_IDO_DECODE) );
	}
	for (VR03_i=0; VR03_i<(16); VR03_i++)/*(7)*/
	{
...
extern /*static*/ OBJ_CALL_FUNC(item_create_random_table)
{
...
//	/* lunatic */	t256(1.5),	/*原作風*/
	...
	return (((convert_score)*(mul_tbl256[(cg_VR20_game_difficulty)]))>>8);
}
...
extern void player_dummy_add_score(u32 score_num_pts)
{
	...
	if (PLAYER_MAX_SCORE < cg_game_score) /* カンスト チェック */
	{
		cg_game_score = PLAYER_MAX_SCORE;
	}
...
static /*u32*/s32 extend_check_score;/*[仕様変更s32で十分]*/
...
extern void player_check_extend_score(void)
{
	...
	#if (1==USE_EXTEND_CHECK)
	if (extend_check_score < cg_game_score) /* エクステンド チェック */
	{
		cg_zanki++; 	/* エクステンド */
		/*
		...
		*/
		...
		AUDIO_19_voice_truck	= TRACK01_PICHUN;/*予備(必要)*/ 				CPU_CALL(cpu_voice_play);
		{
			/*
		...
		//	extend_check_score *= score(10000);
		}
	}
	#endif /* (1==USE_EXTEND_CHECK) */
}
