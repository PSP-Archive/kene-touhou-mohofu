...
/*game_core.c load_stage.c*/global GAME_COMMAND *stage_command_table = NULL;
...
//	sprintf(my_file_common_name, DIRECTRY_NAME_DATA_STR "/dat/" "stage%02d.dat", cg.game_now_stage);
//	sprintf(my_file_common_name, DIRECTRY_NAME_DATA_STR "/dat/" "stage%01d.txt", cg.game_now_stage);
//	sprintf(my_file_common_name, DIRECTRY_NAME_DATA_STR "/dat/" "stage%c.txt", ('0'+ cg.game_now_stage));
//	sprintf(my_file_common_name, DIRECTRY_NAME_DATA_STR "/dat/" "stage%c.txt", ('0'+ cg.game_now_stage));
...
{
	spell_cpu_common_reset();		/* 弾消しコールバックを道中ザコ/ボス/道中中ボスと共用している都合上ここに必要。 */
	#if (1)
	spell_cpu_danmaku_callback_douchuu_init();
