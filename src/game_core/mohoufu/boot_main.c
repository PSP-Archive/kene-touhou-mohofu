...
---------------------------------------------------------*/
	...
	良く解からない理由で幻想入り	http://mohou.huuryuu.com/
#endif
...
static int exit_callback(int arg1, int arg2, void *common)
{
	...
	cg.main_call_func = NULL;/*(アプリ終了指示)*/
	#if (1)/*(要るかも)*/
	/* 本来 cg.main_call_func は セマフォ管理すべきかも。sceKernelCreateSema()?? だけどしてない。 */
	sceKernelDelayThread(1000000);/*(コールバック側が1秒待つ)*/
	cg.main_call_func = NULL;/*(もう一回終了指示)*/
	#endif
	return (0);/*(コールバック側の終了)*/
}
...
static void game_main(void)
{
	{
my_loop:
		if (NULL == cg.main_call_func)
		{
			return; 	/* 終了する。 */
		}
		{
			cg.main_call_func(); /* 各処理を実行 */
		}
...
//	新規にスレッドを作成(regist_home_key [HOME]キーで終了するためのコールバックを登録)
	#endif
	{
		SceUID/*int*/ cb_thread_id;
		// たぶんスタックサイズは 512 で十分。優先度==17はよく知らない。(80==rRootage)
	//	cb_thread_id = sceKernelCreateThread("Update Thread", CallbackThread, 0x11, 512/*4000 0xFA0*/, /*0*/PSP_THREAD_ATTR_USER, NULL); /* スレッドの優先度等を設定 */
	//	cb_thread_id = sceKernelCreateThread("Update Thread", CallbackThread, 0x11, 4000/*4000 0xFA0*/, /*0*/PSP_THREAD_ATTR_USER, NULL); /* スレッドの優先度等を設定 */
		cb_thread_id = sceKernelCreateThread("Update Thread", CallbackThread, 80, 4000/*2000 0x7d0*/, /*0*/PSP_THREAD_ATTR_USER, NULL); /* スレッドの優先度等を設定 */
	//	[HOME]キーのスレッドは優先順位が低くて良いので、[rRootage]の真似をしてスレッドの優先度を下げてみる。(スレッドの優先度17 -> 80。 別に何も変わらない気がする。)
	//	スタックサイズも模倣風の場合別に要らない。
		if (0 > cb_thread_id)
		{	/* スレッドが作成出来ない場合 */
			#if (0)/*(たぶんここが有効でも大丈夫だと思う)*/
		//	けど、一応互換性[Vitaとかよく知らないし]を考慮して無効にしとく。
		//	まあ万一スレッドが作成出来なくても[模倣風の場合終了とかに][今r43の処は]問題無いですし。
		//	[将来的には/saveloadや描画が別スレッドになるかもしれないので/判らない]。
		//
			sceKernelExitGame();/*(強制終了)*/
			#endif
			;
		}
		else
		{	/* スレッドが作成出来たら */
			sceKernelStartThread(cb_thread_id, 0, NULL);	/* スレッド開始 */
		}
	}
