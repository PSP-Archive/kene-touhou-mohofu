...
void floor1_free_info(vorbis_info_floor *i)
{
	...
	if (info)
	{
		if (info->class)			{	u_ogg_free(info->class);}
		if (info->partitionclass)	{	u_ogg_free(info->partitionclass);}
		if (info->postlist) 		{	u_ogg_free(info->postlist);}
		if (info->forward_index)	{	u_ogg_free(info->forward_index);}
		if (info->hineighbor)		{	u_ogg_free(info->hineighbor);}
		if (info->loneighbor)		{	u_ogg_free(info->loneighbor);}
...
{
	char *Aaa = index;
	char *Bbb = u_ogg_malloc(n*sizeof(*Bbb));
...
/*static*/ vorbis_info_floor *floor1_info_unpack (vorbis_info *vi,oggpack_buffer *opb)
{
	...
	vorbis_info_floor1 *info = (vorbis_info_floor1 *)u_ogg_calloc(1,sizeof(*info));
	/* read partitions */
	info->partitions = oggpack_read(opb,5); /* only 0 to 31 legal */
	info->partitionclass = (char *)u_ogg_malloc(info->partitions*sizeof(*info->partitionclass));
	signed int maxclass;
	maxclass = -1;
	...
	info->class = (floor1class *)u_ogg_malloc((maxclass+1)*sizeof(*info->class));
	...
	info->postlist		= (ogg_uint16_t *)u_ogg_malloc((count+2)*sizeof(*info->postlist));
	info->forward_index = (char *)u_ogg_malloc((count+2)*sizeof(*info->forward_index));
	info->loneighbor	= (char *)u_ogg_malloc(count*sizeof(*info->loneighbor));
	info->hineighbor	= (char *)u_ogg_malloc(count*sizeof(*info->hineighbor));
