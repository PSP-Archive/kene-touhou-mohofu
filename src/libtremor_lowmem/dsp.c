...
{
	vorbis_dsp_state *v=u_ogg_calloc(1,sizeof(*v));
	codec_setup_info *ci=(codec_setup_info *)vi->codec_setup;

	v->vi=vi;

	v->work=(ogg_int32_t **)u_ogg_malloc(vi->channels*sizeof(*v->work));
	v->mdctright=(ogg_int32_t **)u_ogg_malloc(vi->channels*sizeof(*v->mdctright));

	int i;
	for (i=0;i<vi->channels;i++)
	{
		v->work[i]=(ogg_int32_t *)u_ogg_calloc(1,(ci->blocksizes[1]>>1)* sizeof(*v->work[i]));
		v->mdctright[i]=(ogg_int32_t *)u_ogg_calloc(1,(ci->blocksizes[1]>>2)* sizeof(*v->mdctright[i]));
	}
...
void vorbis_dsp_destroy(vorbis_dsp_state *v)
{
	...
	if (v)
	{
		...
		{
			for (i=0;i<vi->channels;i++)
			{	if (v->work[i]) 	{u_ogg_free(v->work[i]);}
			}
			u_ogg_free(v->work);
		}
		if (v->mdctright)
		{
			for (i=0;i<vi->channels;i++)
			{	if (v->mdctright[i])	{u_ogg_free(v->mdctright[i]);}
			}
			u_ogg_free(v->mdctright);
		}
		u_ogg_free(v);
	}
}
