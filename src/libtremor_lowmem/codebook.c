	...
	oggpack_buffer *opb,int maptype)
{
	...
	if (s->dec_nodeb==4)
	{
		s->dec_table=u_ogg_malloc((s->used_entries*2+1)*sizeof(*work));
	...
	s->dec_table = u_ogg_malloc((s->used_entries*(s->dec_leafw+1)-2)*s->dec_nodeb);
...
void vorbis_book2_clear(codebook *b)
{
	...
	if (b->q_val)u_ogg_free(b->q_val);
	if (b->dec_table)u_ogg_free(b->dec_table);
...
int vorbis_book2_unpack(codebook *s, oggpack_buffer *opb)
{
	...
	case 1:
		...
		/* dec_type choices here are 1,2; 3 doesn't make sense */
		{
			...
			else
			{
				...
				if (s->q_bits<=8)
				{
					s->q_val=u_ogg_malloc(quantvals);
					for (i=0;i<quantvals;i++)
					{
				...
				{
					s->q_val=u_ogg_malloc(quantvals*2);
					for (i=0;i<quantvals;i++)
					{
						((ogg_uint16_t *)s->q_val)[i]=oggpack_read(opb,s->q_bits);
					}
	...
	case 2:
		...
		else
		{
			...
			s->q_val=u_ogg_malloc(s->q_pack*s->used_entries);
