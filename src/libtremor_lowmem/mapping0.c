...
void mapping_clear_info(vorbis_info_mapping *info)
{
	if (info)
	{
		if (info->chmuxlist)u_ogg_free(info->chmuxlist);
		if (info->submaplist)u_ogg_free(info->submaplist);
		if (info->coupling)u_ogg_free(info->coupling);
...
int mapping_info_unpack(vorbis_info_mapping *info, vorbis_info *vi, oggpack_buffer *opb)
{
	...
	if (oggpack_read(opb,1))
	{
		...
		info->coupling = u_ogg_malloc(info->coupling_steps*sizeof(*info->coupling));
	...
	if (info->submaps>1)
	{
		info->chmuxlist = u_ogg_malloc(sizeof(*info->chmuxlist)*vi->channels);
	...
	info->submaplist = u_ogg_malloc(sizeof(*info->submaplist)*info->submaps);
