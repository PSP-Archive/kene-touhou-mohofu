...
void res_clear_info(vorbis_info_residue *info)
{
	if (info)
	{
		if (info->stagemasks)	{	u_ogg_free(info->stagemasks);}
		if (info->stagebooks)	{	u_ogg_free(info->stagebooks);}
	...
	vorbis_info *vi,oggpack_buffer *opb)
{
	...
	info->stagemasks = u_ogg_malloc(info->partitions*sizeof(*info->stagemasks));
	info->stagebooks = u_ogg_malloc(info->partitions*8*sizeof(*info->stagebooks));
