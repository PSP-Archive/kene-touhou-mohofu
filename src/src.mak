...
# 模倣風ゲーム本体
include $(SRC)/game_core/$(TARGET)/game_core.mak
...
#------------------------------------------------------------------------------
# Portable psp Dx customized library.
#------------------------------------------------------------------------------
ifeq ($(TARGET),mohoufu)
	##not include
	LINK_DXPL = 0
	CFLAGS += -DLINK_PGC_TYPE=0
else
	##use include
	LINK_DXPL = 1
	CFLAGS += -DLINK_PGC_TYPE=2
endif

ifneq ($(LINK_DXPL),1)
	##not include
else
include $(SRC)/dxlib/dxlib.mak

	OBJS += $(OBJ)/libpsp_vram/psp_valloc.o
#	OBJS += $(OBJ)/libpsp_vram/psp_vram.o
	OBJDIRS += $(OBJ)/libpsp_vram
endif
