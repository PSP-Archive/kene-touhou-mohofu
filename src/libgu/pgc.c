...
//#define NOT_USED_NOW (1)/*test*/
#define NOT_USED_NOW (TEST0)/*(release)*/

/*(内部拡張設定)*/
#if (0==LINK_PGC_TYPE)/*(模倣風用設定)*/
...
#define USE_sceGuCallMode		(0)
#define USE_sceGuClearStencil	(0)
#define USE_sceGuSpriteMode 	(0)
...
typedef struct
{
	...
	/* row 2 */
	u8 ambient; 	// Ambient color
	u8 diffuse; 	// Diffuse color
	u8 specular;	// Specular color
	u8 constant;	// Constant attenuation

	/* row 3 */
	u8 linear;		// Linear attenuation
	u8 quadratic;	// Quadratic attenuation
	u8 exponent;	// Light exponent
	u8 cutoff;		// Light cutoff
} GuLightSettings;
...
//
//xxx//static u32 gu_current_frame;
...
#if (1==USE_GU_STACK_32_OBJECT)/*(gu_object_stack)*/
//(?)static u32 *gu_object_stack[32];
#endif /*(1==USE_GU_STACK_32_OBJECT)*/
// flags
static int gu_curr_context; /* GU_DIRECT:0, or GU_CALL:1, or GU_SEND:2, only. */

#if (1==USE_GU_STATE)
static u32 gu_states;/*(need 22bit)*/
#endif /*(1==USE_GU_STATE)*/

static u8 gu_init_flag;
static u8 gu_display_on;
#if (1==USE_sceGuCallMode)
static u8 gu_call_mode;
#endif /*(1==USE_sceGuCallMode)*/
#if (1==USE_GU_STACK_32_OBJECT)/*(gu_object_stack)*/
...
#if 000
static const GuLightSettings light_settings[4] =
{
	{
		0x18, 0x5f, 0x63, 0x64,
		0x65, 0x6f, 0x70, 0x71,
...
void *sceGuGetMemory(int size)
{
	...
	u32 *orig_ptr	= gu_list->current;
	u32 *new_ptr	= (u32 *)(((u32)orig_ptr) + size + 8);

	int lo = ( 8 << 24) |  (((u32)new_ptr)		 & 0x00ffffff);
	int hi = (16 << 24) | ((((u32)new_ptr) >> 8) & 0x0f0000  );
...
//	pspGuSendCommandi(tbpcmd_tbl[mipmap],	((u32)tbp) & 0x00ffffff);
//	pspGuSendCommandi(tbwcmd_tbl[mipmap],	((((u32)tbp) >> 8) & 0x0f0000)|tbw);
//	pspGuSendCommandi(tsizecmd_tbl[mipmap], (s_getExp(height) << 8)|(s_getExp(width)));
	pspGuSendCommandi(/*tbp_cmd*/	(0xa0+mipmap),	  (((u32)tbp)		 & 0x00ffffff) );
	pspGuSendCommandi(/*tbw_cmd*/	(0xa8+mipmap),	(((((u32)tbp) >> 8) & 0x0f0000	)|tbw) );
...
void sceGuColorFunc(int func, u32 color, u32 mask)
{
...
void sceGuClearColor(u32 color)
{
	GuContext *context = &gu_contexts[gu_curr_context];
...
//void sceGuTexEnvColor(u32 color)
//{
...
{
	GuContext *context = &gu_contexts[gu_curr_context];
	context->scissor_start[0]	= x0;
	context->scissor_start[1]	= y0;
	context->scissor_end[0] 	= x1-1;
	context->scissor_end[1] 	= y1-1;
...
void sceGuDrawArray(int prim, int vtype, int count, const void *indices, const void *vertices)
{
	...
	if (indices)
	{
		pspGuSendCommandi(16, (((u32)indices) >> 8) & 0x0f0000);
		pspGuSendCommandi( 2,  ((u32)indices)		 & 0x00ffffff);
	}
	if (vertices)
	{
		pspGuSendCommandi(16, (((u32)vertices) >> 8) & 0x0f0000);
		pspGuSendCommandi( 1,  ((u32)vertices)	  & 0x00ffffff);
	}
...
void sceGuClear(int flags)
{
	...
	#if (0==USE_sceGuClearStencil)
	filter = (context->clear_color & 0x00ffffff);
	#else
	...
	/*(GU_FAST_CLEAR_BITがある場合) (64SLICEでクリア) */
	{
		...
		#if 0//てすと(PGC_ON==PGC_ON_FOR_MOHOUFU)
		count		= 2;/*(模倣風用設定)*/
		#else
		count		= 16;
		#endif
...
void *sceGuSwapBuffers(void)
{
	...
	{	sceDisplaySetFrameBuf(
			(void *)((u32)ge_edram_address + (u32)gu_draw_buffer.disp_buffer),
...
int sceGuFinish(void)
{
	...
	u32 size = ((u32)gu_list->current) - ((u32)gu_list->start);
...
void sceGuStart(int cid, void *list)
{
	...
	u32 *local_list = (u32 *)(((u32)list) | 0x40000000);
	// setup display list
	...
	if (!gu_curr_context)
	{
		...
		{
			pspGuSendCommandi(156,	 ((u32)gu_draw_buffer.frame_buffer) & 0x00ffffff);
			pspGuSendCommandi(157, ((((u32)gu_draw_buffer.frame_buffer) & 0xff000000) >> 8) | gu_draw_buffer.frame_width);
		}
	}
}
...
void sceGuDepthBuffer(void* zbp, int zbw)
{
	...
	pspGuSendCommandi(158, ((u32)zbp) & 0xffffff);
	pspGuSendCommandi(159, ((((u32)zbp) & 0xff000000) >> 8)|zbw);
}
#endif
//
...
void sceGuClearDepth(u32 depth)
{
	GuContext* context = &gu_contexts[gu_curr_context];
...
void sceGuClutMode(u32 cpsm, u32 shift, u32 mask, u32 a3)
{
	u32 argument = (cpsm) | (shift << 2) | (mask << 8) | (a3 << 16);
	pspGuSendCommandi(197, argument);
}

void sceGuClutLoad(int num_blocks, const void* cbp)
{
	pspGuSendCommandi(176, ((u32)cbp) & 0xffffff);
	pspGuSendCommandi(177, (((u32)cbp) >> 8) & 0xf0000);
...
void sceGuDrawBufferList(int psm, void* fbp, int frame_width)
{
	...
	pspGuSendCommandi(156,	 ((u32)fbp) & 0x00ffffff);
	pspGuSendCommandi(157, ((((u32)fbp) & 0xff000000) >> 8) | frame_width);
}
#endif
void sceGuDrawBuffer(int psm, void *fbp, int frame_width)
{
	...
	{	gu_draw_buffer.depth_buffer = (void *)(((u32)fbp) + (u32)((gu_draw_buffer.height * frame_width) << 2));
	}
	...
	#endif
	#if 1
	pspGuSendCommandi(158,	 ((u32)gu_draw_buffer.depth_buffer) & 0x00ffffff);
	pspGuSendCommandi(159, ((((u32)gu_draw_buffer.depth_buffer) & 0xff000000) >> 8) | gu_draw_buffer.depth_width);
	#endif
}
