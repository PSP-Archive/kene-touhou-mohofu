...
MM_s8* MusicMod_InfoLoader(void)
{
	...
	if (len)
	{
		list = mod_music_mem_alloc(len*sizeof(MM_s8));
		if (list)
		{
			list[0] = 0;
...
MM_BOOL ReadComment(u16 len)
{
	...
	{
		mmoo.comment = (MM_s8*)mod_music_mem_alloc(len+1);
		if (!(mmoo.comment))
		{
			return (0);/*(é∏îs)*/
		}
...
MM_BOOL ReadLinedComment(u16 lines, u16 linelen)
{
	...
	if (lines)
	{
		tempcomment 	= (MM_s8*)mod_music_mem_alloc(len+1);
		if (!tempcomment)	{return (0);}
		storage 		= (MM_s8*)mod_music_mem_alloc(linelen+1);
		if (!storage)
		{
			free(tempcomment);
			return (0);
		...
		if (total>lines)
		{	mmoo.comment	= (MM_s8*)mod_music_mem_alloc(total+1);
			if (!(mmoo.comment))
			{
				free(storage);
				free(tempcomment);
...
MM_s8 *DupStr(MM_s8* s, u16 len, MM_BOOL strict)
{
	...
	MM_s8 *d /*= NULL*/;
	d = (MM_s8*)mod_music_mem_alloc(sizeof(MM_s8)*(len+1));
	if (d)
	{
...
{
	MOD_MUSIC_MODULE *mf;
	mf = mod_music_mem_alloc(sizeof(MOD_MUSIC_MODULE));
	return (mf);
}
