...
static MM_BOOL IT_Init(void)
{
	if (!(mh_it 				= (ITHEADER*)	mod_music_mem_alloc(		  sizeof(ITHEADER))))		{return (0);}
	if (!(mm_position_look_up	= (u8*) 		mod_music_mem_alloc(	256 * sizeof(u8)))) 			{return (0);}
	if (!(itpat 				= (ITNOTE*) 	mod_music_mem_alloc(200*64 * sizeof(ITNOTE)))) 		{return (0);}
	if (!(mask					= (u8*) 		mod_music_mem_alloc(	 64 * sizeof(u8)))) 			{return (0);}
	if (!(last					= (ITNOTE*) 	mod_music_mem_alloc(	 64 * sizeof(ITNOTE)))) 		{return (0);}
	return (1);
}
...
/*
	簡易版でも無問題。
	s-jis は isalnum() や toupper() 使ってる時点で端から非対応。
...
//	}
	...
	//
	paraptr_it = (u32*)mod_music_mem_alloc((mh_it->insnum+mh_it->smpnum+mmoo.numpat)* sizeof(u32));
	if (!paraptr_it)
	{
		return (0);
	}
