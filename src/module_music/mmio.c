...
#endif
{
	MYMOD_FILE_READER* reader = (MYMOD_FILE_READER*)mod_music_mem_alloc(sizeof(MYMOD_FILE_READER));
	if (reader)
	{
		/* ファイルアクセスはクラス化して隠蔽 */
...
{
	MFILEWRITER* writer=(MFILEWRITER*)mod_music_mem_alloc(sizeof(MFILEWRITER));
	if (writer)
	{
