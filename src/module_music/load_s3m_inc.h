...
{
	s3mbuf 					= (S3MNOTE*)mod_music_mem_alloc(32*64*sizeof(S3MNOTE));	if (!(s3mbuf))					{return (0);}
	mh_s3m 					= (S3MHEADER*)mod_music_mem_alloc(sizeof(S3MHEADER));		if (!(mh_s3m))					{return (0);}
	mm_position_look_up 	= (u8*)mod_music_mem_alloc(sizeof(u8)*256);			if (!(mm_position_look_up)) 	{return (0);}
...
//	mmoo.bpmlimit 	= 32;
	...
	paraptr_s3m = (u16*)mod_music_mem_alloc((mmoo.numins+mmoo.numpat)*sizeof(u16));
	if (!(paraptr_s3m))
	{return (0);
	}
