...
#------------------------------------------------------------------------------
# revision setting. バージョンや名前を決めます。
#------------------------------------------------------------------------------

TARGET = mohoufu

#RELESE_DATE = 2013/12/08(r43)冬
#RELESE_DATE = 2014/06/08(r44)夏
#RELESE_DATE = 2015/01/01(r45)冬
#RELESE_DATE = 2016/01/01(r46)冬

RELESE_DATE = 2014/06/08
...
#------------------------------------------------------------------------------
# select laguage. エーゴな人の場合です。
#------------------------------------------------------------------------------
# uncommented if, languege english. dafault japanese.
# CFLAGS += -DUSE_LANGUAGE_ENGLISH=1

#------------------------------------------------------------------------------
# developping tools setting. コンパイル環境を決めます。
#------------------------------------------------------------------------------
...
# 1 == pspsdk-setup-0.11.2r3.exe(2011-06-01) を使う。
...
# devkitPSP でコンパイルする場合。(windows 7)
# USE_MINIMALIST_PSP_SDK = 1 にしとく。
# devkitPSP の 名前と場所を C:\pspsdk\ にしちゃう。
# ( Minimalist があるならMinimalistの名前を一時的に変えとく C:\pspsdk_minimalist_v0_11_1_GCC435\ とか)
# compile_for_devkitPSP.txt にあるファイルを Minimalist PSP homebrew SDK for Windows 0.11.2r3. から
# 持ってきて、コピーしとく。(これらは現時点(r42==20130602)で標準的なもの)
# devkitPSP r15(GCC4.3.6) と
# devkitPSP r16_1(GCC4.6.2) は、
# とりあえず(r42で)チェックして ok でした。
# (r41だと /src/PSPL/audio/mixer/mixer.c::sdl_mixer_play_channel() にバグがあるので、
# r41だと r16_1 ではハングアップする。checkchunkintegral()のチェック処理がおかしいので外した方が良い。)
#
# devkitPSP r16(?) と devkitPSP r14(?) はチェックしてない。

#------------------------------------------------------------------------------
# PSP setting. 対応pspを決めます。
#------------------------------------------------------------------------------
...
PSP_FW_VERSION = 371
# psp-2000の最初期のFWは3.60 M33です。だからこれを使い PSP_FW_VERSION = 371 で動作確認しています。
# この場合せっかくpsp-2000なのに拡張メモリは使用できませんが、
# 3.71以降でもつまり全てのpsp-2000で確実に動くと言う事がチェックできます。
...
include ./$(SRC)/game_core/$(TARGET)/UTF8_title.mak
...
ifneq ($(USE_EBOOT_TITLE),1)
	# 0 == リリース版(タイトルの文字無し)
	PSP_EBOOT_ICON	 = ./$(SRC)/game_core/$(TARGET)/resource/ICON0.PNG
	PSP_EBOOT_ICON1  = ./$(SRC)/game_core/$(TARGET)/resource/ICON0.PMF
	PSP_EBOOT_UNKPNG = ./$(SRC)/game_core/$(TARGET)/resource/ICON1.PNG
	PSP_EBOOT_PIC1	 = ./$(SRC)/game_core/$(TARGET)/resource/PIC1.PNG
	PSP_EBOOT_SND0	 = ./$(SRC)/game_core/$(TARGET)/resource/SND0.AT3
else
	...
	# 1 == アップデート版(タイトルの文字あり)
	PSP_EBOOT_ICON	 = ./$(SRC)/game_core/$(TARGET)/resource/ICON0.PNG
#test	PSP_EBOOT_ICON1  = ./$(SRC)/game_core/$(TARGET)/resource/ICON0.PMF
	PSP_EBOOT_UNKPNG = 
	PSP_EBOOT_PIC1	 = 
#test	PSP_EBOOT_SND0	 = ./$(SRC)/game_core/$(TARGET)/resource/SND0.AT3
endif

# 通常開発
# この Makefile で作成するターゲットを指定。
EXTRA_TARGETS		= mk_dir EBOOT.PBP

# Make clean で、特別に消したいものがあれば指定。
# (PSPSDK側の makefile の cleanにオーバラップしている)
# EXTRA_CLEAN		= 
...
# pspsdk-setup-0.11.1 / pspsdk-setup-0.11.2r3
PSPDEV = C:/pspsdk
...
# 位置が悪い場合
#＜略＞
#er.o obj/mohoufu/hiscore.o obj/mohoufu/fps.o obj/mohoufu/soundmanager.o obj/mohoufu/bg.o -lS
...
#＜略＞
#er.o obj/mohoufu/hiscore.o obj/mohoufu/fps.o obj/mohoufu/soundmanager.o obj/mohoufu/bg.o -lp
...
#[pspに関係なく gcc に「valloc();」という名前の関数があり、名前が同じなので正常動作できない。]LIBS += -lpspvalloc
...
#$(OBJ)/game_core/$(TARGET)/%.o: $(SRC)/game_core/$(TARGET)/%.cpp
...
$(OBJ)/game_core/$(TARGET)/%.o: $(SRC)/game_core/$(TARGET)/%.c
...
# エラーが出るのでとりあえず。(???????)。
$(OBJ)/dxlib/safe_alloc.o: $(SRC)/dxlib/safe_alloc.c
	psp-gcc $(CFLAGS_OPTIMIZE) $(CFLAGS) $(LIB_CFLAGS) -c $< -o $@

# エラーが出るのでとりあえず。(???????)。
$(OBJ)/libpsp_vram/psp_valloc.o: $(SRC)/libpsp_vram/psp_valloc.c
	psp-gcc $(CFLAGS_OPTIMIZE) $(CFLAGS) $(LIB_CFLAGS) -c $< -o $@
...
#--------
#minimalist の make.exe は MD と RM は何故か字下げ出来ない。(謎)
#cygwin の make.exe は MD と RM は字下げ出来る。
#--------
...
DELTREE_OBJ_CORE	= @$(RM) -f -rd $(OBJ)/game_core/$(TARGET)
DELTREE_OBJ_JIKI	= @$(RM) -f -rd $(OBJ)/game_core/$(TARGET)/jiki
else
# pspsdk-setup-0.11.2r3
DELTREE_OBJ_ALL 	= $(RM) -f -r -d $(OBJ)
DELTREE_AUDIO_MIXER	= $(RM) -f -r -d $(OBJ)/audio_mixer
DELTREE_OBJ_CORE	= $(RM) -f -r -d $(OBJ)/game_core/$(TARGET)
DELTREE_OBJ_JIKI	= $(RM) -f -r -d $(OBJ)/game_core/$(TARGET)/jiki
...
a:
	...
	@$(RM) -f $(OBJ)/game_core/$(TARGET)/jiki/jiki.o
	@$(RM) -f $(OBJ)/game_core/$(TARGET)/jiki/jiki_shot.o
	@$(RM) -f $(OBJ)/game_core/$(TARGET)/jiki/jiki_bomber.o
	@$(RM) -f $(OBJ)/game_core/$(TARGET)/my_math.o
	@$(RM) -f $(OBJ)/game_core/$(TARGET)/sprite_bullet.o
	@$(RM) -f $(OBJ)/game_core/$(TARGET)/score_panel.o

#	@$(RM) -f $(OBJ)/game_core/$(TARGET)/*.o
#	@$(RM) -f $(OBJ)/game_core/$(TARGET)/boss/*.o
#	@$(RM) -f $(OBJ)/game_core/$(TARGET)/douchu/*.o
#	@$(RM) -f $(OBJ)/game_core/$(TARGET)/draw/*.o
#	@$(RM) -f $(OBJ)/game_core/$(TARGET)/menu/*.o
#	@$(RM) -f $(OBJ)/game_core/$(TARGET)/tama/*.o

# syomei banをminimalistでやる場合のパッチ。
patch:
	@echo syomei ban minimalist patch.
	@$(RM) -f $(OBJ)/game_core/$(TARGET)/boot_main.o
	@$(RM) -f $(TARGET).prx
	@$(RM) -f $(TARGET).elf
	@$(RM) -f PARAM.SFO
	@$(RM) -f EBOOT.PBP
